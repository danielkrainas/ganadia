...
void do_dual_machine_guns(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		act(AT_PLAIN, "$n presses the fire button.", ch, NULL, argument,
		    TO_ROOM);
		if (number_percent() > chance) {
			sprintf(buf,
				"Dual machine guns fire from %s and miss you.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf,
				"You shoot dual machine guns at %s but miss.",
				target->name);
			...
			sprintf(buf,
				"Dual machine guns fire from %s and barely miss %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 8);
			return;
		}
		sprintf(buf, "Dual machine guns fire from %s and hit %s.",
			ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by dual machine guns fired from %s!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "Your dual machine guns fire and hit %s!.",
			target->name);
...
void do_105mm_rifle(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		if (number_percent() > chance) {
			sprintf(buf,
				"a 105mm Rifle fires from %s and misses you.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf,
				"You shoot the 105mm Rifle at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"a 105mm Rifle fires from %s and barely misses %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 16);
			return;
		}
		sprintf(buf, "a 105mm Rifle fires from %s and hits %s.",
			ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by a 105mm Rifle shot from %s!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
...
void do_30mm_cannon(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		if (number_percent() > chance) {
			sprintf(buf,
				"a 30mm cannon fires from %s but misses you.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf,
				"You shoot the 30mm cannon at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"30mm cannon fire from %s barely misses %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 12);
			return;
		}
		sprintf(buf, "a 30mm cannon fires from %s and hits %s.",
			ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by 30mm cannon fire from %s!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
...
void do_grappling_claw(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		if (abs(target->vx - ship->vx) > 100 ||
			...
			send_to_char
			    ("&RYou must be next to them to use the grappling claw.\n\r",
			     ch);
			return;
		}
		ship->statet0++;
		...
		if (number_percent() > chance) {
			sprintf(buf,
				"%s swings their grappling claw at you but misses.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf, "Your grappling claw misses %s.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"%s swings their grappling claw at %s but misses.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 30);
			return;
		}
		sprintf(buf, "%s hits %s with their grappling claw.",
			ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by %s's grappling claw!", ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "Your grappling claw slams into %s!.",
			target->name);
		echo_to_cockpit(AT_YELLOW, ship, buf);
		learn_from_success(ch, gsn_spacecombat);
		learn_from_success(ch, gsn_spacecombat2);
...
void do_machine_gun(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		if (number_percent() > chance) {
			sprintf(buf, "Machine Gun fire from %s misses you.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			...
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"Machine gun fire from %s barely misses %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 4);
			return;
		}
		sprintf(buf, "Machine gun fire from %s hits %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by machine gun fire from %s!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
...
void do_clustermissilelauncher(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		act(AT_PLAIN, "$n presses the fire button.", ch,
		    NULL, argument, TO_ROOM);
		if (number_percent() > chance) {
			sprintf(buf, " %s launches missiles at you but misses.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf, "You launch missiles at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"Missiles launch from %s barely missing %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 24);
			return;
		}
		sprintf(buf, "Missiles launch from %s and hit %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by missiles launched from %s!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "You launch missiles and they hit %s!.",
			target->name);
		echo_to_cockpit(AT_YELLOW, ship, buf);
		learn_from_success(ch, gsn_spacecombat);
		learn_from_success(ch, gsn_spacecombat2);
...
void do_multiplemunitionslauncher(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		act(AT_PLAIN, "$n presses the fire button.", ch,
		    NULL, argument, TO_ROOM);
		if (number_percent() > chance) {
			sprintf(buf, "Grenades and missiles from %s miss you.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf,
				"You shoot the multiple munition launcher at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"Grenades and missiles fire from %s barely missing %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 30);
			return;
		}
		sprintf(buf, "Grenades and missiles fire from %s hitting %s.",
			ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf,
			"You are hit by grenades and missiles fired from %s!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "Your multiple munition fire hits %s!.",
			target->name);
		echo_to_cockpit(AT_YELLOW, ship, buf);
		learn_from_success(ch, gsn_spacecombat);
		learn_from_success(ch, gsn_spacecombat2);
...
void do_plasma_cannon(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		if (number_percent() > chance) {
			sprintf(buf,
				"The plasma cannon fires from %s missing you.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf,
				"You charge up the plasma cannon and fire at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"The plasma cannon fired from %s barely misses %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 42);
			return;
		}
		sprintf(buf, "The plasma cannon fired from %s hits %s.",
			ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by plasma cannon fire from %s!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf,
			"You charge up the plasma cannon and fire, hitting %s!.",
			target->name);
		echo_to_cockpit(AT_YELLOW, ship, buf);
		learn_from_success(ch, gsn_spacecombat);
		learn_from_success(ch, gsn_spacecombat2);
...
void do_gatling_gun(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		act(AT_PLAIN, "$n presses the fire button.", ch,
		    NULL, argument, TO_ROOM);
		if (number_percent() > chance) {
			sprintf(buf, "Gatling gun fire from %s misses you.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf,
				"You shoot the gatling gun at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"Gatling gun fire from %s barely misses %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 16);
			return;
		}
		sprintf(buf, "Gatling gun fire from %s hits %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "You are hit by %s's gatling gun fire!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "You shoot the gatling gun at %s and hit!",
			target->name);
		echo_to_cockpit(AT_YELLOW, ship, buf);
		learn_from_success(ch, gsn_spacecombat);
		learn_from_success(ch, gsn_spacecombat2);
...
void do_super_plasma_cannon(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		if (number_percent() > chance) {
			sprintf(buf,
				"%s charges up their plasma cannon. They fire it at you but miss.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf,
				"You charge up the plasma cannon. You fire it at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
			learn_from_failure(ch, gsn_spacecombat3);
			sprintf(buf,
				"The plasma cannon shot from %s barely misses %s.",
				ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			WAIT_STATE(ch, 64);
			return;
		...
		sprintf(buf,
			"%s charges up their plasma cannon. They fire it at %s and hit!",
			ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf,
			"%s charges up their plasma cannon. They fire it at you and hit!",
			ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf,
			"You charge up the plasma cannon. You fire it at %s and hit!",
			target->name);
		echo_to_cockpit(AT_YELLOW, ship, buf);
		learn_from_success(ch, gsn_spacecombat);
		learn_from_success(ch, gsn_spacecombat2);
