...
void fread_planet(PLANET_DATA * planet, FILE * fp)
{
	...
	for (;;) {
		...
		case 'F':
			...
			KEY("FrustLevel", planet->frust_level,
			    fread_number(fp));
			if (!str_cmp(word, "FrustratedBy")) {
				planet->frustrated_by =
				    get_clan(fread_string(fp));
				fMatch = TRUE;
			}
			break;
		...
		case 'S':
			if (!str_cmp(word, "Starsystem")) {
				planet->starsystem =
				    starsystem_from_name(fread_string(fp));
	...
	if ((fp = fopen(filename, "r")) != NULL) {
		...
		for (;;) {
			...
			if (!str_cmp(word, "CLAN")) {
				...
				sprintf(buf, "Load_clan_file: bad section: %s.",
					word);
				bug(buf, 0);
				break;
			}
...
void do_overthrow(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->pcdata->clan) {
		send_to_char
		    ("You'll have to quit your clan before you can do that\n\r",
		     ch);
		return;
	}
	...
	if (clan->clan_type == CLAN_CRIME || clan->clan_type == CLAN_GUILD) {
		send_to_char
		    ("You can't overthrow a group of bounty hunters!\n\r", ch);
		return;
	}
	...
	if ((victim = get_char_room(ch, arg)) == NULL
		...
		if (stat(fname, &fst) != -1) {
			...
			if (!str_cmp(clan->leader, d->character->name))
				trank = 1;
			else if (!str_cmp(clan->number1, d->character->name))
				trank = 2;
			else if (!str_cmp(clan->number2, d->character->name))
				trank = 3;
			else if (!str_cmp(clan->immortal, d->character->name))
				trank = 0;
...
//         char_to_room(d->character, old_room_vnum);
			...
			--clan->members;
			if (!str_cmp
			    (d->character->name, ch->pcdata->clan->number1)) {
				STRFREE(ch->pcdata->clan->number1);
				ch->pcdata->clan->number1 = STRALLOC("");
			}
			if (!str_cmp
			    (d->character->name, ch->pcdata->clan->number2)) {
			...
			act(AT_MAGIC, "You outcast $N from $t", ch, clan->name,
			    d->character, TO_CHAR);
			act(AT_MAGIC, "$n outcasts $N from $t", ch, clan->name,
			    d->character, TO_ROOM);
...
void do_setclan(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0') {
		...
		send_to_char
		    (" guard1 guard2 patrol1 patrol2 supportvnum war1 war2\n\r",
		     ch);
	...
	if (!strcmp(arg2, "war1")) {
		...
		if (!strcmp(argument, "none")) {
			if (clan->war1) {
				send_to_char
				    ("Removing War status, don't forgot to remove the other clans. \n\r",
				     ch);
				clan->war1 = NULL;
				save_clan(clan);
				return;
				...
				send_to_char
				    ("This clan has no War1 data to remove!\n\r",
				     ch);
				return;
			}
		}
	...
	if (!strcmp(arg2, "war2")) {
		...
		if (!strcmp(argument, "none")) {
			if (clan->war2) {
				...
				send_to_char
				    ("This clan has no war2 data to remove!\n\r",
				     ch);
				return;
			}
		}
	...
	if (!strcmp(arg2, "ally1")) {
		...
		if (!strcmp(argument, "none")) {
			if (clan->ally1) {
				send_to_char
				    ("Removing ally status, don't forgot to remove the other clans. \n\r",
				     ch);
				clan->ally1 = NULL;
				save_clan(clan);
				return;
				...
				send_to_char
				    ("This clan has no ally1 data to remove!\n\r",
				     ch);
				return;
			}
		}
	...
	if (!strcmp(arg2, "ally2")) {
		...
		if (!strcmp(argument, "none")) {
			if (clan->ally2) {
				...
				send_to_char
				    ("This clan has no ally2 data to remove!\n\r",
				     ch);
				return;
			}
		}
...
void do_setplanet(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0') {
		...
		send_to_char
		    (" name filename starsystem governed_by hospital\n\r", ch);
...
void do_showclan(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch, "Description: %s\n\rImmortal: %s\n\rLeader: %s\n\r",
		  clan->description, clan->immortal, clan->leader);
	ch_printf(ch,
...
void do_clans(CHAR_DATA * ch, char *argument)
{
	...
	for (clan = first_clan; clan; clan = clan->next) {
		...
		ch_printf(ch,
			  "  &CPlanets&R: &W%-2d       &CAvg Pop Support&R: &W%-3d&W    &CRevenue&R: &W%-10ld\n\r",
			  pCount, support, revenue);
		ch_printf(ch,
			  "  &CDivision      Leaders                 Spacecraft&R/&CVehicles&R/&CMembers&R/&CFunds\n\r");
		ch_printf(ch,
			  "  &O%-15s %-10s %-10s %-10s %-10s%-2d %-2d %-3d %s\n\r",
			  "&CMain", clan->immortal, clan->leader, clan->number1,
			  clan->number2, clan->spacecraft, clan->vehicles,
			  clan->members, num_punct(clan->funds));
		ch_printf(ch, "  &CAt War With&R: &W%s%s%s%s.\n\r",
			  (clan->war1) ? clan->war1->name : "", (clan->war1
								 && clan->
								 war2) ? " and "
			  : "", (clan->war2) ? clan->war2->name : "",
			  (!clan->war1 && !clan->war2) ? "No one" : "");
		ch_printf(ch, "  &CCurrently Allies With&R:&W %s%s%s%s.\n\r",
			  (clan->ally1) ? clan->ally1->name : "", (clan->ally1
								   && clan->
								   ally2) ?
			  " and " : "", (clan->ally2) ? clan->ally2->name : "",
...
*/
		if (clan->first_subclan) {
			...
			for (subclan = clan->first_subclan; subclan;
				...
				ch_printf(ch,
					  "  &c%-20s %-10s %-10s %-10s %-2d %d %-3s %ld\n\r",
					  subclan->name, subclan->leader,
					  subclan->number1, subclan->number2,
					  subclan->spacecraft,
					  subclan->vehicles, subclan->members,
	...
	ch_printf(ch,
		  "&CAutonomous Groups      Leaders             Spacecraft&R/&CVehicles&R/&CMembers&R/&CFunds&W\n\r");
	for (clan = first_clan; clan; clan = clan->next) {
		if (clan->clan_type != CLAN_CRIME)
			continue;

		ch_printf(ch,
			  "&C%-22s &c%-10s %-10s %-10s %-2d %-2d %-3d %s\n\r",
...
void do_newclans(CHAR_DATA * ch, char *argument)
{
	...
	for (clan = first_clan; clan; clan = clan->next) {
		...
		ch_printf(ch,
			  "  &BPlanets&r: &z%-2d	&BAvg Pop Support&r: &z%-3d&z\n\r",
			  pCount, support);

		if (ch->pcdata->clan == clan || IS_IMMORTAL(ch)) {
			ch_printf(ch,
				  "  &BOfficers&r:&z %-8s %-8s %-8s %-8s\r\n",
				  clan->immortal, clan->leader, clan->number1,
				  clan->number2);
			ch_printf(ch,
				  "  &BSpacecrafts Owned&r: &z%-2d&O    &BFunds&r: &z%-3s   &BRevenue&r:&z%-10ld \r\n",
				  clan->spacecraft, num_punct(clan->funds),
				  revenue);
			...
			if (clan->war1 && clan->war1->name) {
				if (clan->war1->war1
				    && clan->war1->war1 == clan) {
					ch_printf(ch,
						  "  &BWar with %s status&r: &z%s\r\n",
						  clan->war1->name,
						  (clan->war1counter ==
						   clan->war1->
						   war1counter) ? "Tied"
						  : (clan->war1counter >
						     clan->war1->war1counter)
						  ? "Winning" : "Losing");
				}
				...
				if (clan->war2->war2
					...
					ch_printf(ch,
						  ...
						  (clan->war2counter ==
						   ...
						   war2counter) ? "Tied"
						  : (clan->war2counter >
						     clan->war2->war2counter)
						  ? "Winning" : "Losing");
			}

			ch_printf(ch,
				  "  &BCurrently Allies With&r:&z %s%s%s%s.\n\r",
...
void do_planets(CHAR_DATA * ch, char *argument)
{
	...
	for (planet = first_planet; planet; planet = planet->next) {
		ch_printf(ch,
			  "&CPlanet&R: &W%-15s   &CGoverned By&R: &W%s %s\n\r",
			  planet->name,
			  planet->governed_by ? planet->governed_by->name : "",
			  IS_SET(planet->flags,
				 PLANET_NOCAPTURE) ? "(permanent)" : "");
		ch_printf(ch, "&CValue&R: &W%-10ld&R/&W%-10d ",
			  get_taxes(planet), planet->base_value);
		ch_printf(ch,
			  "&CPopulation&R: &W%-5d   &C Pop Support&R: &W%.1f%\n\r",
			  planet->population, planet->pop_support);
		ch_printf(ch,
			  "&CFrustrated By&R: &W%-5s	&CTolerance Level&R: &W%.1f %%\n\r",
			  (planet->frustrated_by) ? planet->
			  frustrated_by->name : "None",
...
void do_guilds(CHAR_DATA * ch, char *argument)
{
	...
	for (clan = first_clan; clan; clan = clan->next) {
		...
		ch_printf(ch,
			  "  &CDivision             Leaders             Spacecraft&R/&CVehicles&R/&CMembers&R/&CFunds\n\r");
		ch_printf(ch,
			  "  &O%-20s %-10s %-10s %-10s %-2d %-2d %-3d %ld\n\r",
			  "&CMain", clan->leader, clan->number1, clan->number2,
			  clan->spacecraft, clan->vehicles, clan->members,
			  clan->funds);
		if (clan->first_subclan) {
			CLAN_DATA *subclan;
...
void do_enlist(CHAR_DATA * ch, char *argument)
{
	...
	if (bounty != NULL) {
		ch_printf(ch, "They don't allow dangerous criminals in %s!",
			  clan->name);
		return;
	}
...
void do_resign(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(ch->name, ch->pcdata->clan->immortal)) {
		ch_printf(ch,
			  "You can't resign from %s ... you are the immortal!\n\r",
			  clan->name);
		return;
	}
...
/*    sprintf(buf, "%s has quit %s!", ch->name, clan->name);
	...
	if (xplose > 0) {
		ch_printf(ch, "You lose %ld engineering experience.\n\r",
			  xplose);
	}
...
void do_clan_withdraw(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch, "You withdraw %ld wulongs from %s's funds.\n\r",
		  amount, clan->name);
...
void do_clan_donate(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch, "You donate %ld wulongs to %s's funds.\n\r", amount,
		  clan->name);
...
void do_capture(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->pcdata && ch->pcdata->clan
	    && (!str_cmp(ch->pcdata->clan->name, "GLM"))) {
		send_to_char
		    ("GLM members can't control planets! Stick to being a merchant!\n\r",
		     ch);
		return;
	}
	...
	if (planet->frustrated_by == ch->pcdata->clan
	    && planet->frust_level > 50) {
		send_to_char
		    ("The population is too frustrated with your organization!\n\r",
		     ch);
		return;
	}
	...
	for (planet = first_planet; planet; planet = planet->next) {
		...
		for (area = planet->first_area; area;
			...
			for (turn = 1; turn <= spin; turn++) {
				...
				if (!str_cmp(area->name, "Dayton City")
				    && value > 50)
				...
				if (IS_NPC(victim)
				    && IS_SET(victim->act, ACT_PROTOTYPE)) {
					victim->pIndexData->level = value;
					victim->pIndexData->ac =
					    100 - value * 2.5;
					victim->pIndexData->hitroll =
					    victim->hitroll;
					victim->pIndexData->damroll =
					    victim->damroll;
				}
				REMOVE_BIT(victim->act, ACT_PROTOTYPE);
				...
				if (number_range(0, 4) > 2 && value > 30) {
					...
					if (number_range(1, 10) == 10
					    && area->rareeq != 0) {
						ovnum = area->rareeq;
					} else if (area->loweq != 0
						   && area->higheq != 0)
						ovnum =
						    number_range(area->loweq,
								 area->higheq);
				}
				if (ovnum != 0
				    && (pObjIndex =
					get_obj_index(ovnum)) == NULL) {
...
void generate_police()
{
	...
	PLANET_DATA *planet;
	sh_int planetcount = 0, value, stat, name;
	CHAR_DATA *victim;
...
void add_point(CLAN_DATA * victor, CLAN_DATA * loser)
{
	...
	if (loser->war1 == victor || loser->war2 == victor) {
	    ...
	    if ((loser->war1 && loser->war1->ally1
		 && loser->war1->ally1 == victor)
		|| (loser->war1 && loser->war1->ally2
		    && loser->war1->ally2 == victor) || (loser->war2
							 && loser->war2->ally1
							 && loser->war2->
							 ally1 == victor)
		|| (loser->war2 && loser->war2->ally2
		    && loser->war2->ally2 == victor)) {
...
void check_winners()
{
	...
	for (clan = first_clan; clan; clan = clan->next) {
		...
		if (clan->war1counter >= 20 && clan->war1) {
			...
			sprintf(log_buf,
				"The Clan %s has defeated the Clan %s!",
				clan->name, clan->war1->name);
			clan->warwoncount++;
		...
		if (clan->war2counter >= 20 && clan->war2) {
			// Announce the winner!
			sprintf(buf,
				"The war between %s and %s is over! %s is the winner!",
...
void do_addradio(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->pcdata->clan->leader != ch->name
	    && ch->pcdata->clan->immortal != ch->name) {
		send_to_char("Huh?\r\n", ch);
		return;
	}
	...
	if (!str_cmp(arg1, "add")) {
		if (victim->pcdata->clantalk1 && victim->pcdata->clantalk2) {
			send_to_char
			    ("&RThey can have no more added clan radio frequences!\r\n",
			     ch);
			return;
		}
		...
		ch_printf(ch, "You allow %s to use your clan radio!\r\n",
			  victim->name);
		ch_printf(victim,
			  "You are now allowed to use %s clan radio!\r\n",
			  ch->pcdata->clan->name);
		save_char_obj(victim);
		return;
	}
	if (!str_cmp(arg1, "remove")) {
		if (!victim->pcdata->clantalk1 && !victim->pcdata->clantalk2) {
			send_to_char
			    ("&RThey have no clan radio frequencys!\r\n", ch);
			return;
		}
		...
		if ((victim->pcdata->clantalk1
			...
			&& victim->pcdata->clantalk2 != ch->pcdata->clan)) {
			send_to_char
			    ("They don't have your clantalk radio frequency!\r\n",
			     ch);
			return;
		}
		...
		ch_printf(ch, "You can no longer use %s clan radio!\r\n",
			  ch->pcdata->clan->name);
		ch_printf(ch,
			  "You remove %s's ability to use your clan radio!\r\n",
			  victim->name);
		return;
	}
}
...
void do_setclantalk(CHAR_DATA * ch, char *argument)
{
	...
	if (!ch->pcdata->clan && !ch->pcdata->clantalk1
	    && !ch->pcdata->clantalk2) {
		send_to_char("Huh?\r\n", ch);
		return;
	}
	...
	if (arg[0] == '\0') {
		send_to_char
		    ("&RThe current clan's radios you have access to: \r\n",
		     ch);
	...
	if (!clan || clan->clan_type == 1) {
		send_to_char
		    ("You can't set -that- clan for a clan frequency. \r\n",
		     ch);
		return;
	}
	...
	if (!check) {
		send_to_char
		    ("You are not allowed to use that clan frequency. \r\n",
		     ch);
		return;
	}
