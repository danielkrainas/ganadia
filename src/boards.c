	...
	if ((fp = fopen(filename, "w")) == NULL) {
		...
		for (pnote = board->first_note; pnote; pnote = pnote->next) {
			fprintf(fp,
				...
				pnote->subject, pnote->voting, pnote->yesvotes,
...
void do_note(CHAR_DATA * ch, char *arg_passed, bool IS_MAIL)
{
	...
	if (!str_cmp(arg, "list")) {
		...
		if (!IS_MAIL) {
			...
			for (pnote = board->first_note; pnote;
			     pnote = pnote->next) {
				vnum++;
				...
				if (is_note_to(ch, pnote)
					...
					if (vnum == anum || fAll) {
						...
						if (ch->gold < 10
							...
							send_to_char
							    ("It costs 10 wulongs to read a message.\n\r",
							     ch);
							return;
						}
						if (get_trust(ch) <
	...
	if (!str_cmp(arg, "write")) {
		...
		if (get_trust(ch) < sysdata.write_mail_free) {
			...
			if (!quill) {
				send_to_char
				    ("You need a datapad to record a message.\n\r",
				     ch);
				return;
			}
			if (quill->value[0] < 1) {
		...
		if ((paper = get_eq_char(ch, WEAR_HOLD)) == NULL
			...
			act(AT_MAGIC,
			    "$n grabs a message tisk to record a note.", ch,
			    NULL, NULL, TO_ROOM);
			act(AT_MAGIC,
	...
	if (!str_cmp(arg, "subject")) {
		if (get_trust(ch) < sysdata.write_mail_free) {
			...
			if (!quill) {
				send_to_char
				    ("You need a datapad to record a disk.\n\r",
				     ch);
				return;
			}
			if (quill->value[0] < 1) {
		...
		if ((paper = get_eq_char(ch, WEAR_HOLD)) == NULL
			...
			paper = obj_to_char(paper, ch);
			equip_char(ch, paper, WEAR_HOLD);
			act(AT_MAGIC, "$n grabs a message disk.", ch, NULL,
			    NULL, TO_ROOM);
			act(AT_MAGIC,
			    "You get a message disk to record your note.", ch,
			    NULL, NULL, TO_CHAR);
	...
	if (!str_cmp(arg, "to")) {
		...
		if ((paper = get_eq_char(ch, WEAR_HOLD)) == NULL
			...
			act(AT_MAGIC,
			    ...
			    NULL, NULL, TO_ROOM);
			act(AT_MAGIC,
			    "You grab a message disk to record your note.", ch,
			    NULL, NULL, TO_CHAR);
		}
	...
	if (!str_cmp(arg, "post")) {
		...
		act(AT_ACTION, "$n uploads a message.", ch, NULL, NULL,
		    TO_ROOM);
			...
			if ((is_note_to(ch, pnote)
				...
				if (take != 0) {
					if (ch->gold < 50
						...
						if (take == 1)
							send_to_char
							    ("It costs 50 wulongs to take your mail.\n\r",
							     ch);
						else
							send_to_char
							    ("It costs 50 wulongs to copy your mail.\n\r",
							     ch);
						return;
					}
					if (get_trust(ch) <
				...
				if (take == 1) {
					...
					act(AT_ACTION, "$n removes a message.",
					    ch, NULL, NULL, TO_ROOM);
				return;
			}
		}
	...
	while ((board = read_board(boardfile, board_fp)) != NULL) {
		...
		if ((note_fp = fopen(notefile, "r")) != NULL) {
			while ((pnote = read_note(notefile, note_fp)) != NULL) {
				LINK(pnote, board->first_note, board->last_note,
				     next, prev);
				board->num_posts++;
			}
		}
...
void do_bstat(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char
	    ("&P+&B--------------&P+&B--------&P+&B------&P+&B------&P+&B------&P+&B------&P+&B-------&P+&B------&P+\n\r"
	     ...
	     ch);
	ch_printf(ch,
		  "&P|&W %-12s &P|&W %6d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %5d &P|&W %5d |\n\r",
...
void do_boards(CHAR_DATA * ch, char *argument)
{
	...
	for (board = first_board; board; board = board->next)
		ch_printf(ch,
			  "&P|&W %-12s &P|&W %6d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %4d &P|&W %5d &P|&W %5d&P|\n\r",
