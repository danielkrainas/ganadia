...
void update_bus()
{
	...
	case 5:
		sprintf(buf, "It continues, 'Next stop, %s'",
			bus_stop[bus_planet + 1]);
		echo_to_ship(AT_CYAN, ship,
			     "An electronic voice says, 'Preparing for launch.'");
		echo_to_ship(AT_CYAN, ship, buf);
		sprintf(buf, "It continues, 'Next stop, %s'",
			bus_stop[bus2_planet + 1]);
		echo_to_ship(AT_CYAN, ship2,
			     "An electronic voice says, 'Preparing for launch.'");
		echo_to_ship(AT_CYAN, ship2, buf);
		bus_pos++;
...
/* Ship movement */
	for (ship = first_ship; ship; ship = ship->next) {
		...
		if (!ship->starsystem)
			continue;
		if (ship->currspeed > 0) {
			if (ship->vx == ship->hx && ship->vy == ship->hy
			    && ship->vz == ship->hz) {
				ship->currspeed = 0;
				echo_to_cockpit(AT_GREEN, ship,
						"The ship stops as it reaches its destination.");
				continue;
			}

			change =
			    sqrt(ship->hx * ship->hx + ship->hy * ship->hy +
				 ship->hz * ship->hz);
		...
		if (ship->currspeed > 0) {
			...
			if (ship->starsystem->planet2
				...
				echo_to_cockpit(AT_YELLOW, ship, buf);
				sprintf(buf, "%s begins orbiting %s.",
					ship->name, ship->starsystem->planet2);
				echo_to_system(AT_ORANGE, ship, buf, NULL);
				ship->currspeed = 0;
				continue;
			}
...
void update_space()
{
	...
	for (ship = first_ship; ship; ship = ship->next) {
			...
			if (ship->hyperdistance <= 0) {
				...
				if (ship->starsystem == NULL) {
					...
					echo_to_ship(AT_YELLOW, ship,
						     "The ship lurches slightly as it comes out of hyperspace.");
					echo_to_ship(AT_YELLOW, ship,
						     "The ship's speed slows down to handle re-entry.");
					ship->currspeed = 0;
					sprintf(buf,
						"%s enters the starsystem at %.0f %.0f %.0f",
						ship->name, ship->vx, ship->vy,
						ship->vz);
					echo_to_system(AT_YELLOW, ship, buf,
						       NULL);
					ship->shipstate = SHIP_READY;
					ship->currjump = NULL;
					STRFREE(ship->home);
					ship->home =
					    STRALLOC(ship->starsystem->name);
		...
		if (ship->starsystem && ship->currspeed > 0) {
			...
			echo_to_room_dnr(AT_BLUE,
					 get_room_index(ship->pilotseat),
					 "Speed: ");
			echo_to_room_dnr(AT_LBLUE,
					 get_room_index(ship->pilotseat), buf);
			sprintf(buf, "%.0f %.0f %.0f", ship->vx, ship->vy,
				ship->vz);
			echo_to_room_dnr(AT_BLUE,
					 get_room_index(ship->pilotseat),
					 "  Coords: ");
			echo_to_room(AT_LBLUE, get_room_index(ship->pilotseat),
				     buf);
			if (ship->pilotseat != ship->coseat) {
				...
				echo_to_room_dnr(AT_LBLUE,
						 get_room_index(ship->coseat),
						 buf);
				sprintf(buf, "%.0f %.0f %.0f", ship->vx,
					ship->vy, ship->vz);
				echo_to_room_dnr(AT_BLUE,
						 get_room_index(ship->coseat),
						 "  Coords: ");
				echo_to_room(AT_LBLUE,
					     get_room_index(ship->coseat), buf);
			}
		}
		...
		if (ship->starsystem) {
			...
			for (target = ship->starsystem->first_ship; target;
				...
				if (target != ship
				    && abs(ship->vx - target->vx) <
				    target_too_close
				    && abs(ship->vy - target->vy) <
				    target_too_close
				    && abs(ship->vz - target->vz) <
				    target_too_close) {
					sprintf(buf,
			...
			if (ship->starsystem->planet1
			    ...
			    && abs(ship->vz - ship->starsystem->p1z) <
			    too_close) {
				sprintf(buf, "Proximity alert: %s  %d %d %d",
					ship->starsystem->planet1,
					ship->starsystem->p1x,
					ship->starsystem->p1y,
					ship->starsystem->p1z);
			...
			if (ship->starsystem->planet2
			    ...
			    && abs(ship->vz - ship->starsystem->p2z) <
			    too_close) {
				sprintf(buf, "Proximity alert: %s  %d %d %d",
					ship->starsystem->planet2,
					ship->starsystem->p2x,
			...
			if (ship->starsystem->planet3
			    ...
			    && abs(ship->vz - ship->starsystem->p3z) <
			    too_close) {
				sprintf(buf, "Proximity alert: %s  %d %d %d",
		...
		if (ship->target0) {
			sprintf(buf, "%s   %.0f %.0f %.0f", ship->target0->name,
				...
				ship->target0->vz);
			echo_to_room_dnr(AT_BLUE, get_room_index(ship->gunseat),
					 "Target: ");
			echo_to_room(AT_LBLUE, get_room_index(ship->gunseat),
				     buf);
		...
		if (ship->target1) {
			sprintf(buf, "%s   %.0f %.0f %.0f", ship->target1->name,
				ship->target1->vx, ship->target1->vy,
				ship->target1->vz);
			echo_to_room_dnr(AT_BLUE, get_room_index(ship->turret1),
					 "Target: ");
			echo_to_room(AT_LBLUE, get_room_index(ship->turret1),
				     buf);
		...
		if (ship->target2) {
			sprintf(buf, "%s   %.0f %.0f %.0f", ship->target2->name,
				ship->target2->vx, ship->target2->vy,
				ship->target2->vz);
			echo_to_room_dnr(AT_BLUE, get_room_index(ship->turret2),
					 "Target: ");
			echo_to_room(AT_LBLUE, get_room_index(ship->turret2),
				     buf);
...
SPACE_DATA *starsystem_from_vnum(int vnum)
{
	...
	for (starsystem = first_starsystem; starsystem;
	     starsystem = starsystem->next)
		if (vnum == starsystem->doc1a || vnum == starsystem->doc2a
		    || vnum == starsystem->doc3a || vnum == starsystem->doc1b
		    || vnum == starsystem->doc2b || vnum == starsystem->doc3b
		    || vnum == starsystem->doc1c || vnum == starsystem->doc2c
...
void save_starsystem(SPACE_DATA * starsystem)
{
	...
	if (!starsystem->filename || starsystem->filename[0] == '\0') {
		sprintf(buf, "save_starsystem: %s has no filename",
			starsystem->name);
		bug(buf, 0);
		return;
	}
...
void fread_starsystem(SPACE_DATA * starsystem, FILE * fp)
{
	...
	for (;;) {
		...
		case 'G':
			...
			KEY("Gravityp1", starsystem->gravityp1,
			    fread_number(fp));
			KEY("Gravityp2", starsystem->gravityp2,
			    fread_number(fp));
			KEY("Gravityp3", starsystem->gravityp3,
			    fread_number(fp));
		...
		case 'L':
			KEY("Location1a", starsystem->location1a,
			    fread_string(fp));
			KEY("Location2a", starsystem->location2a,
			    fread_string(fp));
			KEY("Location3a", starsystem->location3a,
			    fread_string(fp));
			KEY("Location1b", starsystem->location1b,
			    fread_string(fp));
			KEY("Location2b", starsystem->location2b,
			    fread_string(fp));
			KEY("Location3b", starsystem->location3b,
			    fread_string(fp));
			KEY("Location1c", starsystem->location1c,
			    fread_string(fp));
			KEY("Location2c", starsystem->location2c,
			    fread_string(fp));
			KEY("Location3c", starsystem->location3c,
			    fread_string(fp));
			break;
...
void load_space()
{
	...
	for (;;) {
		...
		if (!load_starsystem(filename)) {
			sprintf(buf, "Cannot load starsystem file: %s",
				filename);
			bug(buf, 0);
		}
	}
...
void do_paint(CHAR_DATA * ch, char *argument)
{
	...
	if (arg2[0] == '\0' && (str_cmp(arg2, "Blood")
		...
		send_to_char
		    ("&B<&RBlood&B> <&RGreen&B> <&RDBlue&B> <&RCyan&B> <&RLRed&B> \n\r&w",
		     ch);
		send_to_char
		    ("&B<&RLGreen&B> <&RLBlue&B> <&RBlue&B> <&RBrown&B> <&RPurple&B> \n\r&w",
		     ch);
	...
	if (ship->shipstate != SHIP_DOCKED) {
		send_to_char
		    ("&RYour name can only be changed when your docked!\n\r",
		     ch);
		return;
	}

	if (!ship->owner || !ship->pilot || !ship->copilot
	    || check_pilot(ch, ship)) {
		...
		sprintf(buf,
			"Sorry, %s, You are not able to paint that ship, try it when your a pilot.",
			ch->name);
		send_to_char(buf, ch);
		return;
	}
...
void do_override(CHAR_DATA * ch, char *argument)
{
	...
	if (eShip == ship) {
		send_to_char
		    ("&RYou are in the cockpit, just hit the controls!\n\r",
		     ch);
		return;
	}
	...
	if (!strcmp(arg2, "stop")) {
		...
		echo_to_cockpit(AT_GREY, eShip,
				"Braking thrusters fire and the ship stops");
...
void do_renameship(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->gold < 50000) {
		send_to_char
		    ("&RYou do not have enough dollars for this request.\n\r",
		     ch);
		return;
	}
...
void showstarsystem(CHAR_DATA * ch, SPACE_DATA * starsystem)
{
	...
	ch_printf(ch, "Planet1: %s   Gravity: %d   Coordinates: %d %d %d\n\r",
		  starsystem->planet1, starsystem->gravityp1, starsystem->p1x,
		  starsystem->p1y, starsystem->p1z);
	...
	ch_printf(ch, "Gate1: %s   Coordinates: %d %d %d\n\r",
		  starsystem->gate1, starsystem->g1x, starsystem->g1y,
		  starsystem->g1z);
	ch_printf(ch, "Gate2: %s   Coordinates: %d %d %d\n\r",
		  starsystem->gate2, starsystem->g2x, starsystem->g2y,
		  starsystem->g2z);
	ch_printf(ch, "Gate3: %s   Coordinates: %d %d %d\n\r",
		  starsystem->gate3, starsystem->g3x, starsystem->g3y,
		  starsystem->g3z);
	ch_printf(ch, "Gate4: %s   Coordinates: %d %d %d\n\r",
		  starsystem->gate4, starsystem->g4x, starsystem->g4y,
		  starsystem->g4z);
	ch_printf(ch, "Gate5: %s   Coordinates: %d %d %d\n\r",
		  starsystem->gate5, starsystem->g5x, starsystem->g5y,
		  starsystem->g5z);
	ch_printf(ch, "Gate6: %s   Coordinates: %d %d %d\n\r",
		  starsystem->gate6, starsystem->g6x, starsystem->g6y,
		  starsystem->g6z);
	return;
}
...
void
echo_to_system(int color, SHIP_DATA * ship, char *argument, SHIP_DATA * ignore)
{
	SHIP_DATA *target;
...
bool is_facing(SHIP_DATA * ship, SHIP_DATA * target)
{
	...
	cosofa =
	    (hx * dx + hy * dy + hz * dz) / (sqrt(hx * hx + hy * hy + hz * hz) +
...
void fread_ship_weapon(SHIP_WEAPON_DATA * weapon, FILE * fp)
{
	...
	for (;;) {
		...
		case 'A':
			...
			KEY("act_target", weapon->act_target,
			     fread_string(fp));
		case 'B':
...
void fread_ship(SHIP_DATA * ship, FILE * fp) 
{
	...
	ship->last_weapon = NULL;
			...
			KEY("Filename", ship->filename,
			     fread_string_nohash(fp));
			KEY("Firstroom", ship->firstroom, fread_number(fp));
			KEY("Firstweapon", ship->firstweapon,
			     fread_number(fp));
			...
			KEY("Secondweapon", ship->secondweapon,
			     fread_number(fp));
			...
			KEY("Thirdweapon", ship->thirdweapon,
			     fread_number(fp));
...
//    SHIP_OBJ_DATA obj;    
	...
	for (;;) {
		...
		if (!str_cmp(word, "#weapon")) {
			...
			LINK(weapon, first_ship_weapon, last_ship_weapon, next,
			     prev);
		}
	}
}
	...
	if ((fp = fopen(filename, "r")) != NULL) {
		...
		for (;;) {
			...
			if (!str_cmp(word, "SHIP")) {
				...
				sprintf(buf, "Load_ship_file: bad section: %s.",
					word);
				bug(buf, 0);
				break;
			}
	...
	if (!(found))
		...
		if (!str_cmp("Public", ship->owner) || ship->type == MOB_SHIP) {
			...
			LINK(ship, pRoomIndex->first_ship,
			     pRoomIndex->last_ship, next_in_room, prev_in_room);
...
void do_setship(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg2, "turret1")) {
		...
		if (tempnum == ship->cockpit || tempnum == ship->entrance ||
		    tempnum == ship->turret2 || tempnum == ship->hangar
	...
	if (!str_cmp(arg2, "turret2")) {
		...
		if (tempnum == ship->cockpit || tempnum == ship->entrance ||
		    tempnum == ship->turret1 || tempnum == ship->hangar
		    || tempnum == ship->engineroom) {
			send_to_char
			    ("That room is already being used by another part of the ship\n\r",
			     ch);
	...
	if (!str_cmp(arg2, "hangar")) {
		...
		if (tempnum == ship->cockpit || tempnum == ship->turret1
		    || tempnum == ship->turret2 || tempnum == ship->engineroom)
		{
			send_to_char
			    ("That room is already being used by another part of the ship\n\r",
			     ch);
		...
		if (ship->class == FIGHTER_SHIP) {
			send_to_char
			    ("Monopods are too small to have hangars for other ships!\n\r",
			     ch);
			return;
		}
		ship->hangar = tempnum;
	...
	if (!str_cmp(arg2, "type")) {
		if (!str_cmp(argument, "republic"))
			...
			send_to_char
			    ("Ship type must be either: republic, imperial, civilian or mob.\n\r",
			     ch);
			return;
		}
		send_to_char("Done.\n\r", ch);
...
*/
	...
	if (!str_cmp(arg2, "payload")) {
		...
		if (ship->hull_type > (MAX_HULL_TYPES - 1)
		     || ship->hull_type < 0)
			 {
			send_to_char
			    ("Invalid Type, 'help hulltypes' for a list", ch);
...
void equip_ship_weapon(SHIP_WEAPON_DATA * proto, SHIP_DATA * ship)
{
	...
	LINK(weapon, ship->first_weapon, ship->last_weapon, next_in_ship,
	     prev_in_ship);
...
void do_remweapon(CHAR_DATA * ch, char *arg)
{
	...
	UNLINK(weapon, ship->first_weapon, ship->last_weapon, next_in_ship,
	       prev_in_ship);
...
void do_addweapon(CHAR_DATA * ch, char *arg)
{
	...
	LINK(dup, ship->first_weapon, ship->last_weapon, next_in_ship,
	     prev_in_ship);
...
/*	if(!str_cmp(field,"action"))
	...
	if (!str_cmp(field, "burst")) {
		if (atoi(arg) < 1) {
			send_to_char
			    ("Weapon must have a burst of atleast 1\n\r", ch);
			return;
		}
	...
	if (!str_cmp(field, "maxdamage")) {
		if (atoi(arg) < 0) {
			send_to_char
			    ("Weapon must have a maxdamage of atleast 1\n\r",
			     ch);
			return;
		}
	...
	if (!str_cmp(field, "mindamage")) {
		if (atoi(arg) < 0) {
			send_to_char
			    ("Weapon must have a mindamage of atleast 1\n\r",
			     ch);
			return;
		}
...
void do_showship(CHAR_DATA * ch, char *argument) 
{
	...
	char let = 'A';
		   ...
		   SHIP_REPUBLIC ? "New Republic" : (ship->type ==
						      SHIP_IMPERIAL ? "Imperial"
						      : (ship->type ==
							  SHIP_CIVILIAN ?
							  "Civilian" : "Mob")),
		   ship->class ==
		   FIGHTER_SHIP ? "Starfighter" : (ship->class ==
						    MIDSIZE_SHIP ? "Midship" :
		   ...
		   ship->home, ship->description, ship->owner, ship->pilot,
		   ship->copilot);
	ch_printf(ch, "Firstroom: %d   Lastroom: %d", ship->firstroom,
		   ship->lastroom);
	ch_printf(ch,
		   "Cockpit: %d   Entrance: %d   Hangar: %d  Engineroom: %d\n\r",
...
void do_speeders(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch)) {
		...
		if (!count) {
			send_to_char
			    ("There are no land or air vehicles owned by you.\n\r",
			     ch);
		}
...
void do_allships(CHAR_DATA * ch, char *argument)
{
	...
	for (ship = first_ship; ship; ship = ship->next) {
		...
		ch_printf(ch, "%-35s %-15s ", strip_color(ship->name),
			  ship->owner);
		if (ship->type == MOB_SHIP || ship->class == SHIP_PLATFORM) {
			ch_printf(ch, "\n\r");
			continue;
...
void
new_missile(SHIP_DATA * ship, SHIP_DATA * target, CHAR_DATA * ch,
	    int missiletype)
{
	SPACE_DATA *starsystem;
	MISSILE_DATA *missile;
...
void ship_from_starsystem(SHIP_DATA * ship, SPACE_DATA * starsystem)
{
	...
	if (ship->prev_in_starsystem)
		ship->prev_in_starsystem->next_in_starsystem =
		    ship->next_in_starsystem;

	if (ship->next_in_starsystem)
		ship->next_in_starsystem->prev_in_starsystem =
		    ship->prev_in_starsystem;
...
void damage_ship_ch(SHIP_DATA * ship, int min, int max, CHAR_DATA * ch)
{
	...
	if (damage > 0) {
		...
		if (number_range(1, 100) <= 2 && ship->statet1 != LASER_DAMAGED
		    && ship->turret1) {
		...
		if (number_range(1, 100) <= 2 && ship->statet2 != LASER_DAMAGED
		    && ship->turret2) {
			echo_to_room(AT_BLOOD + AT_BLINK,
...
void destroy_ship(SHIP_DATA * ship, CHAR_DATA * ch)
{
	...
	if (ship->class == FIGHTER_SHIP)
		...
		echo_to_ship(AT_WHITE + AT_BLINK, ship,
		...
		if (room != NULL) {
			...
			while (rch) {
				if (IS_IMMORTAL(rch)) {
					...
					if (ch) {
						sprintf(log_buf,
							"%s killed by %s at %d (In the Ship %s)",
							rch->name,
							(IS_NPC(ch) ? ch->
							 short_descr : ch->
							 name),
...
void do_board(CHAR_DATA * ch, char *argument)
{
	...
	if ((toroom = get_room_index(ship->entrance)) != NULL) {
		...
		act(AT_PLAIN, "$n enters the ship.", ch, NULL, argument,
		    TO_ROOM);
		do_look(ch, "auto");
...
bool rent_ship(CHAR_DATA * ch, SHIP_DATA * ship)
{
	...
	if (ch->gold < price) {
		ch_printf(ch,
			  "&RRenting this ship costs %ld. You don't have enough dollars!\n\r",
			  price);
		return FALSE;
	}
...
void do_leaveship(CHAR_DATA * ch, char *argument)
{
	...
	if ((toroom = get_room_index(ship->location)) != NULL) {
		act(AT_PLAIN, "$n exits the ship.", ch, NULL, argument,
		    TO_ROOM);
		act(AT_PLAIN, "You exit the ship.", ch, NULL, argument,
		    TO_CHAR);
		char_from_room(ch);
		char_to_room(ch, toroom);
		act(AT_PLAIN, "$n steps out of a ship.", ch, NULL, argument,
		    TO_ROOM);
		do_look(ch, "auto");
	} else
		send_to_char
...
void do_launch(CHAR_DATA * ch, char *argument)
{
	...
	if (number_percent() < chance) {
		...
		if (!is_rental(ch, ship)) {
			...
			if (ship->missiles)
				price +=
				    (50 * (ship->maxmissiles - ship->missiles));
			else if (ship->torpedos)
				price +=
				    (75 * (ship->maxtorpedos - ship->torpedos));
			else if (ship->rockets)
				price +=
				    (150 * (ship->maxrockets - ship->rockets));
		...
		if (ch->pcdata && ch->pcdata->clan
			...
			ch_printf(ch,
				  "&GIt costs %s %ld dollars to ready this ship for launch.\n\r",
			...
			ch->gold -= price;
			ch_printf(ch,
				  "&GYou pay %ld dollars to ready the ship for launch.\n\r",
				  price);
...
void do_land(CHAR_DATA * ch, char *argument)
{
	...
	if (str_prefix(argument, ship->starsystem->location1a) &&
		...
		if (ship->class == CAPITAL_SHIP
		    && target->class != SHIP_PLATFORM) {
			send_to_char
			    ("&RCapital ships can only dock on platforms.\n\r",
			     ch);
			return;
		}
		if (!target->bayopen) {
			send_to_char
			    ("&RTheir hangar is closed. You'll have to ask them to open it for you\n\r",
			     ch);
			return;
		}
		if ((target->vx > ship->vx + 200)
		    ...
		    || (target->vz < ship->vz - 200)) {
			send_to_char
			    ("&RThat ship is too far away! You'll have to fly a little closer.\n\r",
			     ch);
			return;
		}
	} else {
		...
		if ((vx > ship->vx + 200) || (vx < ship->vx - 200) ||
			...
			send_to_char
			    ("&R That platform is too far away! You'll have to fly a little closer.\n\r",
			     ch);
			return;
		}
	}
	...
	if (number_percent() < chance) {
		...
		act(AT_PLAIN, "$n begins the landing sequence.", ch, NULL,
		    argument, TO_ROOM);
		echo_to_ship(AT_YELLOW, ship,
		...
		if (starsystem_from_vnum(ship->lastdoc) != ship->starsystem) {
			if (ch->skill_level[PILOTING_ABILITY] < 10) {
				...
				send_to_char
				    ("Your level is too high to gain experience for using this skill.\n\r",
				     ch);
			}
		}
		return;
...
void landship(SHIP_DATA * ship, char *arg)
{
	...
	if (!str_cmp("Public", ship->owner)) {
		...
		echo_to_cockpit(AT_YELLOW, ship,
				"Repairing, refueling, and reloading ship...");
	}
...
void do_trajectory(CHAR_DATA * ch, char *argument)
{
	...
	if (vx == ship->vx && vy == ship->vy && vz == ship->vz) {
		ch_printf(ch, "The ship is already at %.0f %.0f %.0f !", vx, vy,
			  vz);
	}
	...
	if (ship->class == FIGHTER_SHIP
	    || (ship->class == MIDSIZE_SHIP && ship->manuever > 50))
		ship->shipstate = SHIP_BUSY_3;
	else if (ship->class == MIDSIZE_SHIP
		 || (ship->class == CAPITAL_SHIP && ship->manuever > 50))
		ship->shipstate = SHIP_BUSY_2;
	else
		ship->shipstate = SHIP_BUSY;
...
void do_buyship(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->gold < price) {
		ch_printf(ch,
			  "&RThis ship costs %ld. You don't have enough dollars!\n\r",
			  price);
		return;
	}
	...
	act(AT_PLAIN,
	    "$n walks over to a terminal and makes a money transaction.", ch,
	    NULL, argument, TO_ROOM);
...
void do_clanbuyship(CHAR_DATA * ch, char *argument)
{
	...
	clan = ch->pcdata->clan;
	mainclan =
	    ch->pcdata->clan->mainclan ? ch->pcdata->clan->mainclan : clan;
...
void do_info(CHAR_DATA * ch, char *argument)
{
		   ...
		   (target->class == CAPITAL_SHIP ? "Capital Ship" :
			...
			(ship->class == WHEELED ? "Wheeled Transport" :
			 (ship->class == LAND_CRAWLER ? "Crawler" :
			  (ship->class ==
			   WALKER ? "Walker" : "Unknown"))))))))), target->name,
		  target->filename);
	ch_printf(ch,
		  "&CDescription&R: &W%s\n\r&COwner&R: &W%s   &CPilot&R: &W%s   &CCopilot&R: &W%s\n\r",
		  target->description, target->owner, target->pilot,
		  target->copilot);
	...
	ch_printf(ch,
		  "&CMaximum Speed&R: &B(&W%d&B)   &CManuever&R: &B(&W%d&B)\n\r",
		  target->realspeed, target->manuever);
	ch_printf(ch,
		  "&CComm Range&R: &B(&W%d&B)   &CMax Chaff&R: &B(&W%d&B)&W\n\r",
		  target->comm, target->maxchaff);
	ch_printf(ch,
		   "&CMax Payload&R: &B(&W%d&B)   &CMax Slots&R: &B(&W%d&B)&W\n\r",
	...
	if (target->first_weapon) {
		...
		for (weapon; weapon; weapon = weapon->next_in_ship) {
			if (!weapon)
				break;
			ch_printf(ch,
				  "&z&r[&W%c&r]&g %-15s&b:&w%-25s &r(&C%d&cMA&r|&C%d&cMH&r)\n\r",
				  wep, weapon->name, weapon->desc,
				  weapon->maxammo, weapon->maxheat);
			wep++;
		}
	} else {
...
void do_autopilot(CHAR_DATA * ch, char *argument)
{
	...
	act(AT_PLAIN, "$n flips a switch on the control panel.", ch, NULL,
	    argument, TO_ROOM);
			...
			if (!ship->hatchopen) {
				...
				if (ship->location != ship->lastdoc ||
				    (ship->shipstate != SHIP_DOCKED
				     && ship->shipstate != SHIP_DISABLED)) {
					send_to_char
					    ("&RPlease wait until the ship lands!\n\r",
					     ch);
					return;
				}
				ship->hatchopen = TRUE;
				...
				act(AT_PLAIN, "$n opens the hatch.", ch, NULL,
				    argument, TO_ROOM);
				sprintf(buf, "The hatch on %s opens.",
					ship->name);
				echo_to_room(AT_YELLOW,
	...
	if (!argument || argument[0] == '\0' || !str_cmp(argument, "hatch")) {
		...
		if (ship == NULL) {
			...
			if (ship->hatchopen) {
				...
				act(AT_PLAIN, "$n closes the hatch.", ch, NULL,
				    argument, TO_ROOM);
				sprintf(buf, "The hatch on %s closes.",
					ship->name);
				echo_to_room(AT_YELLOW,
					     get_room_index(ship->location),
					     buf);
...
void do_status(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch, "&CFuel&c&R:&W %d&R/&W%d   &CPower&c&R:&W %d&R/&W%d\n\r",
		  target->energy,
		  target->maxenergy, target->power, target->maxpower);
	...
	for (weapon; weapon; weapon = weapon->next_in_ship) {
		if (!weapon)
			break;
		ch_printf(ch,
			  "&z&r[&W%c&r]&g %-15s &r(&C%d&R/&C%d&r)&cAmmo &r(&C%d&R/&C%d&r)&cHeat\n\r",
			  wep, weapon->name, weapon->ammo, weapon->maxammo,
			  weapon->heat, weapon->maxheat);
		wep++;
	}
...
void do_hyperspace(CHAR_DATA * ch, char *argument)
{
	...
	if (ship->shipstate != SHIP_READY) {
		send_to_char
		    ("&RPlease wait until the ship has finished its current manuever.\n\r",
		     ch);
		return;
	}
	...
	if ((vx > ship->vx + 200) || (vx < ship->vx - 200) ||
	    ...
	    (vz > ship->vz + 200) || (vz < ship->vz - 200)) {
		send_to_char
		    ("&RThe hyperspace gate is too far away! You'll have to fly a little closer.\n\r",
		     ch);
		return;
	}
	if (ch->pcdata) {
		if (ch->pcdata->bank >= fee || ch->pcdata->bank > 1000000) {
			sprintf(buf,
				"You are charged %d dollars for the use of the gate.",
				fee);
			echo_to_room(AT_GREEN, get_room_index(ship->pilotseat),
				     buf);
			ch->pcdata->bank -= fee;
		} else {
			sprintf(buf,
				"You don't have enough money in your account to pay the toll of %d.",
				fee);
	...
	act(AT_PLAIN, "$n steers the ship into the hyperspace gate\n\r.", ch,
	    NULL, argument, TO_ROOM);
	echo_to_ship(AT_YELLOW, ship,
		     "The ship lurches slightly as it enters a hyperspace gate.");
	echo_to_cockpit(AT_YELLOW, ship,
			"The stars disappear and you enter hyperspace.");
...
void do_target(CHAR_DATA * ch, char *argument)
{
	...
	if (target == NULL || target == ship) {
		send_to_char
		    ("&RThe ship has left the area. Targeting aborted.\n\r",
		     ch);
		return;
	}
...
void do_fire2(CHAR_DATA * ch, char *arg)
{
	...
	if (arg[0] == '\0') {
		send_to_char
		    ("&RYou must specify the weapon you want to fire!\n", ch);
		return;
	}
	...
	chance = IS_NPC(ch) ? ch->top_level 
	    : (int)(ch->perm_dex * 2 +
		    ch->pcdata->learned[gsn_spacecombat] /
	...
	while (i > 0) {
		...
		weapon->ammo--;
		if (weapon->ammo < 0) {
			sprintf(buf, "ALERT!!! %s is out of ammo",
				weapon->name);
			echo_to_cockpit(AT_BLOOD, ship, buf);
			break;
		}
		chance = URANGE(10, chance, 90);
			...
			sprintf(buf, "%s fire from %s at you but miss.",
				 weapon->name, ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf, "%s fire at %s but miss.", weapon->name,
				 target->name);
			...
			sprintf(buf, "%s from %s fires and barely misses %s.",
				 weapon->name, ship->name, target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			return;
			}
		sprintf(buf, "%s from %s fire and hits %s.", weapon->name,
			 ship->name, target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		sprintf(buf, "%s from %s fire and hit you!", weapon->name,
			 ship->name);
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "%s from your ship fire hits %s!.", weapon->name,
			 target->name);
			      ...
			      "A small explosion vibrates through the ship.");
		damage_ship_ch(target, weapon->mindamage, weapon->maxdamage,
				ch);
	}
...
void do_fire(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		act(AT_PLAIN, "$n presses the fire button.", ch,
		    NULL, argument, TO_ROOM);
		if (number_percent() > chance) {
			sprintf(buf, "Lasers fire from %s at you but miss.",
				ship->name);
			echo_to_cockpit(AT_ORANGE, target, buf);
			sprintf(buf, "The ships lasers fire at %s but miss.",
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
		...
		sprintf(buf, "Laserfire from %s hits %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "%s fires a missile towards %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		learn_from_success(ch, gsn_weaponsystems);
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "%s fires a torpedo towards %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		learn_from_success(ch, gsn_weaponsystems);
		if (ship->class == CAPITAL_SHIP || ship->class == SHIP_PLATFORM)
	...
	if (ch->in_room->vnum == ship->gunseat
		...
		echo_to_cockpit(AT_BLOOD, target, buf);
		sprintf(buf, "%s fires a heavy rocket towards %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
		learn_from_success(ch, gsn_weaponsystems);
		if (ship->class == CAPITAL_SHIP || ship->class == SHIP_PLATFORM)
	...
	if (ch->in_room->vnum == ship->turret1
		...
		if (number_percent() > chance) {
			...
			sprintf(buf,
				...
				target->name);
			echo_to_cockpit(AT_ORANGE, ship, buf);
			sprintf(buf, "%s fires at %s but misses.", ship->name,
				target->name);
			echo_to_system(AT_ORANGE, ship, buf, target);
			learn_from_failure(ch, gsn_spacecombat);
			learn_from_failure(ch, gsn_spacecombat2);
		...
		sprintf(buf, "Turboasers fire from %s, hitting %s.", ship->name,
			target->name);
		echo_to_system(AT_ORANGE, ship, buf, target);
...
void do_calculate(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		for (starsystem = first_starsystem; starsystem;
			...
			ch_printf(ch, "%-30s %d\n\r", starsystem->name,
				  (abs
				   (starsystem->xpos - ship->starsystem->xpos) +
				   abs(starsystem->ypos -
				       ship->starsystem->ypos)) / 2);
			count++;
		}
		if (!count) {
	...
	send_to_char
	    ("&GHyperspace course set. Ready for entry into a gate.\n\r", ch);
...
void do_repairship(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (ship->shipstate == SHIP_HYPERSPACE) {
			send_to_char
			    ("You can't repair your ship in hyperspace!\n\r",
			     ch);
			return;
		}
		...
		if (str_cmp(argument, "hull") && str_cmp(argument, "drive") &&
		    ...
		    && str_cmp(argument, "weapon")) {
			send_to_char
			    ("&RYou need to specify something to repair:\n\r",
			     ch);
			send_to_char
			    ("&rTry: hull, drive, launcher or weapon\n\r", ch);
			return;
		}
	...
	if (!str_cmp(arg, "hull")) {
		change =
		    URANGE(0,
			   number_range((int)(ship->maxhull * 0.1),
					(int)(ship->maxhull * .01)),
	...
	if (!str_cmp(arg, "weapon")) {
		...
		for (weapon; weapon; weapon = weapon->next_in_ship) {
			if (weapon->broken) {
				...
				weapon->heat = 0;
				ch_printf(ch,
					  "&G%s is restored to working condition.\n\r",
					  weapon->name);
				break;
			}
		}
...
bool ship_was_in_range(SHIP_DATA * ship, SHIP_DATA * target)
{
	if (target && ship && target != ship)
		if (abs(target->ox - ship->vx) <
		    100 * (ship->sensor +
			   10) * ((target->class ==
				   SHIP_READY ? 2 : target->class) + 3)
		    && abs(target->oy - ship->vy) <
		    100 * (ship->sensor +
			   10) * ((target->class ==
				   SHIP_READY ? 2 : target->class) + 3)
		    && abs(target->oz - ship->vz) <
		    100 * (ship->sensor +
			   10) * ((target->class ==
				   SHIP_READY ? 2 : target->class) + 3))
			return TRUE;
...
void do_jumpvector(CHAR_DATA * ch, char *argument)
{
	...
	if (target == ship) {
		send_to_char
		    ("You can figure out where you are going on your own.\n\r",
		     ch);
		return;
	}
	...
	if (target->shipstate == SHIP_HYPERSPACE) {
		...
		send_to_char
		    ("After some deliberation, you figure out its projected course.\n\r",
		     ch);
		sprintf(buf, "%s Heading: %.0f, %.0f, %.0f", target->name, tx,
			ty, tz);
		echo_to_cockpit(AT_BLOOD, ship, buf);
...
void do_openbay(CHAR_DATA * ch, char *argument)
{
	...
	if (ship_from_pilotseat(ch->in_room->vnum) == NULL
		...
		send_to_char
		    ("&RYou aren't in the pilots chair or hangar of a ship!\n\r",
		     ch);
		return;
	}
...
ch_ret drive_ship(CHAR_DATA * ch, SHIP_DATA * ship, EXIT_DATA * pexit, int fall)
{
...
#ifdef DEBUG
	if (pexit) {
		sprintf(buf, "drive_ship: %s to door %d", ch->name,
			pexit->vdir);
		log_string(buf);
	}
#endif
	...
	if (!pexit || (to_room = pexit->to_room) == NULL) {
		if (drunk)
			send_to_char
			    ("You drive into a wall in your drunken state.\n\r",
			     ch);
		else
	...
	if (IS_SET(pexit->exit_info, EX_CLOSED)
		...
		if (!IS_SET(pexit->exit_info, EX_SECRET)
			...
			if (drunk) {
				act(AT_PLAIN,
				    "$n drives into the $d in $s drunken state.",
				    ch, NULL, pexit->keyword, TO_ROOM);
				act(AT_PLAIN,
				    "You drive into the $d in your drunken state.",
...
*/
	...
	if (!fall) {
		...
		if (IS_SET(to_room->room_flags, ROOM_NO_DRIVING)) {
			send_to_char
			    ("You can't take a vehicle through there!\n\r", ch);
			return rNONE;
		}
		...
		if (IS_SET(pexit->exit_info, EX_CLIMB)) {
			...
			if (ship->class < CLOUD_CAR) {
				send_to_char
				    ("You need to fly or climb to get up there.\n\r",
				     ch);
				return rNONE;
			}
		}
...
void do_stealth(CHAR_DATA * ch, char *arg)
{
	...
	if (ship->stealth) {
		ship->stealth = FALSE;
		sprintf(buf,
			"&z&YA bright light flashes as %s appears out of no where",
			ship->name);
		echo_to_system(AT_ORANGE, ship, buf, ship);
		echo_to_cockpit(AT_WHITE, ship,
				"The ships hull glows bright as it becomes visible again.");
	} else {
		ship->stealth = TRUE;
		sprintf(buf,
			"&z&Y%s shimmers as it disappears from your scanners",
			ship->name);
		echo_to_system(AT_ORANGE, ship, buf, ship);
		echo_to_cockpit(AT_WHITE, ship,
				"The ships hull glistens as it becomes transparent.");
	}
}
