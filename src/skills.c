...
bool check_skill(CHAR_DATA * ch, char *command, char *argument)
{
	...
	if (!skill_table[sn]->skill_fun) {
		...
		retcode =
		    (*skill_table[sn]->spell_fun) (sn, ch->top_level, ch, vo);
		end_timer(&time_used);
...
void do_slookup(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "all")) {
		...
		for (sn = 0;
		     sn < top_herb && herb_table[sn] && herb_table[sn]->name;
		     sn++)
			pager_printf(ch, "%d) %s\n\r", sn,
				     herb_table[sn]->name);
	} else {
		...
		ch_printf(ch,
			  ...
			  skill->minimum_position, skill->min_mana,
			  skill->beats);
		ch_printf(ch, "Flags: %d  Guild: %d  Code: %s\n\r",
			  skill->flags, skill->guild,
		...
		for (aff = skill->affects; aff; aff = aff->next) {
			...
			if (aff->location) {
				strcat(buf, " modifies ");
				strcat(buf,
				       a_types[aff->location % REVERSE_APPLY]);
				strcat(buf, " by '");
		...
		if (skill->type != SKILL_HERB && skill->guild >= 0
		    && skill->guild < MAX_ABILITY) {
...
void do_sset(CHAR_DATA * ch, char *argument)
{
	...
	if (get_trust(ch) > LEVEL_GREATER
		...
		if (!str_cmp(arg2, "code")) {
			...
			} else if ((dofun = skill_function(argument)) !=
				   skill_notfound) {
...
void learn_from_success(CHAR_DATA * ch, int sn)
{
	...
	if (ch->pcdata->learned[sn] < 100) {
		chance =
		    ch->pcdata->learned[sn] + (5 * skill_table[sn]->difficulty);
	...
	if (IS_NPC(ch) || percent < ch->pcdata->learned[gsn_gouge]) {
		...
		if (global_retcode == rNONE) {
			if (!IS_AFFECTED(victim, AFF_BLIND)) {
				...
				af.modifier = -6;
				af.duration =
				    3 + (ch->skill_level[COMBAT_ABILITY] / 20);
				af.bitvector = AFF_BLIND;
				affect_to_char(victim, &af);
				act(AT_SKILL, "You can't see a thing!", victim,
				    NULL, NULL, TO_CHAR);
			}
...
void do_dig(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		add_timer(ch, TIMER_DO_FUN,
			  UMIN(skill_table[gsn_dig]->beats / 10, 3), do_dig, 1);
	...
	case 1:
		if (!ch->dest_buf) {
			...
			act(AT_PLAIN, "$n's digging was interrupted!", ch, NULL,
			    NULL, TO_ROOM);
	...
	if (arg[0] != '\0') {
		if ((pexit = find_door(ch, arg, TRUE)) != NULL
			...
			if ((number_percent() * (shovel ? 1 : 4)) <
				...
				send_to_char("You dig open a passageway!\n\r",
					     ch);
				act(AT_PLAIN, "$n digs open a passageway!", ch,
				    NULL, NULL, TO_ROOM);
		...
		act(AT_PLAIN, "$n's dig did not discover any exit...", ch, NULL,
		    NULL, TO_ROOM);
		return;
	}
...
void do_search(CHAR_DATA * ch, char *argument)
{
	...
	for (gch = ch->in_room->first_person; gch; gch = gch->next_in_room) {
		if (IS_AFFECTED(gch, AFF_HIDE) && !IS_IMMORTAL(gch)
		    && ch != gch) {
			peoplehide = TRUE;
			break;
		}
...
void do_steal(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_NPC(victim)
	    && (IS_SET(victim->act, ACT_PACIFIST)
		|| IS_SET(victim->act, ACT_NOSTEAL))) {
	...
	if (!IS_NPC(victim) && victim->pcdata->learned[gsn_charm]) {
		if (get_curr_cha(ch) < get_curr_cha(victim)) {
			if (number_range(0, get_curr_lck(victim) / 3) > 3) {
				act(AT_RED,
				    "You stop in your tracks because of $N's charm!",
				    ch, NULL, victim, TO_CHAR);
				act(AT_RED,
				    "$n stops in $s tracks and can not steal from you because of your charm!",
				    ch, NULL, victim, TO_VICT);
				act(AT_RED,
				    "$n stops and can not steal from $N because of $S charm!",
				    ch, NULL, victim, TO_NOTVICT);
				learn_from_success(ch, gsn_charm);
				return;
			}
		}
	}
	...
	if (!str_cmp(arg1, "wulongs")
	    || !str_cmp(arg1, "wulong") || !str_cmp(arg1, "money")) {
		int amount;

		amount = (int)(victim->gold * number_range(1, 5) / 50);
		if (amount <= 0) {
		...
		ch->gold += amount;
		victim->gold -= amount;
		ch_printf(ch, "Aha!  You got %d wulongs.\n\r", amount);
		...
		if (IS_NPC(victim)) ;
		{
			...
			ch_printf(ch,
				  "&WYou gain %ld smuggling experience!\n\r",
				  xp);
		}
		return;
	}
...
void do_backstab(CHAR_DATA * ch, char *argument)
{
	...
	if (get_eq_char(ch, WEAR_DUAL_WIELD) != NULL) {
		send_to_char
		    ("You can't do that while wielding two weapons.\n\r", ch);
		return;
	}
	...
	percent =
	    number_percent() - (get_curr_dex(ch) - 14) + (get_curr_dex(victim) -
							  13);
...
void do_kick(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->focus == "none" || ch->focus == NULL) {
		send_to_char
		    ("You must focus on an opponent before killing.\n\r", ch);
		return;
	} else {
		argument = ch->focus;
	}
	...
	if (IS_NPC(ch) || number_percent() < ch->pcdata->learned[gsn_kick]) {
		...
		global_retcode =
		    damage(ch, victim,
			   number_range(1, ch->skill_level[COMBAT_ABILITY]),
			   gsn_kick);
	} else {
...
/* 	Required because of focus	Jet/Kristen 8/11/03 */
	...
	if (IS_NPC(ch) || percent < ch->pcdata->learned[gsn_disarm] * 2 / 3)
		...
		act(AT_PLAIN, "$n fails to disarm $N.", ch, NULL, victim,
		    TO_NOTVICT);
		act(AT_PLAIN, "$n fails to disarm you.", ch, NULL, victim,
		    TO_VICT);
...
void do_pick(CHAR_DATA * ch, char *argument)
{
	...
	if ((ship = ship_in_room(ch->in_room, arg)) != NULL) {
		...
		{
			send_to_char("You failed.\n\r", ch);
			sprintf(buf, "[ALARM] %s attempting to pick %s.",
				ch->name, ship->name);
...
bool check_parry(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (!IS_NPC(victim) && !IS_SET(victim->pcdata->flags, PCFLAG_GAG))
		/*SB*/
		    act(AT_SKILL, "You parry $n's attack.", ch, NULL, victim,
			TO_VICT);

	if (!IS_NPC(ch) && !IS_SET(ch->pcdata->flags, PCFLAG_GAG))	/* SB */
		act(AT_SKILL, "$N parries your attack.", ch, NULL, victim,
		    TO_CHAR);
...
void do_circle(CHAR_DATA * ch, char *argument)
{
	...
	percent =
	    number_percent() - (get_curr_dex(ch) - 16) + (get_curr_dex(victim) -
							  13);
