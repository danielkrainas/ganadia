...
char *halucinated_object(int ms, bool fShort)
{
	...
	case 15:
		return "The long lost boots of Spike lie off to the side.";
	case 16:
		return
...
/*Scryn 8/13*/
	if (IS_AFFECTED(victim, AFF_BLOODYEYE)) {
		...
		ch_printf(ch,
			  "%s has a murderous stare and bloodshot eyes.\n\r",
			  IS_NPC(victim) ? capitalize(victim->
						      short_descr) : (victim->
								      name));
	}
...
*/
	...
	case POS_RESTING:
		if (ch->position == POS_RESTING)
			...
			strcat(buf,
			       " is sprawled out at the foot of your mount.");
		else
...
void show_char_to_char_1(CHAR_DATA * victim, CHAR_DATA * ch)
{
	...
	if (victim->description[0] != '\0') {
		...
		act(AT_PLAIN, "You see nothing special about $M.", ch, NULL,
		    victim, TO_CHAR);
	}
	...
	if (!IS_NPC(victim)) {
		ch_printf(ch,
			  "%s looks about %d cm tall and seems to weigh about %d kg.\n\r",
	...
	for (iWear = 0; iWear < MAX_WEAR; iWear++) {
		if ((obj = get_eq_char(victim, iWear)) != NULL
			...
			if (!found) {
				send_to_char("\n\r", ch);
				act(AT_PLAIN, "$N is using:", ch, NULL, victim,
				    TO_CHAR);
				found = TRUE;
			}
...
void show_ships_to_char(SHIP_DATA * ship, CHAR_DATA * ch)
{
	...
	for (rship = ship; rship; rship = nship) {
		...
		if ((nship = rship->next_in_room) != NULL) {
			...
			ch_printf(ch, "%-35s",
				  (nship->name) ? nship->name : "no name set");
	...
	if (arg1[0] == '\0' || !str_cmp(arg1, "auto")) {
		...
		if (!ch->desc->original) {
			...
			if ((get_trust(ch) >= LEVEL_IMMORTAL)
				...
				send_to_char("[", ch);
				send_to_char(flag_string
					     (ch->in_room->room_flags, r_flags),
					     ch);
				if (ch->in_room->room_flags2 != 0) {
					send_to_char(" ", ch);
					send_to_char(flag_string
						     (ch->in_room->room_flags2,
						      r_flags2), ch);
				}
				send_to_char("]", ch);
			}
		...
		if (str_cmp(arg1, "auto"))
			if ((ship =
				...
				if (ship->starsystem) {
					...
					if (ship->starsystem->gate1
					    && str_cmp(ship->starsystem->gate1,
						       ""))
						ch_printf(ch, "%s\n\r",
							  ship->starsystem->
							  gate1);
					if (ship->starsystem->gate2
					    && str_cmp(ship->starsystem->gate2,
						       ""))
						ch_printf(ch, "%s\n\r",
							  ship->starsystem->
							  gate2);
					if (ship->starsystem->gate3
					    && str_cmp(ship->starsystem->gate3,
						       ""))
						ch_printf(ch, "%s\n\r",
							  ship->starsystem->
							  gate3);
					if (ship->starsystem->gate4
					    && str_cmp(ship->starsystem->gate4,
						       ""))
						ch_printf(ch, "%s\n\r",
							  ship->starsystem->
							  gate4);
					if (ship->starsystem->gate5
					    && str_cmp(ship->starsystem->gate5,
						       ""))
						ch_printf(ch, "%s\n\r",
							  ship->starsystem->
							  gate5);
					if (ship->starsystem->gate6
					    && str_cmp(ship->starsystem->gate6,
						       ""))
						ch_printf(ch, "%s\n\r",
							  ship->starsystem->
							  gate6);
					if (ship->starsystem->planet1
					    && str_cmp(ship->starsystem->
						       planet1, ""))
						ch_printf(ch, "%s\n\r",
							  ship->starsystem->
							  planet1);
			...
			if (!fAuto) {
				if (IS_SET(pexit->exit_info, EX_CLOSED)) {
					sprintf(buf + strlen(buf),
						"&z%-5s &g- &r(&Rclosed&r)&W\n\r",
						capitalize(dir_name
							   [pexit->vdir]));
				} else if (IS_SET(pexit->exit_info, EX_WINDOW)) {
					sprintf(buf + strlen(buf),
						"&z%-5s &g- &r(&wwindow&r)&W\n\r",
						capitalize(dir_name
							   [pexit->vdir]));
...
void do_help(CHAR_DATA * ch, char *argument)
{
	...
	if ((pHelp = get_help(ch, argument)) == NULL) {
		send_to_char
		    ("Sorry, there is no help file on that subject.\n\r", ch);
		return;
	}
		...
		if (is_number(arg)) {
		    ...
		    if ((pClan = get_clan(arg)))
				...
				if (iRace == MAX_RACE && fClanMatch == FALSE) {
					send_to_char
					    ("That's not a race, or organization.\n\r",
					     ch);
					return;
				}
			}
...
/* start from last to first to get it in the proper order */
	for (d = last_descriptor; d; d = d->prev) {
		...
		default:
			break;
		case 200:
			race = "The Ghost in the Machine&W";
			break;
		case MAX_LEVEL - 0:
			race = "Project Lead";
			break;
		case MAX_LEVEL - 1:
			race = "Lead Programmer";
			break;
		case MAX_LEVEL - 2:
			race = "Administrator";
			break;
		...
		sprintf(buf, "&r(&O%-20s &r)&W %s%s%s%s%s%s%s%s%s%s%s\n\r",
			...
			IS_SET(wch->pcdata->flags,
			       PCFLAG_OUTLAW) ? "&R[OUTLAW]&W " : "",
			IS_SET(wch->act, PLR_RP) ? "&R[RP]&W " : "",
			(wch->pcdata->bountyrelease > 0) ? "&R[JAILED]&W " : "",
			IS_SET(wch->act, PLR_AFK) ? "&R[AFK] " : "",
			wch->desc->connected ==
			CON_EDITING ? "&R[Writing]&W " : "", char_name,
			newtitle, extra_title,
			(IS_IMMORTAL(ch)) ? clan_name : (wch->pcdata
							 && wch->pcdata->clan
							 && (wch->pcdata->
							     clan ==
							     ch->pcdata->clan
...
void do_consider(CHAR_DATA * ch, char *argument)
{
	...
	act(AT_WHITE, "$n examines $N closely looking for any weaknesses.", ch,
	    NULL, victim, TO_NOTVICT);
	act(AT_WHITE, "You examine $N closely looking for any weaknesses.", ch,
	    NULL, victim, TO_CHAR);
	act(AT_WHITE, "$n examines you closely looking for weaknesses.", ch,
	    NULL, victim, TO_VICT);
	...
	diff =
	    victim->top_level - ch->top_level + GET_HITROLL(victim) -
	    GET_HITROLL(ch);
	if (diff <= -35) {
	...
	diff =
	    victim->top_level - ch->top_level + GET_DAMROLL(victim) -
	    GET_DAMROLL(ch);
	if (diff <= -35) {
	...
	if (diff <= -11) {
		...
		msg =
		    "Conclusion: You shouldn't have a lot of trouble defeating $M.";
	} else if (diff <= 10) {
		msg =
		    "Conclusion: $N is no match for you.  You can easily beat $M.";
	} else {
		msg =
...
void do_practice(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		...
		for (sn = 0; sn < top_sn; sn++) {
			...
			if (skill_table[sn]->guild == LEADERSHIP_ABILITY
			    && !IS_IMMORTAL(ch))
				continue;
...
*/
		...
		if (ch->gold < skill_table[sn]->min_level * 10) {
			sprintf(buf,
				"$n tells you, 'I charge %d dollars to teach that. You don't have enough.'",
				skill_table[sn]->min_level * 10);
			act(AT_TELL,
			    "$n tells you 'You don't have enough money.'", mob,
			    NULL, ch, TO_VICT);
			return;
		}
...
void do_teach(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		...
		if (skill_table[sn]->guild < 0
		    || skill_table[sn]->guild >= MAX_ABILITY) {
			act(AT_TELL, "Thats just not going to happen.", victim,
			    NULL, ch, TO_VICT);
			return;
		}
		...
		if (victim->pcdata->learned[sn] >= adept) {
			act(AT_TELL, "$n must practice that on their own.",
			    victim, NULL, ch, TO_VICT);
			return;
		}
		if (ch->pcdata->learned[sn] < 100) {
			...
			act(AT_ACTION, buf, ch, NULL, skill_table[sn]->name,
			    TO_CHAR);
			sprintf(buf, "%s teaches you $T.", ch->name);
			act(AT_ACTION, buf, victim, NULL, skill_table[sn]->name,
			    TO_CHAR);
		}
	}
	return;
...
void do_channels(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		send_to_char(!IS_SET(ch->deaf, CHANNEL_103)
			     ? " +BROADCAST" : " -broadcast", ch);

		send_to_char(!IS_SET(ch->deaf, CHANNEL_TELLS)
			     ? " +TELLS" : " -tells", ch);
		send_to_char(!IS_SET(ch->deaf, CHANNEL_OOC) ? " +OOC" : " -OOC",
			     ch);
		...
		send_to_char(!IS_SET(ch->deaf, CHANNEL_ASK) ? " +ASK" : " -ask",
			     ch);
		...
		if (!str_cmp(arg + 1, "auction"))
			...
			bit = CHANNEL_PRAY;
		else if (!str_cmp(arg + 1, "oldbieooc"))
			bit = CHANNEL_AVTALK;
			...
			bit = CHANNEL_NEWBIE;
		else if (!str_cmp(arg + 1, "broadcast"))
			bit = CHANNEL_103;
		else if (!str_cmp(arg + 1, "otells"))
			bit = CHANNEL_OTELLS;
		else {
...
//      else if ( !str_prefix( arg+1, "flee"     ) ) bit = PLR_FLEE;
		...
		if (bit) {
			...
			if (!str_prefix(arg + 1, "norecall"))
				...
				bit = PCFLAG_PAGERON;
			else if (!str_prefix(arg + 1, "autofocus"))
				bit = PCFLAG_AUTOFOCUS;
...
void do_rp(CHAR_DATA * ch, char *argument)
{
	...
	if IS_SET
		...
		act(AT_GREY, "$n is no longer roleplaying.", ch, NULL, NULL,
		    TO_ROOM);
	} else {
