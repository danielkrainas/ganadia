...
void do_score(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch, "&CAge&R: &W%3d years old &CLog In&R:&W  %s\r",
	...
	ch_printf(ch,
		  "&CAlign&R: &W%-5d &CWimpy&R: &W%-3d    &CQuest Points&R: &W%-2d   &CToggle&R: &W",
	...
	ch_printf(ch,
		  "&CHit Points&R: &C%d &Wof &B%d   &CStamina&R: &C%d &Wof &B%d\n\r",
		  ch->hit, ch->max_hit, ch->move, ch->max_move);

	ch_printf(ch,
		  "&CStr&R:&B(&W%2d&B) &CDex&R:&B(&W%2d&B) &CCon&R:&B(&W%2d&B) &CInt&R:&B(&W%2d&B) &CWis&R:&B(&W%2d&B) &CCha&R:&B(&W%2d&B) &CLck&R:&B(&W??&B) \n\r",
		  get_curr_str(ch), get_curr_dex(ch), get_curr_con(ch),
		  get_curr_int(ch), get_curr_wis(ch), get_curr_cha(ch));
	...
	ch_printf(ch,
		  "&CBANK&R: &W%-10s   &CMkills&R: &W%-5.5d    &CPkills&R: &W%-5.5d\n\r",
		  num_punct(ch->pcdata->bank), ch->pcdata->mkills,
		  ch->pcdata->pkills);

	ch_printf(ch,
		  "&CWeight&R: &W%5.5d &B(&Cmax &W%7.7d&B)    &CItems&R: &W%5.5d &B(&Cmax &W%5.5d&B)   &CPdeaths&R: &W%-5.5d\n\r",
		  ch->carry_weight, can_carry_w(ch), ch->carry_number,
		  can_carry_n(ch), ch->pcdata->pdeaths);

	ch_printf(ch,
		  "&CPager&R: &B(&W%c&B) &W%3d    &CAutoExit&B(&W%c&B)  &CAutoLoot&B(&W%c&B)   &CAutosac&B(&W%c&B)\n\r",
		  IS_SET(ch->pcdata->flags, PCFLAG_PAGERON) ? 'X' : ' ',
		  ch->pcdata->pagerlen, IS_SET(ch->act,
					       PLR_AUTOEXIT) ? 'X' : ' ',
		  IS_SET(ch->act, PLR_AUTOLOOT) ? 'X' : ' ', IS_SET(ch->act,
								    PLR_AUTOSAC)
		  ? 'X' : ' ');
	ch_printf(ch, "&CAFK Message&R:&W ");
	if (ch->pcdata->afkmessage == NULL)
		ch->pcdata->afkmessage = STRALLOC("");
	if (ch->pcdata->afkmessage[0] == '\0'
	...
	if (ch->pcdata->maxarresttime > 0)
		ch_printf(ch,
			  "&CMaximum Time (In Ticks) with a Bounty&R: &B(&W%d&B)\r\n",
			  ch->pcdata->maxarresttime);
	if (ch->pcdata->clantalk1 || ch->pcdata->clantalk2) {
		ch_printf(ch,
			  "&CYou have the ability to use these clan radios&R:&W %s%s%s\r\n",
			  (ch->pcdata->clantalk1) ? ch->pcdata->clantalk1->
			  name : "", (ch->pcdata->clantalk1
				      && ch->pcdata->clantalk2) ? " and " : "",
			  (ch->pcdata->clantalk2) ? ch->pcdata->clantalk2->
			  name : "");
	}
	statcount = 0;
	...
	if (statcount >= 75)
		ch_printf(ch,
			  "&BYour body is at the max of its ability to gain anything from Stat Training.\r\n");
	...
	if (ch->position != POS_SLEEPING)
		...
		case 4:
			send_to_char
			    ("Your mind is going 100 miles an hour.\n\r", ch);
			break;
		case 5:
		...
		case 6:
			send_to_char
			    ("Your mind and body are slipping apart.\n\r", ch);
			break;
		case 7:
		...
		send_to_char
		    ("Your sleep is filled with strange and vivid dreams.\n\r",
		     ch);
	...
	if (ch->pcdata->weaponl > 1)
		ch_printf(ch,
			  "You have about %d ticks left till you can get a new Weapon License.\r\n",
			  ch->pcdata->weaponl);
	if (ch->pcdata->weaponl == -1)
		ch_printf(ch,
			  "Your Weapon License is suspended indefinitely. \r\n");
	...
	if (ch->pcdata->clan) {
		send_to_char
		    ("&R----------------------------------------------------------------------------\n\r",
		     ch);
		ch_printf(ch,
			  "&CORGANIZATION&R: &W%-35s &CPkills&W/&CDeaths&R: &Y%3.3d&R/&C&W%3.3d",
			  ch->pcdata->clan->name, ch->pcdata->clan->pkills,
			  ch->pcdata->clan->pdeaths);
		send_to_char("\n\r", ch);
		if (ch->pcdata->clan->clan_type != 1) {
			ch_printf(ch,
				  "&CWars Won&R: &W%d	&CWars Lost&R: &W%d\r\n",
				  ch->pcdata->clan->warwoncount,
				  ch->pcdata->clan->warlostcount);
		}
		if (ch->pcdata->clan->war1 || ch->pcdata->clan->war2) {
			ch_printf(ch, "&CAt War With&R:&W %s%s%s.",
				  (ch->pcdata->clan->war1) ? ch->pcdata->clan->
				  war1->name : "", (ch->pcdata->clan->war1
						    && ch->pcdata->clan->
						    war2) ? " and " : "",
				  (ch->pcdata->clan->war2) ? ch->pcdata->clan->
				  war2->name : "");
			send_to_char("\n\r", ch);
		}
	}
	if (IS_IMMORTAL(ch)) {
		...
		ch_printf(ch,
			  "&CIMMORTAL DATA&R:  &CWizinvis &B[&W%s&B]  &CWizlevel &B(&W%d&B)\n\r",
			  IS_SET(ch->act, PLR_WIZINVIS) ? "&RX" : " ",
			  ch->pcdata->wizinvis);

		ch_printf(ch, "&CBamfin&R:  &W%s\n\r",
			  (ch->pcdata->bamfin[0] != '\0')
			  ? ch->pcdata->
			  bamfin : "%s appears in a swirling mist.", ch->name);
		ch_printf(ch, "&CBamfout&R: &W%s\n\r",
			  (ch->pcdata->bamfout[0] !=
			   '\0') ? ch->pcdata->
			  bamfout : "%s leaves in a swirling mist.", ch->name);

		/* Area Loaded info - Scryn 8/11 */
		if (ch->pcdata->area) {
			ch_printf(ch,
				  "&CVnums&R:   &CRoom &B(&W%-5.5d - %-5.5d&B)   &CObject &B(&W%-5.5d - %-5.5d&B)   &CMob &B(&W%-5.5d - %-5.5d&B)\n\r",
	...
	if (ch->first_affect) {
		...
		send_to_char
		    ("&C----------------------------------------------------------------------------\n\r",
		     ch);
		send_to_char("&CAFFECT DATA&R:                            ",
			     ch);
		for (paf = ch->first_affect; paf; paf = paf->next) {
			if ((sktmp = get_skilltype(paf->type)) == NULL)
				continue;
			if (ch->top_level < 20) {
				ch_printf(ch, "&B[&W%-34.34s&B]    ",
					  sktmp->name);
				if (i == 0)
					i = 2;
				if ((++i % 3) == 0)
				...
				if (paf->modifier == 0)
					...
					ch_printf(ch,
						  "&B[&W%-11.11s;%+-3.3d %7.7s;%5d rds&B]&W    ",
						  supercapitalize(sktmp->name),
						  paf->modifier,
						  tiny_affect_loc_name(paf->
								       location),
...
void do_oldscore(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch,
		  ...
		  ch->top_level, ch->pcage, (get_age(ch)));
	...
	ch_printf(ch, "You have have %d dollars.\n\r", ch->gold);
...
void do_level(CHAR_DATA * ch, char *argument)
{
	...
	for (ability = 0; ability < MAX_ABILITY; ability++)
		if (ability != FORCE_ABILITY
			...
			ch_printf(ch,
				  "&W%-15s   &CLevel&R: &W%-3d   &CMax&R: &W%-3d   &CExp&R: &W%-10ld   &CNext&R: &W%-10ld\n\r",
				  ability_name[ability],
				  ch->skill_level[ability], max_level(ch,
								      ability),
				  ch->experience[ability],
				  exp_level(ch->skill_level[ability] + 1));
		else
			ch_printf(ch,
				  "&W%-15s   &CLevel&R:&W %-3d   &CMax&R: &W???   &CExp&R: &W???          &CNext&R: &W???\n\r",
				  ability_name[ability],
		...
		for (paf = ch->first_affect; paf; paf = paf->next)
			if ((skill = get_skilltype(paf->type)) != NULL) {
				...
				if (ch->top_level >= 20) {
					...
					if (paf->duration < 6)
						set_char_color(AT_WHITE +
							       AT_BLINK, ch);
					ch_printf(ch, "(%5d)   ",
						  paf->duration);
				}
				ch_printf(ch, "%-18s\n\r",
					  supercapitalize(skill->name));
			}
	}
	return;
}
