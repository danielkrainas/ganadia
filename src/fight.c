...
void violence_update(void)
{
	...
	for (ch = last_char; ch; lst_ch = ch, ch = gch_prev) {
		...
		if (IS_SET(ch->in_room->room_flags, ROOM_SAFE)
		    && victim->perm_frc == 0 && ch->perm_frc == 0) {
			sprintf(buf,
			   ...
			   else */
		{
			...
			if (!IS_NPC(ch)
			    ...
			    && number_range(1, 4) == 1) {
				damage(ch, ch,
				       ch->pcdata->condition[COND_BLEEDING],
				       TYPE_UNDEFINED);
				act(AT_BLOOD, "You're losing blood...", ch,
				    NULL, NULL, TO_CHAR);
				act(AT_BLOOD, "$n is losing blood...", ch, NULL,
				    NULL, TO_ROOM);
			}
		}
	...
	if (get_eq_char(ch, WEAR_DUAL_WIELD)) {
		dual_bonus =
		    IS_NPC(ch) ? (ch->skill_level[COMBAT_ABILITY] /
				  10) : (ch->pcdata->learned[gsn_dual_wield] /
					 10);
		chance =
	...
	if (!IS_NPC(ch) && wield) {
		...
		case 9:
			*gsn_ptr = gsn_pistols;
			break;
		case 11:
			*gsn_ptr = gsn_heavy_weapons;
			break;

		}
		if (*gsn_ptr != -1)
			bonus =
			    (int)((ch->pcdata->learned[*gsn_ptr] - 50) / 10);
...
ch_ret one_hit(CHAR_DATA * ch, CHAR_DATA * victim, int dt)
{
	...
	thac0_32 = 8;
	thac0 =
	    interpolate((int)(ch->skill_level[COMBAT_ABILITY] / 4), thac0_00,
	...
	if (diceroll == 1 || (diceroll != 40 && diceroll < thac0 - victim_ac)) {
		...
		if (diceroll < thac0 - victim_ac
		    && diceroll >=
		    thac0 - (10 + (dex_app[get_curr_dex(ch)].defensive / 10))
		    && diceroll != 19 && diceroll != 0) {
			if (prof_gsn != -1)
				learn_from_failure(ch, prof_gsn);
			act(AT_PLAIN, "Your attack is deflected by $N's armor.",
			    ch, NULL, victim, TO_CHAR);
			act(AT_PLAIN, "Your armor deflects $n's attack.", ch,
			    NULL, victim, TO_VICT);
			/*
	...
	if (!IS_NPC(ch) && ch->pcdata->learned[gsn_enhanced_damage] > 0) {
		dam +=
		    (int)(dam * ch->pcdata->learned[gsn_enhanced_damage] / 120);
	...
	if (dt == (TYPE_HIT + WEAPON_RIFLE) && wield
	    && wield->item_type == ITEM_WEAPON) {
		if (wield->value[4] < 1) {
			act(AT_YELLOW,
			    "$n points their rifle at you, but nothing happens.",
			    ch, NULL, victim, TO_VICT);
			act(AT_YELLOW,
			    "*CLICK* ... Your rifle needs ammunition!", ch,
			    NULL, victim, TO_CHAR);
			if (IS_NPC(ch)) {
	...
	else if (dt == (TYPE_HIT + WEAPON_HEAVY_WEAPON) && wield
		 && wield->item_type == ITEM_WEAPON) {
		if (wield->value[4] < 1) {
			act(AT_YELLOW,
			    "$n points their heavy weapon at you, but nothing happens.",
			    ch, NULL, victim, TO_VICT);
			act(AT_YELLOW, "Your heavy weapon needs reloading ...",
			    ch, NULL, victim, TO_CHAR);
			if (IS_NPC(ch)) {
				do_remove(ch, wield->name);
			}
	...
	else if (dt == (TYPE_HIT + WEAPON_PISTOL) && wield
		 && wield->item_type == ITEM_WEAPON) {
		if (wield->value[4] < 1) {
			act(AT_YELLOW,
			    "$n points their pistol at you, but nothing happens.",
			    ch, NULL, victim, TO_VICT);
			act(AT_YELLOW,
			    "*CLICK* ... Your pistol needs reloading!", ch,
			    NULL, victim, TO_CHAR);
			if (IS_NPC(ch)) {
				do_remove(ch, wield->name);
			}
...
ch_ret damage(CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
		...
		if (damobj) {
			...
			if (dam > 18) {
				if (!IS_NPC(victim) && number_range(1, 10) == 1)
					gain_condition(victim, COND_BLEEDING,
						       1);
			}
		} else
	 ...
	 */
	if (dam && ch != victim && !IS_NPC(ch) && ch->fighting
	    && ch->fighting->xp) {
		xp_gain =
		    (int)(xp_compute(ch, victim) * 1 * dam) / victim->max_hit;
	...
	case POS_MORTAL:
		act(AT_DYING, "$n is mortally wounded, and about to die.",
		    victim, NULL, NULL, TO_ROOM);
		send_to_char
		    ("&RYou are knocked unconcious, and are bleeding freely.\n\r",
		     victim);
	...
	case POS_STUNNED:
		if (!IS_AFFECTED(victim, AFF_PARALYSIS)) {
			act(AT_ACTION,
			    "$n is stunned, but will probably recover.", victim,
			    NULL, NULL, TO_ROOM);
			send_to_char
	...
	case POS_DEAD:
		if (dt >= 0 && dt < top_sn) {
			...
			if (skill->die_char && skill->die_char[0] != '\0')
				act(AT_DEAD, skill->die_char, ch, NULL, victim,
				    TO_CHAR);
			if (skill->die_vict && skill->die_vict[0] != '\0')
				act(AT_DEAD, skill->die_vict, ch, NULL, victim,
				    TO_VICT);
			if (skill->die_room && skill->die_room[0] != '\0')
				act(AT_DEAD, skill->die_room, ch, NULL, victim,
				    TO_NOTVICT);
		}
		if (IS_NPC(victim) && IS_SET(victim->act, ACT_NOKILL))
			...
			if (!IS_SET(victim->act, ACT_NOCORPSE))
				act(AT_DEAD, "$n is DEAD!", victim, 0, 0,
				    TO_ROOM);
			else
				act(AT_DEAD, "$n has been DEFEATED!", victim, 0,
				    0, TO_ROOM);
		} else
			act(AT_DEAD, "$n is about to DIE!", victim, 0, 0,
			    TO_ROOM);
	...
	if (victim->hit <= 0 && !IS_NPC(victim)) {
		...
		if (victim->gold != 0) {
			...
			act(AT_ACTION, "$n drops some wulongs.", victim, NULL,
			    NULL, TO_ROOM);
...
/*	check_killer( ch, victim );	-added to raw_kill */
	...
	if (!npcvict
	    && victim->hit > 0 && victim->hit <= victim->wimpy
	    && victim->wait == 0)
		do_flee(victim, "");
		...
		if (!IS_NPC(ch)) {
			for (x = 0; x < 32; x++) {
				if (IS_SET(victim->vip_flags, 1 << x)
				    && !IS_SET(victim->act, ACT_DEADLY)) {
					SET_BIT(ch->pcdata->wanted_flags,
						1 << x);
					ch_printf(ch,
						  "&YYou are now wanted on %s.&w\n\r",
						  planet_flags[x],
						  victim->short_descr);
					disintigration(victim, ch, 50000);
				}
			}
			if (ch->pcdata->clan)
...
/* Take care of polymorphed chars */
	...
	if (victim->in_room && IS_NPC(victim) && victim->vip_flags != 0
	    && victim->in_room->area && victim->in_room->area->planet
	    && !IS_NPC(ch)) {
	...
	if (IS_NPC(victim) && !IS_NPC(ch)
	    && (IS_SET(victim->act, ACT_CITIZEN)
		|| IS_SET(victim->act, ACT_SUPPORTER))) {
		...
		if (planet && planet->frustrated_by
		    && planet->frustrated_by->name && planet->governed_by
		    && planet->frustrated_by != planet->governed_by && clan
		    && clan->clan_type == 0) {
...
//    send_to_char("&RYou are slightly aware of the IV's in your arm \n\ras you are transported to the hospital.\n\r", victim);
	send_to_char
	    ("&RYou feel a coursing of excrutiating pain through your body, and then...\n\n\r",
	     victim);
	send_to_char
	    ("&R...nothing.  The sirens of an ambulance blare as you hear paramedics\n\r",
	     victim);
	send_to_char
	    ("&Rshouting orders, but their voices fade away to silence.\n\n\r",
	     victim);
	send_to_char
	    ("&GSome time later, you wake up in a hospital bed, barely clinging to life.&W\n\r",
	     victim);
	...
	act(AT_BLOOD, "$n's life has been tested.", victim, NULL, NULL,
	    TO_ROOM);
	...
	if (ch && victim && !IS_NPC(ch)
	    ...
	    && ch->pcdata
	    && ch->pcdata->clan
	    && ch != victim && (!str_cmp(ch->pcdata->clan->name, "RBH"))
	    && bounty && bounty->amount > 0) {
		canget = TRUE;
	}
	if (ch && victim && !IS_NPC(ch) && !IS_NPC(victim)
	    && ch->pcdata->clan
	    && !str_cmp(ch->pcdata->clan->name, "ISSP") && bounty) {
		canget = TRUE;
	}
	// War stuff! - Gatz

	if (!IS_NPC(ch) && !IS_NPC(victim) && ch->pcdata->clan
	    && victim->pcdata->clan)
...
void group_gain(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	for (gch = ch->in_room->first_person; gch; gch = gch->next_in_room) {
		...
		if (!IS_NPC(gch) && IS_NPC(victim) && gch->pcdata
			...
			xp = 0;
			sprintf(buf,
				"You receive no experience for killing your organization's resources.\n\r");
...
make it simple instead */
	...
	return URANGE(-1000, (int)(gch->alignment - victim->alignment / 5),
		      1000);
...
int xp_compute(CHAR_DATA * gch, CHAR_DATA * victim)
{
	...
	return URANGE(1, xp,
		      (exp_level(gch->skill_level[COMBAT_ABILITY] + 1) -
		       exp_level(gch->skill_level[COMBAT_ABILITY])));
}
...
void dam_message(CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
	...
	if (dam == 0) {
		vs = "&Rm&ri&Rs&rs&W";
		vp = "&Rm&ri&Rs&rs&Re&rs&W";
	} else if (dampc <= 5) {
		vs = "&Cb&ca&Cr&ce&Cl&cy &Cs&cc&Cr&ca&Ct&cc&Ch&W";
		vp = "&Cb&ca&Cr&ce&Cl&cy &Cs&cc&Cr&ca&Ct&cc&Ch&ce&Cs&W";
	} else if (dampc <= 10) {
		vs = "&Gs&gc&Gr&ga&Gt&gc&Gh&W";
		vp = "&Gs&gc&Gr&ga&Gt&gc&Gh&ge&Gs&W";
	} else if (dampc <= 20) {
		vs = "&Pn&pi&Pc&pk&W";
		vp = "&Pn&pi&Pc&pk&Ps&W";
	} else if (dampc <= 30) {
		vs = "&Bg&br&Ba&bz&Be&W";
		vp = "&Bg&br&Ba&bz&Be&bs&W";
	} else if (dampc <= 40) {
		vs = "&Cb&Rr&Cu&Ri&Cs&Re&W";
		vp = "&Cb&Rr&Cu&Ri&Cs&Re&Cs&W";
	} else if (dampc <= 50) {
		vs = "&Gh&Ci&Gt&W";
		vp = "&Ch&Gi&Ct&Gs&W";
	} else if (dampc <= 60) {
		vs = "&Yi&Gn&Yj&Gu&Yr&Ge&W";
		vp = "&Yi&Gn&Yj&Gu&Yr&Ge&Ys&W";
	} else if (dampc <= 75) {
		vs = "&Rt&Bh&Rr&Ba&Rs&Bh&W";
		vp = "&Rt&Bh&Rr&Ba&Rs&Bh&Re&Bs&W";
	} else if (dampc <= 80) {
		vs = "&Yw&Co&Yu&Cn&Yd&W";
		vp = "&Yw&Co&Yu&Cn&Yd&Cs&W";
	} else if (dampc <= 90) {
		vs = "&rm&ca&ru&cl&W";
		vp = "&rm&ca&ru&cl&rs&W";
	} else if (dampc <= 125) {
		vs = "&Rd&ge&Rc&gi&Rm&ga&Rt&ge&W";
		vp = "&Rd&ge&Rc&gi&Rm&ga&Rt&ge&Rs&W";
	} else if (dampc <= 150) {
		vs = "&Pd&Ce&Pv&Ca&Ps&Ct&Pa&Ct&Pe&W";
		vp = "&Pd&Ce&Pv&Ca&Ps&Ct&Pa&Ct&Pe&Cs&W";
	} else if (dampc <= 200) {
		vs = "&Rm&Ya&Ri&Ym&W";
		vp = "&Rm&Ya&Ri&Ym&Rs&W";
	} else if (dampc <= 300) {
		...
		vs = "&RE&rV&RI&rS&RC&rE&RR&rA&RT&RE&W";
		vp = "&RE&rV&RI&rS&RC&rE&RR&rA&RT&rE&RS&W";
	} else if (dampc <= 990) {
		vs = "&R* &COBLITERATE &R*&W";
		vp = "&R* &COBLITERATES &R*&W";
	} else {
		vs = "&R*&C*&R* &CANNIHILATE &R*&C*&R*&W";
		vp = "*** ANNIHILATES ***";
	}
	...
	if (dam == 0
	    && (!IS_NPC(ch) && (IS_SET(ch->pcdata->flags, PCFLAG_GAG))))
		gcflag = TRUE;
	...
	if (dt == TYPE_HIT || dam == 0) {
		...
		if (skill) {
			...
			if (dam == 0) {
				...
				if (skill->miss_char
				    && skill->miss_char[0] != '\0') {
					act(AT_HIT, skill->miss_char, ch, NULL,
					    victim, TO_CHAR);
					found = TRUE;
				}
				if (skill->miss_vict
				...
				if (skill->hit_vict
				    && skill->hit_vict[0] != '\0')
					act(AT_HITME, skill->hit_vict, ch, NULL,
					    victim, TO_VICT);
				if (skill->hit_room
				    && skill->hit_room[0] != '\0')
	...
	if (!IS_NPC(ch) && !IS_NPC(victim)) {
		...
		if (adren > 0) {
			if (victim->perm_frc < 5) {
				if (avmessage == 0) {
					send_to_char
					    ("&RYou feel your heart rate speed up!\n\r",
					     victim);
					avmessage++;
				}
				victim->perm_frc += adren;
				if (victim->perm_frc > 5) {
					send_to_char
					    ("&RYou feel your chest start to burn with Adrenaline!\n\r",
					     victim);
					victim->perm_frc = 5;
				}
...
void do_attack(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->focus == "none" || ch->focus == NULL) {
		send_to_char
		    ("You must focus on an opponent before attacking.\n\r", ch);
		return;
	} else {
		argument = ch->focus;
	}
...
void do_focus(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(argument, "none")) {
		if (ch->focus == "none") {
			send_to_char
			    ("You are going to stop focusing on nobody?\n\r",
			     ch);
			return;
		} else {
	...
	if (IS_NPC(victim) && victim->frustration > 60) {
		...
		case 1:
			do_say(victim,
			       "If you want to fight, you picked the wrong day!");
			break;
		default:
			do_say(victim,
			       "I am in a bad mood, don't mess with me.");
			break;
		}
	}
	...
	if (ch->master == victim) {
		send_to_char
		    ("You can't attack the person you are following!\n\r", ch);
		return;
	}
	...
	if (!IS_NPC(ch) && !IS_NPC(victim) && !IS_IMMORTAL(ch)) {
		...
		if (ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, "RBH")
		    && bounty)
			check = TRUE;
		...
		if (ch->pcdata->clan && victim->pcdata->clan
		    && (ch->pcdata->clan->war1 == victim->pcdata->clan
			|| ch->pcdata->clan->war2 == victim->pcdata->clan))
			check = TRUE;
	}

	if (!check && !IS_NPC(victim) && !IS_NPC(ch)) {
		sprintf(log_buf,
			"%s has focused on %s! Possible Illegal Action!",
			ch->name, victim->name);
		log_string(log_buf);
		to_channel(log_buf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL);
	}
	act(AT_PLAIN, "You focus your attention on $N.", ch, NULL, victim,
	    TO_CHAR);
	act(AT_PLAIN, "$n focuses $s attention on you.", ch, NULL, victim,
	    TO_VICT);
	act(AT_PLAIN, "$n focuses $s attention on $N.", ch, NULL, victim,
	    TO_NOTVICT);
	// Auto-focus - Gatz

	if (!IS_NPC(victim) && !IS_NPC(ch)
	    && IS_SET(victim->pcdata->flags, PCFLAG_AUTOFOCUS)
	    && (victim->focus == "none" || victim->focus == NULL)) {
		if (!IS_NPC(victim)
		    && (victim->focus == "none" || victim->focus == NULL)) {
			act(AT_PLAIN, "You quickly focus back on $n.", ch, NULL,
			    victim, TO_VICT);
			act(AT_PLAIN, "$N quickly focuses on you.", ch, NULL,
			    victim, TO_CHAR);
			act(AT_PLAIN, "$N quickly focuses on $n.", ch, NULL,
			    victim, TO_NOTVICT);
...
bool check_anti_tanking(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	for (lowbie = ch->in_room->first_person; lowbie;
	     lowbie = lowbie->next_in_room) {
		if (lowbie == ch || lowbie == victim || IS_NPC(lowbie))
			continue;
		if (who_fighting(lowbie) == ch
		    && lowbie->skill_level[COMBAT_ABILITY] <
		    victim->skill_level[COMBAT_ABILITY] - 20) {
			act(AT_CARNAGE,
			    "$n sees that $N has joined the fight, and decides $e is a much better prey!",
			...
			sprintf(buf,
				"check_anti_tanking: Tanking detected. %s is tanking for %s, with mob %d",
...
void ricochet(CHAR_DATA * ch)
{
	...
	if (count > 1) {
		if (number_range(0, 5) > 4) {
			...
			act(AT_YELLOW, "$n's bullet ricochets and hits $N!", ch,
			    NULL, rch, TO_NOTVICT);
			act(AT_YELLOW, "$n's bullet ricochets and hits you!",
			    ch, NULL, rch, TO_VICT);
			act(AT_YELLOW, "Your bullet ricochets and hits $N!", ch,
			    NULL, rch, TO_CHAR);
