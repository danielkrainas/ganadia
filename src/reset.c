...
bool
is_room_reset(RESET_DATA * pReset, ROOM_INDEX_DATA * aRoom, AREA_DATA * pArea)
{
...
void
edit_reset(CHAR_DATA * ch, char *argument, AREA_DATA * pArea,
	   ROOM_INDEX_DATA * aRoom)
{
	char arg[MAX_INPUT_LENGTH];
	...
	if (!*arg || !str_cmp(arg, "?")) {
		char *nm =
		    (ch->substate ==
		     SUB_REPEATCMD ? "" : (aRoom ? "rreset " : "reset "));
		char *rn = (aRoom ? "" : " [room#]");
...
/*      pReset = make_reset('P', 1, pObj->vnum, num, reset->arg1);*/
			...
			if ((vnum = atoi(arg)) < 1)
				vnum = 1;
			pReset =
			    make_reset('P', reset->extra + 1, pObj->vnum, vnum,
				       0);
		...
		if (arg[0] == '\0' || !(num = (int)str_cmp(arg, "room")) ||
			...
			if ((vnum = atoi(arg)) < 1)
				vnum = 1;
			pReset =
			    make_reset('O', 0, pObj->vnum, vnum, pRoom->vnum);
...
/** Prevent Day and Night mob resets ** Rythmic */
			...
			{
				ROOM_INDEX_DATA *pRoomPrev =
				    get_room_index(pReset->arg3 - 1);

				if (pRoomPrev
				    && IS_SET(pRoomPrev->room_flags,
					      ROOM_PET_SHOP))
		...
		case 'E':
			if (!(pObjIndex = get_obj_index(pReset->arg1))) {
				bug("Reset_area: 'E' or 'G': bad obj vnum %d.",
				    pReset->arg1);
				continue;
			}
			if (!mob) {
			...
			if (IS_SET(pReset->extra, TRAP_OBJ)) {
				...
				if (pArea->nplayer > 0 ||
				    count_obj_list(get_obj_index(OBJ_VNUM_TRAP),
						   pRoomIndex->first_content) >
				    0)
					break;
				to_obj =
				    make_trap(pReset->arg1, pReset->arg1, 10,
					      pReset->extra);
			...
			case BIT_RESET_DOOR:
				{
					...
					doornum =
					    (pReset->arg2 & BIT_RESET_DOOR_MASK)
...
void
list_resets(CHAR_DATA * ch, AREA_DATA * pArea, ROOM_INDEX_DATA * pRoom,
	    int start, int end)
{
	RESET_DATA *pReset;
	...
	for (pReset = pArea->first_reset; pReset; pReset = pReset->next) {
		...
		case 'E':
			...
			if (!(obj = get_obj_index(pReset->arg1)))
				...
				oname = obj->name;
			sprintf(pbuf, "%s (%d) -> %s (%s) [%d]", oname,
				pReset->arg1, mname,
				(pReset->command ==
				 'G' ? "carry" : wear_locs[pReset->arg3]),
				pReset->arg2);
			if (mob && mob->pShop)
				strcat(buf, " (shop)\n\r");
			else
		...
		case 'P':
			...
			if (pReset->arg3 > 0) {
				...
				rname =
				    (obj2 ? obj2->name : "Object2: *BAD VNUM*");
				lastobj = obj2;
			} else if (!lastobj)
		...
		case 'B':
			{
				...
				if (IS_SET(pReset->arg2, BIT_RESET_SET)) {
				    ...
				    if (IS_SET(pReset->arg2, BIT_RESET_TOGGLE))
				{
					strcpy(pbuf, "Toggle: ");
					pbuf += 8;
				} else {
 ...
 */
RESET_DATA *add_reset(AREA_DATA * tarea, char letter, int extra, int arg1,
		      int arg2, int arg3)
{
	RESET_DATA *pReset;
 ...
 */
RESET_DATA *place_reset(AREA_DATA * tarea, char letter, int extra, int arg1,
			int arg2, int arg3)
{
