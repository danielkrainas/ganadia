...
/*  gettimeofday( &boot_time, NULL);   okay, so it's kludgy, sue me :) */
	...
	sprintf(log_buf, "Ganadia ready on port %d.", port);
	log_string(log_buf);
	game_loop();
	/*   close( control  );
	   close( control2 );
 ...
 */
	...
	if (!str_cmp(buf, "68.19.238.59")
	    ...
	    || !str_cmp(buf, "66.190.38.142")) {
		write_to_descriptor(desc,
				    "This IP has been caught illegally logging onto Immortal Accounts.",
				    0);
		free_desc(dnew);
		return;
	}
...
bool flush_buffer(DESCRIPTOR_DATA * d, bool fPrompt)
{
	...
	if (d->snoop_by) {
		...
		if (d->character && d->character->name) {
			...
			if (d->original && d->original->name)
				sprintf(buf, "%s (%s)", d->character->name,
					d->original->name);
			else
...
/*	extern int lang_array[];
	...
	case CON_GET_NAME:
		...
		if (!d->character) {
			sprintf(log_buf, "Bad player file %s@%s.", argument,
				d->host);
			log_string(log_buf);
			write_to_buffer(d,
		...
		if (fOld) {
			...
			write_to_buffer(d, buf, 0);
			send_to_char
			    (" Remember, names from Cowboy Bebop are not authed.&R (&rY&G/&rN&R)&r?&w ",
			     ch);
			//   write_to_buffer( d, buf, 0 );
...
//Enter RP Age, Height and Weight - Gatz
		...
		ch_printf(ch,
			  "(Note: Age doesn't affect gameplay, and will remain constant.)\r\n");
		ch_printf(ch,
	...
	case CON_GET_NEW_AGE:
		...
		if (x < 14 || x > 60) {
			write_to_buffer(d,
					"\n\rSorry, that isn't in the age range.",
					0);
			write_to_buffer(d,
					"\n\rEnter a age between 14 and 60: ",
					0);
			return;
		}
	...
	case CON_GET_NEW_HEIGHT:
		...
		if (x < 100 || x > 200) {
			write_to_buffer(d,
					"\n\rSorry, that isn't in the height range.",
					0);
			write_to_buffer(d,
					"\n\rRange is 100 to 200 cm, enter new height: ",
					0);
			return;
		}
		ch->pcheight = x;
	...
	case CON_GET_NEW_WEIGHT:
		...
		if (x < 50 || x > 150) {
			write_to_buffer(d,
					"\n\rSorry, that isn't in the weight range.",
					0);
			return;
		}
...
*/
	...
	case CON_READ_MOTD:
		...
		if (ch->top_level == 0) {
			...
			for (iLang = 0; lang_array[iLang] != LANG_UNKNOWN;
			     iLang++)
				if (lang_array[iLang] ==
				    race_table[ch->race].language)
					break;
			if (lang_array[iLang] == LANG_UNKNOWN)
				bug("Nanny: invalid racial language.");
			else {
				if ((iLang =
				     skill_lookup(lang_names[iLang])) < 0)
					...
					if (ch->race == RACE_QUARREN
					    && (iLang =
						skill_lookup("quarren")) >= 0) {
						ch->pcdata->learned[iLang] =
						    100;
						SET_BIT(ch->speaks,
							LANG_QUARREN);
					}
				...
				if (clan->war1 || clan->war2) {
					send_to_char
					    ("&W-------------------------------------------\n\r",
					     ch);
					ch_printf(ch,
						  "&RYou are currently at war with: %s%s%s.",
						  (clan->war1) ? clan->war1->
						  name : "", (clan->war1
							      && clan->
							      war2) ? " and " :
	...
	for (ch = first_char; ch; ch = ch->next) {
		if (!IS_NPC(ch)
			...
			if (fConn == FALSE) {
				...
				act(AT_ACTION, "$n has reconnected.", ch, NULL,
				    NULL, TO_ROOM);
	...
	for (dold = first_descriptor; dold; dold = dold->next) {
		if (dold != d && (dold->character || dold->original)
			...
			sprintf(log_buf, "%s attempting to multiplay with %s.",
				...
				character->name, d->character->name);
			log_string_plus(log_buf, LOG_COMM, sysdata.log_level);
			//        d->character = NULL;
			//        free_char( d->character );
			//        return TRUE;
		 ...
		 const void *arg1, const void *arg2)
{
	...
	while (*str != '\0') {
		...
		if (!arg2 && *str >= 'A' && *str <= 'Z') {
			...
			case 'Q':
				i = (to ==
				     ch) ? "your" : his_her[URANGE(0, ch->sex,
								   2)];
				break;
			case 'p':
...
bool pager_output(DESCRIPTOR_DATA * d)
{
	...
	d->pagecmd = -1;
	if (IS_SET(ch->act, PLR_ANSI))
		if (write_to_descriptor(d->descriptor, "\033[1;36m", 7) ==
		    FALSE)
			return FALSE;
...
void do_copyover(CHAR_DATA * ch, char *argument)
{
	...
	if (!fp) {
		...
		log_printf("Could not write to copyover file: %s",
			   COPYOVER_FILE);
	...
	if (!str_cmp(argument, "warn")) {
		...
		case 1:
			do_echo(ch,
				"^r ^x &RCindy&z:&W Coming Soon to Theaters: COPYOVER!&w ^x");
			break;
		case 2:
			do_echo(ch,
				"^b ^x An Earth Shattering Earthquake Rattles Your Bones&w ^b ^x");
			break;
		case 3:
			do_echo(ch,
				"^y ^x &RCindy&z:&W \"Copyover\" Is In The News Again, As Another One Is Heading Our Way!&w ^y ^x");
			break;
		case 4:
			do_echo(ch,
				"^c ^x &RCindy&z:&W Breaking story, Copyover is coming!&w ^c ^x");
			break;
		}
		return;
	}
	if (!str_cmp(argument, "poscrash")) {
		...
		case 1:
			do_echo(ch,
				"^p ^x &CPunch: Aye Dios Mio! Possible crash!&w ^p ^x");
			break;
		case 2:
			do_echo(ch,
				"^p ^x &PJudy: Eeps, possible crash! I hope I don't break a nail!&w ^p ^x");
			break;
		case 3:
			do_echo(ch,
				"^p ^x &CPunch: Hold it Amigos, a crash may happen!&w ^p ^x");
			break;
		case 4:
			do_echo(ch,
				"^p ^x &PJudy: Ahhhhhh, my hair, possible crash!!!!!&w ^p ^x");
			break;
		}
		return;
	}
...
void copyover_recover()
{
	...
	for (;;) {
		...
		if (!fOld) {	/* Player file not found?! */
			...
			act(AT_ACTION, "$n materializes!", d->character, NULL,
			    NULL, TO_ROOM);
			d->connected = CON_PLAYING;
			if (++num_descriptors > sysdata.maxplayers)
				sysdata.maxplayers = num_descriptors;
