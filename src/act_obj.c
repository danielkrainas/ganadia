...
void do_get(CHAR_DATA * ch, char *argument)
{
	...
	if (arg2[0] == '\0') {
		if (number <= 1 && str_cmp(arg1, "all")
			...
			/* 'get obj' */
			obj =
			    get_obj_list(ch, arg1, ch->in_room->first_content);
			if (!obj) {
				act(AT_PLAIN, "I see no $T here.", ch, NULL,
				    arg1, TO_CHAR);
				return;
			}
			separate_obj(obj);
			...
			if (!found) {
				if (fAll)
					...
					act(AT_PLAIN, "I see no $T here.", ch,
					    NULL, chk, TO_CHAR);
		...
		if ((container = get_obj_here(ch, arg2)) == NULL) {
			act(AT_PLAIN, "I see no $T here.", ch, NULL, arg2,
			    TO_CHAR);
			return;
		}
		...
		if (number <= 1 && str_cmp(arg1, "all")
			...
			if (!obj) {
				act(AT_PLAIN,
				    ...
				    "I see nothing like that in the $T.", ch,
				    NULL, arg2, TO_CHAR);
				return;
			}
			separate_obj(obj);
...
void do_junk(CHAR_DATA * ch, char *argument)
{
	...
	for (obj = ch->first_carrying; obj; obj = obj_next) {
		obj_next = obj->next_content;
		if ((nifty_is_name(argument, obj->name)) && can_see_obj(ch, obj)
	...
	if (number <= 1 && str_cmp(arg1, "all") && str_prefix("all.", arg1)) {
		...
		for (obj = ch->first_carrying; obj; obj = obj_next) {
			...
			if ((fAll || nifty_is_name(chk, obj->name))
				...
				cnt += obj->count;
				obj_from_char(obj);
				act(AT_ACTION, "$n puts $p in $P.", ch, obj,
				    container, TO_ROOM);
				act(AT_ACTION, "You put $p in $P.", ch, obj,
				    container, TO_CHAR);
				obj = obj_to_obj(obj, container);
				found = TRUE;
...
void do_drop(CHAR_DATA * ch, char *argument)
{
	...
	if (number > 0) {
		...
		if (!str_cmp(arg, "dollars") || !str_cmp(arg, "dollar")) {
			...
			act(AT_ACTION, "$n drops some money.", ch, NULL, NULL,
			    TO_ROOM);
...
void do_give(CHAR_DATA * ch, char *argument)
{
	...
	if (is_number(arg1)) {
		...
		if (ch->gold < amount) {
			send_to_char
			    ("Very generous of you, but you haven't got that much money.\n\r",
			     ch);
			return;
		}
		...
		act(AT_ACTION, "$n gives $N some money.", ch, NULL, victim,
		    TO_NOTVICT);
		act(AT_ACTION, "You give $N some money.", ch, NULL, victim,
		    TO_CHAR);
		send_to_char("OK.\n\r", ch);
...
*/
	...
	if (victim->carry_number + (get_obj_number(obj) / obj->count) >
	    can_carry_n(victim)) {
		act(AT_PLAIN, "$N has $S hands full.", ch, NULL, victim,
		    TO_CHAR);
		return;
	}
	...
	if (victim->carry_weight + (get_obj_weight(obj) / obj->count) >
	    can_carry_w(victim)) {
		act(AT_PLAIN, "$N can't carry that much weight.", ch, NULL,
		    victim, TO_CHAR);
		return;
	}
...
bool can_dual(CHAR_DATA * ch)
{
	...
	if (get_eq_char(ch, WEAR_WIELD)) {
		if (get_eq_char(ch, WEAR_WIELD)->value[3] == 11) {
			send_to_char
			    ("You can not dual wield with a heavy weapon!\n\r",
			     ch);
			return FALSE;
		}
	}
...
void wear_obj(CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, sh_int wear_bit)
{
	...
	if (check_size) {
		...
		if (ch->race == RACE_DEFEL) {
			act(AT_MAGIC,
			    ...
			    ch, NULL, NULL, TO_CHAR);
			act(AT_ACTION, "$n wants to use $p, but doesn't.", ch,
			    obj, NULL, TO_ROOM);
			return;
		}

		if (IS_OBJ_STAT(obj, ITEM_HUTT_SIZE)) {
			act(AT_MAGIC, "That item is too big for you.", ch, NULL,
			    NULL, TO_CHAR);
		...
		if (IS_OBJ_STAT(obj, ITEM_LARGE_SIZE)
		    || IS_OBJ_STAT(obj, ITEM_HUMAN_SIZE)) {
			act(AT_MAGIC, "That item is the wrong size for you.",
			    ch, NULL, NULL, TO_CHAR);
			act(AT_ACTION, "$n tries to use $p, but can't.", ch,
			    obj, NULL, TO_ROOM);
			return;
		}

		if (IS_OBJ_STAT(obj, ITEM_SMALL_SIZE)) {
			act(AT_MAGIC, "That item is too small for you.", ch,
			    NULL, NULL, TO_CHAR);
			act(AT_ACTION,
			    "$n tries to use $p, but it is too small.", ch, obj,
			    NULL, TO_ROOM);
			return;
		}
	}
	...
	if (obj->item_type == ITEM_LIGHT) {
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n holds $p as a light.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You hold $p as your light.", ch,
				    obj, NULL, TO_CHAR);
			} else
			...
			if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
				if (!obj->action_desc
					...
					act(AT_ACTION,
					    ...
					    obj, NULL, TO_ROOM);
					act(AT_ACTION,
					    "You wear $p around your neck.", ch,
					    obj, NULL, TO_CHAR);
				} else
					actiondesc(ch, obj, NULL);
			}
	...
	case ITEM_WEAR_BODY:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n fits $p on $s body.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You fit $p on your body.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_HEAD:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n dons $p upon $s head.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You don $p upon your head.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_EYES:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n places $p on $s eyes.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You place $p on your eyes.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_EARS:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n wears $p on $s ears.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You wear $p on your ears.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_LEGS:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n slips into $p.", ch, obj,
				    NULL, TO_ROOM);
				act(AT_ACTION, "You slip into $p.", ch, obj,
				    NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_FEET:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n wears $p on $s feet.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You wear $p on your feet.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_HANDS:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n wears $p on $s hands.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You wear $p on your hands.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_ARMS:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n wears $p on $s arms.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You wear $p on your arms.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_ABOUT:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n wears $p about $s body.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You wear $p about your body.",
				    ch, obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_WAIST:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n wears $p about $s waist.",
				    ch, obj, NULL, TO_ROOM);
				act(AT_ACTION, "You wear $p about your waist.",
				    ch, obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WEAR_SHIELD:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n uses $p as a shield.", ch,
				    obj, NULL, TO_ROOM);
				act(AT_ACTION, "You use $p as a shield.", ch,
				    obj, NULL, TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_WIELD:
		...
		if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
			if (!obj->action_desc || obj->action_desc[0] == '\0') {
				act(AT_ACTION, "$n wields $p.", ch, obj, NULL,
				    TO_ROOM);
				act(AT_ACTION, "You wield $p.", ch, obj, NULL,
				    TO_CHAR);
			} else
				actiondesc(ch, obj, NULL);
		}
	...
	case ITEM_HOLD:
		...
		if (obj->item_type == ITEM_DEVICE
			...
			act(AT_ACTION, "$n holds $p in $s hands.", ch, obj,
			    NULL, TO_ROOM);
			act(AT_ACTION, "You hold $p in your hands.", ch, obj,
			    NULL, TO_CHAR);
		}
		...
		if (!get_eq_char(ch, WEAR_ANKLE_L)) {
			if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
				if (!obj->action_desc
				    || obj->action_desc[0] == '\0') {
					act(AT_ACTION,
					    "$n fits $p around $s left ankle.",
					    ch, obj, NULL, TO_ROOM);
					act(AT_ACTION,
					    "You fit $p around your left ankle.",
					    ch, obj, NULL, TO_CHAR);
				} else
					actiondesc(ch, obj, NULL);
			}
		...
		if (!get_eq_char(ch, WEAR_ANKLE_R)) {
			if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
				if (!obj->action_desc
				    || obj->action_desc[0] == '\0') {
					act(AT_ACTION,
					    "$n fits $p around $s right ankle.",
					    ch, obj, NULL, TO_ROOM);
					act(AT_ACTION,
					    "You fit $p around your right ankle.",
					    ch, obj, NULL, TO_CHAR);
				} else
					actiondesc(ch, obj, NULL);
			}
	...
	case ITEM_WEAR_BACK:
		{
			...
			if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
				if (!obj->action_desc
				    || obj->action_desc[0] == '\0') {
					act(AT_ACTION,
					    "$n slips $p on $s back.", ch, obj,
					    NULL, TO_ROOM);
					act(AT_ACTION,
					    "You slip $p on your back.", ch,
					    obj, NULL, TO_CHAR);
				} else
					actiondesc(ch, obj, NULL);
			}
	...
	case ITEM_WEAR_FACE:
		{
			...
			if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
				if (!obj->action_desc
				    || obj->action_desc[0] == '\0') {
					act(AT_ACTION,
					    "$n places $p on $s face.", ch, obj,
					    NULL, TO_ROOM);
					act(AT_ACTION,
					    "You place $p on your face.", ch,
					    obj, NULL, TO_CHAR);
				} else
					actiondesc(ch, obj, NULL);
			}
	...
	case ITEM_WEAR_HIP:
		{
			...
			if (!oprog_use_trigger(ch, obj, NULL, NULL, NULL)) {
				if (!obj->action_desc
				    || obj->action_desc[0] == '\0') {
					act(AT_ACTION, "$n place $p on $s hip.",
					    ch, obj, NULL, TO_ROOM);
					act(AT_ACTION,
					    "You place $p on your hip.", ch,
					    obj, NULL, TO_CHAR);
				} else
					actiondesc(ch, obj, NULL);
			}
...
void do_sacrifice(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0' || !str_cmp(arg, ch->name)) {
		act(AT_ACTION,
		    "$n offers $mself to $s deity, who graciously declines.",
	...
	if (!IS_NPC(ch)) {
		if (ch->pcdata->clan == NULL
		    || ch->pcdata->clan->immortal == NULL)
			sprintf(buf,
				"Spike gives you one wulong for your sacrifice.\n\r");
		else if (ch->pcdata->clan != NULL
			 && ch->pcdata->clan->immortal != NULL)
			sprintf(buf,
				"%s gives you one wulong for your sacrifice.\n\r",
				(ch->pcdata->clan->immortal !=
				 '\0') ? ch->pcdata->clan->immortal : "Spike");
		send_to_char(buf, ch);
		if (ch->pcdata->clan == NULL
		    || ch->pcdata->clan->immortal == NULL) {
			...
			act(AT_ACTION, buf, ch, obj, ch->pcdata->clan->immortal,
			    TO_ROOM);
		}
	}
...
void do_brandish(CHAR_DATA * ch, char *argument)
{
	...
	if ((sn = staff->value[3]) < 0
	    || sn >= top_sn || skill_table[sn]->spell_fun == NULL) {
	...
	if (staff->value[2] > 0) {
		if (!oprog_use_trigger(ch, staff, NULL, NULL, NULL)) {
			act(AT_MAGIC, "$n brandishes $p.", ch, staff, NULL,
			    TO_ROOM);
			act(AT_MAGIC, "You brandish $p.", ch, staff, NULL,
			    TO_CHAR);
		}
...
void do_zap(CHAR_DATA * ch, char *argument)
{
	...
	if (wand->value[2] > 0) {
		if (victim) {
			if (!oprog_use_trigger(ch, wand, victim, NULL, NULL)) {
				act(AT_MAGIC, "$n aims $p at $N.", ch, wand,
				    victim, TO_ROOM);
				act(AT_MAGIC, "You aim $p at $N.", ch, wand,
				    victim, TO_CHAR);
			}
		} else {
			if (!oprog_use_trigger(ch, wand, NULL, obj, NULL)) {
				act(AT_MAGIC, "$n aims $p at $P.", ch, wand,
				    obj, TO_ROOM);
				act(AT_MAGIC, "You aim $p at $P.", ch, wand,
				    obj, TO_CHAR);
			}
		}
	...
	if (--wand->value[2] <= 0) {
		act(AT_MAGIC, "$p explodes into fragments.", ch, wand, NULL,
		    TO_ROOM);
		act(AT_MAGIC, "$p explodes into fragments.", ch, wand, NULL,
		    TO_CHAR);
...
void do_auction(CHAR_DATA * ch, char *argument)
{
	...
	if ((time_info.hour > 19 || time_info.hour < 6)
	    && auction->item == NULL) {
		set_char_color(AT_LBLUE, ch);
		send_to_char
	...
	if (arg1[0] == '\0') {
		if (auction->item != NULL) {
			...
			if (auction->bet > 0)
				sprintf(buf,
					"Current bid on this item is %d wulongs.\n\r",
					auction->bet);
			else
				sprintf(buf,
...
/*          spell_identify (0, LEVEL_HERO - 1, ch, auction->item); */
			...
			sprintf(buf,
				...
				magic_bit_name(obj->magic_flags), obj->weight,
				obj->cost);
			set_char_color(AT_LBLUE, ch);
			send_to_char(buf, ch);
			...
			if ((obj->item_type == ITEM_CONTAINER)
				...
				show_list_to_char(obj->first_content, ch, TRUE,
						  FALSE);
			}
...
/*	    ch_printf( ch, "Bid: %d\n\r",newbet);	*/
			...
			if (newbet < (auction->bet + 10000)) {
				send_to_char
				    ("You must at least bid 10000 wulongs over the current bid.\n\r",
				     ch);
				return;
			}
			...
			if (newbet > 2000000000) {
				send_to_char
				    ("You can't bid over 2 billion wulongs.\n\r",
				     ch);
				return;
			}
			...
			sprintf(buf,
				"A bid of %d wulongs has been received on %s.\n\r",
...
/* finally... */
	...
	if (atoi(arg2) < 0) {
		send_to_char
		    ("You can't auction something for less than 0 wulongs!\n\r",
		     ch);
		return;
	}
...
*/
		...
		case ITEM_ARMOR:
			...
			sprintf(buf,
				"A new item is being auctioned: %s at %d wulongs.",
			...
			if (obj->in_room->first_person && number_percent() > 15) {
				...
				for (rch = obj->in_room->first_person; rch;
					...
					if (number_range(0, chcnt) == 0)
						vch = rch;
				act(AT_WHITE, "$p falls on $n!", vch, obj, NULL,
				    TO_ROOM);
				act(AT_WHITE, "$p falls on you!", vch, obj,
				    NULL, TO_CHAR);
