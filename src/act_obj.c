    ...
    if (arg1[0] == '\0')
    {
	    ...
	    AFFECT_DATA *paf;
  	    obj = auction->item;
            ...
            /* show item data here */
            if (auction->bet > 0)
                sprintf (buf, "Current bid on this item is %d wulongs.\n\r",auction->bet);
            else
...
/*	    ch_printf( ch, "Bid: %d\n\r",newbet);	*/
	    ...
	    {
		send_to_char("You can't bid over 2 billion wulongs.\n\r", ch);
		return;
	    }
		...
		save_char_obj(ch);
            ...
            sprintf (buf,"A bid of %d wulongs has been received on %s.\n\r",newbet,auction->item->short_descr);
            talk_auction (buf);
            return;
...
*/
	...
	sprintf (buf, "A new item is being auctioned: %s at %d wulongs.", obj->short_descr, auction->starting);
	talk_auction (buf);
