...
int get_cost(CHAR_DATA * ch, CHAR_DATA * keeper, OBJ_DATA * obj, bool fBuy)
{
	...
	if (fBuy) {
		cost =
		    (int)(cost * (80 + UMIN(ch->top_level, LEVEL_AVATAR))) /
		    100;
		...
		for (obj2 = keeper->first_carrying; obj2;
...
void do_buy(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP)) {
		...
		if (!pRoomIndexNext) {
			bug("Do_buy: bad pet shop at vnum %d.",
			    ch->in_room->vnum);
...
//      SET_BIT(ch->act, PLR_BOUGHT_PET);
		...
		if (!IS_OBJ_STAT(obj, ITEM_INVENTORY) && (noi > 1)) {
			...
			act(AT_TELL,
			    ...
			    " to sell more than one at a time.'", keeper, NULL,
			    ch, TO_VICT);
			ch->reply = keeper;
			return;
		}
		if (!IS_NPC(ch) && ch->pcdata && ch->pcdata->clan) {
			...
			if (planet && planet->governed_by
			    ...
			    && str_cmp(ch->in_room->area->name, "WTS HQ")) {
				if (planet->governed_by->war1
				    || planet->governed_by->war2) {
					if (planet->governed_by->war1 ==
					    ch->pcdata->clan
					    || planet->governed_by->war2 ==
					    ch->pcdata->clan) {
						act(AT_TELL,
						    "$n tells you 'I wouldn't be caught dead selling to your"
						    " clan!'", keeper, NULL, ch,
						    TO_VICT);
						ch->reply = keeper;
						return;
...
/*	if( !IS_NPC(ch) && planet && planet->frustrated_by && ch->pcdata
	    ...
	    && planet->frustrated_by == ch->pcdata->clan)
	{
		...
		if (noi == 1) {
			...
			act(AT_ACTION,
			    "$N puts them into a bag and hands it to you.", ch,
			    NULL, keeper, TO_CHAR);
		}

		if (!IS_NPC(ch) && ch->pcdata->learned[gsn_discount]) {
			if (number_range(0, get_curr_cha(ch) / 4) > 3) {
				act(AT_TELL,
				    "$n tells you 'I like your charisma! You get a discount!'",
				    keeper, obj, ch, TO_VICT);
		...
		if (ch->pcdata && ch->pcdata->clan
			...
			act(AT_TELL,
			    "$n tells you 'I give discounts to GLM members!'",
			    keeper, obj, ch, TO_VICT);
			cost = cost * .90;
		}
		...
		if ((planet = ch->in_room->area->planet) != NULL && clancheck) {
			if (planet->governed_by == clan
			    && planet->pop_support > 10) {
				ch_printf(ch,
					  "&B%s tells you, 'Since you're %s, you get a %.0f percent discount!'",
					  keeper->short_descr, clan->name,
					  (planet->pop_support / 10));
				send_to_char("\n\r", ch);
		...
		if (planet && clancheck && planet->governed_by) {
			if (planet->governed_by->ally1
			    || planet->governed_by->ally2) {
				if (planet->governed_by->ally1 == clan
				    || planet->governed_by->ally2 == clan) {
					ch_printf(ch,
						  "&B%s tells you, 'Since you're %s, an ally of %s you get a discount!'",
						  keeper->short_descr,
						  clan->name,
		...
		if (keeper->gold > maxgold) {
			...
			keeper->gold = maxgold / 2;
			act(AT_ACTION,
			    "$n puts some wulongs into a large safe.", keeper,
			    NULL, NULL, TO_ROOM);
		}
			...
			if (noi > 1) {
				bag =
				    create_object(get_obj_index
						  (OBJ_VNUM_SHOPPING_BAG), 1);
				/* perfect size bag ;) */
				bag->value[0] =
				    bag->weight + (buy_obj->weight * noi);
				buy_obj->count = noi;
...
void do_list(CHAR_DATA * ch, char *argument)
{
	if (IS_SET(ch->in_room->room_flags, ROOM_PET_SHOP)) {
		...
		pRoomIndexNext = get_room_index(ch->in_room->vnum + 1);
		if (!pRoomIndexNext) {
			bug("Do_list: bad pet shop at vnum %d.",
			    ch->in_room->vnum);
			send_to_char("You can't do that here.\n\r", ch);
			return;
		}
...
void do_sell(CHAR_DATA * ch, char *argument)
{
	...
	if (cost > keeper->gold) {
		act(AT_TELL, "$n makes a wulong transaction.", keeper, obj, ch,
		    TO_VICT);
	...
	sprintf(buf, "You sell $p for %d wulong%s.", cost,
		cost == 1 ? "" : "s");
...
void do_shopstat(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch,
		  "buy0 [%s]  buy1 [%s]  buy2 [%s]  buy3 [%s]  buy4 [%s]\n\r",
...
void do_shops(CHAR_DATA * ch, char *argument)
{
	...
	for (shop = first_shop; shop; shop = shop->next)
		ch_printf(ch,
			  ...
			  shop->buy_type[1], shop->buy_type[2],
...
void do_repairset(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		...
		send_to_char("\n\rField being one of:\n\r", ch);
		send_to_char
		    ("  fix0 fix1 fix2 profit type open close keeper\n\r", ch);
		return;
	}
	...
	if (!str_cmp(arg2, "keeper")) {
		...
		if (mob2->rShop) {
			send_to_char
			    ("That mobile already has a repair shop.\n\r", ch);
			return;
		}
		mob->rShop = NULL;
		mob2->rShop = repair;
...
void do_repairshops(CHAR_DATA * ch, char *argument)
{
	...
	for (repair = first_repair; repair; repair = repair->next)
		ch_printf(ch,
			  ...
			  repair->open_hour, repair->close_hour,
			  repair->fix_type[0], repair->fix_type[1],
			  repair->fix_type[2]);
	return;
}
