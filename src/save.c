...
void fwrite_char(CHAR_DATA * ch, FILE * fp)
{
	...
	fprintf(fp, "Room         %d\n",
		(ch->in_room == get_room_index(ROOM_VNUM_LIMBO)
		 && ch->was_in_room) ? ch->was_in_room->vnum : ch->in_room->
		vnum);
	if (ch->plr_home != NULL)
	...
	if (IS_NPC(ch)) {
		...
		fprintf(fp, "AttrPerm     %d %d %d %d %d %d %d\n", ch->perm_str,
			ch->perm_int, ch->perm_wis, ch->perm_dex, ch->perm_con,
		...
		fprintf(fp, "Condition    %d %d %d %d %d\n",
			...
			ch->pcdata->condition[3], ch->pcdata->condition[4]);
	...
	track =
	    URANGE(2, ((ch->top_level + 3) * MAX_KILLTRACK) / LEVEL_AVATAR,
		   MAX_KILLTRACK);
...
bool load_char_obj(DESCRIPTOR_DATA * d, char *name, bool preload)
{
	...
	if (stat(strsave, &fst) != -1) {
		if (fst.st_size == 0) {
			sprintf(strsave, "%s%c/%s", BACKUP_DIR,
...
void fread_char(CHAR_DATA * ch, FILE * fp, bool preload)
{
	...
	for (;;) {
		...
		case 'A':
			KEY("Arresttime", ch->pcdata->arresttime,
			    fread_number(fp));
			KEY("Arrestcount", ch->pcdata->arrestcount,
			    fread_number(fp));
		...
		case 'B':
			KEY("BountyDate", ch->pcdata->bountyrelease,
			    fread_number(fp));
			KEY("Bamfin", ch->pcdata->bamfin,
			    fread_string_nohash(fp));
			KEY("Bamfout", ch->pcdata->bamfout,
			    fread_string_nohash(fp));
		...
		case 'C':
			...
			if (!str_cmp(word, "Clantalk1")) {
				...
				if (!preload
				    && ch->pcdata->clantalk1_name[0] != '\0'
				    && (ch->pcdata->clantalk1 =
					get_clan(ch->pcdata->clantalk1_name)) ==
				    NULL) {
					sprintf(buf,
			...
			if (!str_cmp(word, "clantalk2")) {
				...
				if (!preload
				    && ch->pcdata->clantalk2_name[0] != '\0'
				    && (ch->pcdata->clantalk2 =
					get_clan(ch->pcdata->clantalk2_name)) ==
				    NULL) {
					sprintf(buf,
						"Warning: the organization %s no longer exists, and therefore you no longer\n\rbelong to that organization.\n\r",
						ch->pcdata->clantalk2_name);
			...
			if (!str_cmp(word, "Condition")) {
				line = fread_line(fp);
				sscanf(line, "%d %d %d %d %d", &x1, &x2, &x3,
				       &x4);
		...
		case 'H':
			KEY("Hackcount", ch->pcdata->hackcount,
			    fread_number(fp));
		...
		case 'L':
			KEY("Lastname", ch->pcdata->last_name,
			    fread_string(fp));
		...
		case 'M':
			KEY("MaxArresttime", ch->pcdata->maxarresttime,
			    fread_number(fp));
		...
		case 'O':
			KEY("Outcast_time", ch->pcdata->outcast_time,
			    fread_number(fp));
		...
		case 'S':
			...
			if (!str_cmp(word, "Setclantalk")) {
				...
				if (!preload
				    && ch->pcdata->setclantalk_name[0] != '\0'
				    && (ch->pcdata->setclantalk =
					get_clan(ch->pcdata->
						 setclantalk_name)) == NULL) {
					sprintf(buf,
						"Warning: the organization %s no longer exists, and therefore you no longer\n\rbelong to that organization.\n\r",
						ch->pcdata->setclantalk_name);
					send_to_char(buf, ch);
			...
			if (!str_cmp(word, "Skill")) {
				...
				if (preload)
					...
					if (file_ver < 3)
						sn = skill_lookup(fread_word
								  (fp));
					else
						sn = bsearch_skill_exact
						    (fread_word(fp),
						     gsn_first_skill,
						     gsn_first_weapon - 1);
					if (sn < 0)
		...
		case 'E':
			if (!str_cmp(word, "End")) {
				...
				if (lastplayed != 0) {
					...
					hitgain =
					    ((int)(current_time - lastplayed) /
					     60);
					ch->hit =
					    URANGE(1, ch->hit + hitgain,
						   ch->max_hit);
					ch->move =
					    URANGE(1, ch->move + hitgain,
						   ch->max_move);
		...
		case 'W':
			...
			KEY("Wulongs", ch->gold, fread_number(fp));
			...
			KEY("Wanted", ch->pcdata->wanted_flags,
			    fread_number(fp));
			break;
		}
	...
	for (;;) {
		...
		if (!fMatch) {
			...
			while ((paf = obj->first_affect) != NULL) {
				UNLINK(paf, obj->first_affect, obj->last_affect,
				       next, prev);
				DISPOSE(paf);
			}
			DISPOSE(obj);
