...
void load_mobiles args((AREA_DATA * tarea, FILE * fp, DATA_CONN * dc));
void load_objects args((AREA_DATA * tarea, FILE * fp));
void load_resets args((AREA_DATA * tarea, FILE * fp));
void load_rooms args((AREA_DATA * tarea, FILE * fp, DATA_CONN * dc));
...
void boot_db(bool fCopyover)
{
	 ...
	 */
	{
		...
		lhour =
		    (current_time -
		     650336715) / (PULSE_TICK / PULSE_PER_SECOND);
...
void load_helps()
{
	...
	if((conn = db_open()) == NULL) {
		bug("load_helps: could not connect to database");
		return;
	}
	
	cursor = mongo_find(conn, DSET_HELPS, NULL, NULL, 0, 0, 0);
	while(mongo_cursor_next(cursor) == MONGO_OK) {
		entry = mongo_cursor_bson(cursor);
		CREATE(pHelp, HELP_DATA, 1);
		pHelp->id = dread_id(DKEY_ID, entry);
		pHelp->level = dread_number("level", entry);
		pHelp->keyword = dread_string("keyword", entry);
		pHelp->text = dread_string("text", entry);
 ...
 * Load a mob section.
 */
void load_mobiles(AREA_DATA * tarea, FILE * fp, DATA_CONN * conn)
{
	MOB_INDEX_DATA *pMobIndex;
	char *ln;
	...
	for (;;) {
		...
		if (letter != 'S' && letter != 'C' && letter != 'Z') {
			bug("Load_mobiles: vnum %d: letter '%c' not Z, S or C.",
			    vnum, letter);
	...
	for (;;) {
		...
		case 'M':
			if (get_mob_index(arg1) == NULL && fBootDb)
				boot_log
				    ("Load_resets: %s (%d) 'M': mobile %d doesn't exist.",
				     tarea->filename, count, arg1);
			if (get_room_index(arg3) == NULL && fBootDb)
				boot_log
				    ("Load_resets: %s (%d) 'M': room %d doesn't exist.",
		...
		case 'O':
			if (get_obj_index(arg1) == NULL && fBootDb)
				boot_log
				    ("Load_resets: %s (%d) '%c': object %d doesn't exist.",
				     tarea->filename, count, letter, arg1);
			if (get_room_index(arg3) == NULL && fBootDb)
				boot_log
				    ("Load_resets: %s (%d) '%c': room %d doesn't exist.",
		...
		case 'P':
			...
			if (arg3 > 0)
				if (get_obj_index(arg3) == NULL && fBootDb)
					boot_log
					    ("Load_resets: %s (%d) 'P': destination object %d doesn't exist.",
					     tarea->filename, count, arg3);
				else if (extra > 1)
					not01 = TRUE;
			break;
		...
		case 'H':
			if (arg1 > 0)
				if (get_obj_index(arg1) == NULL && fBootDb)
					boot_log
					    ("Load_resets: %s (%d) 'H': object %d doesn't exist.",
					     tarea->filename, count, arg1);
			break;
		...
		case 'D':
			...
			if (!pRoomIndex) {
				bug("Load_resets: 'D': room %d doesn't exist.",
				    arg1);
				bug("Reset: %c %d %d %d %d", letter, extra,
				    arg1, arg2, arg3);
				if (fBootDb)
					boot_log
					    ("Load_resets: %s (%d) 'D': room %d doesn't exist.",
					     tarea->filename, count, arg1);
				break;
			}
			...
			if (arg2 < 0
				...
				if (fBootDb)
					boot_log
					    ("Load_resets: %s (%d) 'D': exit %d not door.",
		...
		case 'R':
			...
			if (!pRoomIndex && fBootDb)
				boot_log
				    ("Load_resets: %s (%d) 'R': room %d doesn't exist.",
				     tarea->filename, count, arg1);
 ...
 * Load a room section.
 */
void load_rooms(AREA_DATA * tarea, FILE * fp, DATA_CONN * dc)
{
	DATA_ENTRY * entry;
	ROOM_INDEX_DATA *pRoomIndex;
	char buf[MAX_STRING_LENGTH];
	...
	for (;;) {
		...
		ln = fread_line(fp);
		x1 = x2 = x3 = x4 = x5 = x6 = x7 = 0;
		sscanf(ln, "%d %d %d %d %d %d %d", &x1, &x2, &x3, &x4, &x5, &x6,
		       &x7);
		...
		for (;;) {
			...
			if (letter == 'D') {
				...
				if (door < 0 || door > 10) {
					...
					pexit =
					    make_exit(pRoomIndex, NULL, door);
					pexit->description = fread_string(fp);
					pexit->keyword = fread_string(fp);
					pexit->exit_info = 0;
					ln = fread_line(fp);
					x1 = x2 = x3 = x4 = 0;
					sscanf(ln, "%d %d %d %d", &x1, &x2, &x3,
					       &x4);
		...
		if(fBootDb && (entry = dfind_number(DKEY_VNUM, vnum, DSET_ROOMS, dc)) != NULL) {
			pRoomIndex->id = dread_id(DKEY_ID, entry);
	...
	for (wiz = first_wiz; wiz; wiz = wiz->next) {
		if (wiz->level > LEVEL_AVATAR) {
			if (wiz->level < ilevel) {
				...
				switch (ilevel) {
				case MAX_LEVEL - 0:
					towizfile("&C Project Lead&W");
					break;
				case MAX_LEVEL - 1:
					towizfile("&R Lead Programmer&W");
					break;
				case MAX_LEVEL - 2:
					towizfile("&p Programmers&W");
					break;
				case MAX_LEVEL - 3:
					towizfile("&Y Lead Builder&W");
					break;
				case MAX_LEVEL - 4:
					towizfile("&c Lead Roleplay&W");
					break;
				case MAX_LEVEL - 5:
					towizfile("&B Building Specialist&W");
					break;
				case MAX_LEVEL - 6:
					towizfile("&p Senior Builders&W");
					break;
				case MAX_LEVEL - 7:
					towizfile("&g Advanced Builders&W");
					break;
				case MAX_LEVEL - 8:
					towizfile("&r Junior Builders&W");
					break;
				case MAX_LEVEL - 9:
					towizfile("&P Player Testers&W");
					break;
				case MAX_LEVEL - 10:
					towizfile("&R Advanced RP&W");
					break;
				case MAX_LEVEL - 11:
					towizfile("&C RP Assistants&W");
					break;
				case MAX_LEVEL - 12:
					towizfile("&G Trainees&W");
					break;
				case MAX_LEVEL - 13:
					towizfile("&B Janitors&W");
					break;
				default:
					towizfile(" &z Retired&W");
					break;
				}
			}
...
void load_mudprogs(AREA_DATA * tarea, FILE * fp)
{
	...
	for (;;)
		...
		case 'm':
			...
			working =
			    mprog_file_read(fread_word(fp), working, iMob);
			working->next = NULL;
			fread_to_eol(fp);
			break;
		}
...
void load_objprogs(AREA_DATA * tarea, FILE * fp)
{
	...
	for (;;)
		...
		case 'm':
			...
			working =
			    oprog_file_read(fread_word(fp), working, iObj);
			working->next = NULL;
			fread_to_eol(fp);
			break;
		}
...
void oprog_read_programs(FILE * fp, OBJ_INDEX_DATA * pObjIndex)
{
	...
	while (!done) {
		...
		case IN_FILE_PROG:
			mprg =
			    oprog_file_read(fread_string(fp), mprg, pObjIndex);
			fread_to_eol(fp);
			switch (letter = fread_letter(fp)) {
			case '>':
				CREATE(mprg->next, MPROG_DATA, 1);
...
void load_roomprogs(AREA_DATA * tarea, FILE * fp)
{
	...
	for (;;)
		...
		case 'm':
			...
			working =
			    rprog_file_read(fread_word(fp), working, iRoom);
			working->next = NULL;
			fread_to_eol(fp);
			break;
		}
...
void rprog_read_programs(FILE * fp, ROOM_INDEX_DATA * pRoomIndex)
{
	...
	while (!done) {
		...
		case IN_FILE_PROG:
			mprg =
			    rprog_file_read(fread_string(fp), mprg, pRoomIndex);
			fread_to_eol(fp);
			switch (letter = fread_letter(fp)) {
			case '>':
				CREATE(mprg->next, MPROG_DATA, 1);
...
OBJ_INDEX_DATA *make_object(int vnum, int cvnum, char *name)
{
	...
	if (!cObjIndex) {
		...
		pObjIndex->extra_flags =
		    cObjIndex->extra_flags | ITEM_PROTOTYPE;
...
/*    FILE *fpin;
	...
	if((dc = dopen()) == NULL) {
		bug("load_area_file: could not connect to database", 0);
		return;
	}
	...
	if(fBootDb && tarea && (entry = dfind_string("name", tarea->name, DSET_AREAS, dc)) != NULL) {
		tarea->id = dread_id(DKEY_ID, entry);
		tarea->name = dread_string("name", entry);
		tarea->author = dread_string("author", entry);
		tarea->flags = dread_number("flags", entry);
		tarea->high_economy = dread_number("highEconomy", entry);
		tarea->low_economy = dread_number("lowEconomy", entry);
		area->resetmsg = dread_string("resetMsg", entry);
		area->reset_frequency = dread_number("resetFrequency", entry);
	}
	...
	while (dentry) {
		if (dentry->d_name[0] != '.') {
			...
			if (rlow && rhi && !badfile) {
				...
				pArea->name = fread_string_nohash(fp);
#else
				sprintf(buf, "{PROTO} %s's area in progress",
					dentry->d_name);
...
void fread_sysdata(SYSTEM_DATA * sys, FILE * fp)
{
	...
	for (;;) {
		...
		case 'G':
			KEY("Guildoverseer", sys->guild_overseer,
			    fread_string(fp));
			KEY("Guildadvisor", sys->guild_advisor,
			    fread_string(fp));
			break;
...
mongo * db_open()
{
	...
	{
		bug("dopen: could not connect to database");
		return NULL;	
	}
...
int db_close(mongo * conn)
{
	if(!conn)
	{
		bug("dclose: connection is null");
		return 0;
	}
...
//	log_string("Database connection opened");
	return (DATA_CONN*)conn;
}

int dclose(DATA_CONN *conn)
{
	if(!conn)
	{
		bug("dclose: connection is null");
	...
	mongo_destroy(conn);
	DISPOSE(conn);
//	log_string("Database connection closed");
	return 1;
}

DATA_CURSOR * dcursor_open(char * setName, DATA_CONN * conn)
{
	return (DATA_CURSOR*)mongo_find(conn, setName, NULL, NULL, 1000, 0, 0);
}

int dcursor_close(DATA_CURSOR *cursor)
{
	if(!cursor)
	{
		return 0;
	...
	mongo_cursor_destroy((mongo_cursor*)cursor);
	DISPOSE(cursor);
	return 1;
}

int dcursor_next(DATA_CURSOR * dc)
{
	int status = mongo_cursor_next((mongo_cursor*)dc);
	if(status == MONGO_OK)
	{
		return 1;
	}
...
DATA_ENTRY * dcursor_entry(DATA_CURSOR * dc)
{
	return mongo_cursor_bson((mongo_cursor*)dc);
}

char * dread_id(char * field, DATA_ENTRY * e)
{
	bson_iterator iterator[1];
	bson_oid_t *oid;
	char * buf;
	bson_iterator_init(iterator, (bson*)e);
	if(bson_find(iterator, (bson*)e, field)) 
	{
...
char dread_letter(char *field, DATA_ENTRY * e)
{
	char *r;
	if(!e)
	{
		bug("dread_letter: entry is null");
		return '\0';
	}

	r = dread_string(field, e);
	if(r != NULL)
	{
		return r[0];
	}
...
int dread_number(char *field, DATA_ENTRY * e)
{
	if(!e)
	{
		bug("dread_number: entry is null");
		return 0;
	}
	...
	if(bson_find(iterator, (bson*)e, field)) 
	{
		int i = bson_iterator_int(iterator);
		return i;
	}
...
char * dread_string(char * field, DATA_ENTRY * e)
{
	if(!e)
	{
		bug("dread_string: entry is null");
		return NULL;
	}
	...
	if(bson_find(iterator, (bson*)e, field)) 
	{
		char * s = STRALLOC((char*)bson_iterator_string(iterator));
		return s;
	}
...
char * dread_word(char * field, DATA_ENTRY * e)
{
	if(!e)
	{
		bug("dread_word: entry is null");
		return NULL;
	}

	return dread_string(field, e);
}

char * dread_line(char * field, DATA_ENTRY * e)
{
	if(!e)
	{
		bug("dread_line: entry is null");
		return NULL;
	}
...
DATA_ENTRY * dread_entry(char * field, DATA_ENTRY * e)
{
	bson_iterator iterator[1];
	bson *so = NULL;
	if(bson_find(iterator, (bson*)e, field)) 
	{
		bson_iterator_subobject(iterator, so);
		return (DATA_ENTRY*)so;
	}

	return NULL;
}

int dwrite_entry(char * field, DATA_ENTRY * value, DATA_ENTRY * e)
{
	bson *v = (bson*)value;
	bson *b = (bson*)e;
	bson_iterator iterator[1];
	bson_iterator_init(iterator, b);
	bson_append_element(b, field, iterator);
	return 1;
}

int dwrite_letter(char * field, char value, DATA_ENTRY * e)
{
	char buf[2];
	buf[0] = value;
	buf[1] = '\0';
	return dwrite_string(field, buf, e);
}

int dwrite_number(char * field, int value, DATA_ENTRY * e)
{
	bson *b = (bson*)e;
	bson_append_int(b, field, value);
	return 1;
}

int dwrite_string(char * field, char * value, DATA_ENTRY * e)
{
	bson *b = (bson*)e;
	bson_append_string(b, field, value);
	return 1;
}

int dwrite_id(char * field, char * id, DATA_ENTRY * e)
{
	bson_oid_t oid[1];
	bson_oid_from_string(oid, id);
	bson_append_oid((bson*)e, field, oid);
	return 1;
}

int dwrite_word(char * field, char * value, DATA_ENTRY * e)
{
	return dwrite_string(field, value, e);
}

int dwrite_line(char * field, char * value, DATA_ENTRY * e)
{
	return dwrite_string(field, value, e);
}

int dwrite_push_object(char * field, DATA_ENTRY * e)
{
	bson_append_start_object(e, field);
	return 1;
}

int dwrite_pop_object(DATA_ENTRY * e)
{
	bson_append_finish_object(e);
	return 1;
}

int dwrite_push_array(char * field, DATA_ENTRY * e)
{
	bson_append_start_array(e, field);
	return 1;
}

int dwrite_pop_array(DATA_ENTRY * e)
{
	bson_append_finish_array(e);
	return 1;
}

int dwrite_array_number(int index, int value, DATA_ENTRY * e)
{
	char buf[10];
	if(index < 0) {
		bug("dwrite_array_number: index less than 0", 0);
		return 0;
	}

	sprintf(buf, "%d", index);
	return dwrite_number(buf, value, e);
}

int dwrite_array_string(int index, char * value, DATA_ENTRY * e)
{
	char buf[10];
	if(index < 0) {
		bug("dwrite_array_string: index less than 0", 0);
		return 0;
	}

	sprintf(buf, "%d", index);
	return dwrite_string(buf, value, e);
}

int dwrite_array_entry(int index, DATA_ENTRY * value, DATA_ENTRY * e)
{
	char buf[10];
	if(index < 0) {
		bug("dwrite_array_entry: index less than 0", 0);
		return 0;
	}

	sprintf(buf, "%d", index);
	return dwrite_entry(buf, value, e);
}

int dwrite_array_push_object(int index, DATA_ENTRY * e)
{
	char buf[10];
	if(index < 0) {
		bug("dwrite_array_push_object: index less than 0", 0);
		return 0;
	}

	sprintf(buf, "%d", index);
	return dwrite_push_object(buf, e);
}

DATA_ENTRY * dfind_by_id(char * id, char * setName, DATA_CONN * dc)
{
	return dfind_string(DKEY_ID, id, setName, dc);
}

DATA_ENTRY * dfind_number(char * key, int value, char * setName, DATA_CONN * dc)
{
	DATA_ENTRY * result = NULL;
	bson query[1];
	bool found = FALSE;
	CREATE(result, DATA_ENTRY, 1);
	bson_init(query);
	bson_append_int(query, key, value);
	bson_finish(query);
	found = mongo_find_one(dc, setName, query, NULL, result);
	bson_destroy(query);
	if(found != MONGO_OK)
		DISPOSE(result);
...
DATA_ENTRY * dfind_string(char * key, char * value, char * setName, DATA_CONN * dc)
{
	DATA_ENTRY * result;
	bson query[1];
	bool found = FALSE;
	CREATE(result, DATA_ENTRY, 1);
	bson_init(query);
	bson_append_string(query, key, value);
	bson_finish(query);
...
int dentry_init(DATA_ENTRY * entry)
{
	bson *b = (bson*)entry;
	bson_init(b);
	return 1;
...
int dentry_finalize(DATA_ENTRY * entry)
{
	bson_finish((bson *)entry);
	return 1;
}

int dentry_destroy(DATA_ENTRY * entry)
{
	if(!entry)
	{
		bug("dentry_destroy: entry is null");
		return 0;
	}
...
int dentry_save(DATA_ENTRY * e, char *setName, DATA_CONN * conn)
{
	char buf[MAX_STRING_LENGTH];
	bson *b = (bson*)e;
	mongo * c = (mongo*)conn;
	//sprintf(buf, "writing to database: %s", setName);
	//log_string(buf);
	//bson_print_raw(b, 10);
	mongo_insert(c, setName, b, NULL);
	return 1;
}
