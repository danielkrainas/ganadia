...
void do_mptransfer(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg1, "all")) {
		for (victim = ch->in_room->first_person; victim;
			...
			if (victim != ch && !NOT_AUTHED(victim)
			    && can_see(ch, victim)) {
...
void do_mpforce(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "all")) {
		...
		if (!IS_NPC(victim) && (!victim->desc) && IS_IMMORTAL(victim)) {
			progbug
			    ("Mpforce - Attempting to force link dead immortal",
			     ch);
			return;
		}
...
void do_mpapplyb(CHAR_DATA * ch, char *argument)
{
	...
	case 3:
		send_to_char
		    ("The gods permit you to enter the Star Wars Reality.\n\r",
		     victim);
...
ch_ret simple_damage(CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
	...
	victim->hit -= dam;
	if (!IS_NPC(victim)
	    && get_trust(victim) >= LEVEL_IMMORTAL && victim->hit < 1)
		victim->hit = 1;
...
*/
	...
	if (clan && vclan && (clan->war1 || clan->war2)) {
...
*/
	...
	if (!str_cmp(ship->owner, victim->name)
	    ...
	    || !str_cmp(ship->copilot, victim->name)) {
		do_emote(ch,
			 "hammers on a missle head as $e finishes reloading the ship's weapons.");
