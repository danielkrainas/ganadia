	...
	if ((fp = fopen(filename, "r")) != NULL) {
		...
		for (;;) {
			...
			if (!str_cmp(word, "SLAY")) {
				...
				sprintf(buf, "Load_slay_file: bad section: %s.",
					word);
				bug(buf, 0);
				continue;
			}
...
void do_slay(CHAR_DATA * ch, char *argument)
{
	...
	if (type[0] == '\0') {
		act(AT_IMMORT, "You brutally slay $N!", ch, NULL, victim,
		    TO_CHAR);
		act(AT_IMMORT, "$n chops you up into little pieces!", ch, NULL,
		    victim, TO_VICT);
		act(AT_IMMORT, "$n brutally slays $N!", ch, NULL, victim,
		    TO_NOTVICT);
		set_cur_char(victim);
		raw_kill(ch, victim);
		return;
		...
		for (slay = first_slay; slay; slay = slay->next) {
			if ((!str_cmp(type, slay->type)
			     && !str_cmp("Any", slay->owner))
			    || (!str_cmp(slay->owner, ch->name)
				&& !str_cmp(type, slay->type))
...
void do_destroyslay(CHAR_DATA * ch, char *argument)
{
	...
	for (pslay = first_slay; pslay; pslay = pslay->next) {
		if (!str_cmp(arg, pslay->type)) {
			...
			STRFREE(pslay->rmsg);
			ch_printf(ch,
				  "Slaytype \"%s\" has beed deleted. Be sure to save the slay table.\n\r",
				  arg);
			return;
		}
	}
