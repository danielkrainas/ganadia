...
void do_bank( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
       send_to_char( "&CUsage&R: &WBANK &R<&Wdeposit&R|&Wwithdraw&R|&Wbalance&R|&Wtransfer&R> &C[&Wamount&C]&W &C[&Wreceiver&C]&W\n\r", ch );
			...
			switch(number_range(0,8))
			{
				...
				case 4:
					if(ch->alignment < 300)
					{
						...
						act(AT_YELLOW, "$n begins screaming obcenities at the confusing book.", ch, NULL, NULL, TO_ROOM);
					}
					else
					{
						act(AT_YELLOW, "You break down in tears our of frustration and confusion.", ch, NULL, NULL, TO_CHAR);
...
void do_newendurance( CHAR_DATA *ch, char *argument)
{
	switch(ch->substate)
	{
		default:
			...
			if(ch->hit < 150)
			{
				send_to_char("&RYou are fair to injuryed to train your Endurance!\r\n", ch);
				return;
			}
			...
			switch(number_range(0,3))
			{
				...
				break;
				case 2:
					act(AT_GREEN, "You stretch slightly and start to do fast pace running on a treadmil.",
						ch, NULL, NULL, TO_CHAR);
					act(AT_GREEN, "$n stretches and steps onto a treadmil, starting to do some running.",
						ch, NULL, NULL, TO_ROOM);
					time = 3;
				break;
				case 1:
...
TO_ROOM);
		...
		case 3:
			...
			if(ch->stimer < 3)
			{
				act(AT_GREEN, "You stop the treadmil and step off, resting for a momemnt.",
					ch, NULL, NULL, TO_CHAR);
				act(AT_GREEN, "$n stops the treadmil and steps off, resting for a moment.", ch, NULL, NULL, TO_ROOM);
				return;
			}
			// Messages go here!
			switch(number_range(0,5))
			{
				case 5:
					...
					act(AT_YELLOW, "$n almost trips on the treadmil!", ch, NULL, NULL, TO_ROOM);
				break;
				case 4:
					if(ch->hit > 500)
					{
...
TO_CHAR);
				...
				case 2:
					...
					act(AT_YELLOW, "$n grimaces slightly as they run on the treadmil.", ch, NULL, NULL, TO_ROOM);
				break;
				case 1:
					if(ch->alignment < 0)
					{
					...
					else
					{
						act(AT_YELLOW, "You try to take your mind off the pain by thinking of something else.",
							ch, NULL, NULL, TO_CHAR);
						act(AT_YELLOW, "$n looks disconnected as $e runs on the treadmil.", ch, NULL, NULL, TO_ROOM);
					}
				break;
				default:
					act(AT_YELLOW, "You wince in pain as you feel a blister forming on your foot.", ch, NULL, NULL, TO_CHAR);
					act(AT_YELLOW, "$n winces in pain as $e runs on the treadmil.", ch, NULL, NULL, TO_ROOM);
				break;
			}
			ch->contrain += number_range(1, get_curr_lck(ch)/5);
			ch->contrain += number_range(0,2);
			...
			if(ch->hit < 150)
			{
				act(AT_GREEN, "You fall of the treadmil, with your legs too tired to move.", ch, NULL, NULL, TO_CHAR);
				act(AT_GREEN, "$n falls hard of the treadmil.", ch, NULL, NULL, TO_ROOM);
				return;
			}
			ch->stimer -= 3;
			...
			switch(ch->stimer)
			{
					...
					else
					{
						switch(number_range(0,3))
						{
							...
							default:
								...
								else if(str_cmp(ch->in_room->area->planet->governed_by->name, "WTS"))
								{
									if(victim->alignment > 300)
										sprintf(buf, "WTS is an odd mix, I dunno if I can really say anything.");
									else
										sprintf(buf, "WTS needs to stop with the 'White Tiger' crap and kill more!");
								}
								ch->stimer = 7;
							break;
						}
				...
				case 2:
					...
					else
					{
						...
						{
							case 2:
								sprintf(buf, "WTS of course! Gotta love the Tigers!");
								ch->stimer = 9;
							break;
							case 1:
		...
		case 3:
			...
			switch(ch->stimer)
			{
				...
				case 9:
					...
					else if(ch->pcdata->clan && str_cmp(ch->pcdata->clan->name, "WTS"))
...
void do_gainage(CHAR_DATA *ch, char *argument)
{
	...
	send_to_char("100 because you may have over-trained in a category. As you get closer to losing a stat\r\n", ch);
	send_to_char("the percentage will slowly approach 0, at 0 you will be losing stats in that area.\r\n", ch);
