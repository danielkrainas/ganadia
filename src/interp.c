...
void interpret(CHAR_DATA * ch, char *argument)
{
	...
	if ((!IS_NPC(ch) && IS_SET(ch->act, PLR_LOG))
	    || fLogAll
	    || loglvl == LOG_BUILD || loglvl == LOG_HIGH
	    || loglvl == LOG_ALWAYS) {
...
/*	if ( ch->desc && ch->desc->original )
	...
	if (tmptime > 1500000) {
		sprintf(log_buf, "[*****] LAG: %s: %s %s (R:%d S:%d.%06d)",
			ch->name, cmd->name,
		...
		if (!IS_NPC(ch) && IS_NPC(victim)
			...
			case 0:
				if (!IS_SET(ch->in_room->room_flags, ROOM_SAFE)
					...
					act(AT_ACTION, "You slap $N.", victim,
					    NULL, ch, TO_CHAR);
					act(AT_ACTION, "$n slaps you.", victim,
					    NULL, ch, TO_VICT);
				} else {
					act(AT_ACTION,
			...
			case 8:
				act(AT_SOCIAL, social->others_found,
				    victim, NULL, ch, TO_NOTVICT);
				act(AT_SOCIAL, social->char_found, victim, NULL,
				    ch, TO_CHAR);
				act(AT_SOCIAL, social->vict_found, victim, NULL,
				    ch, TO_VICT);
				break;
			...
			case 12:
				act(AT_ACTION, "$n slaps $N.", victim, NULL, ch,
				    TO_NOTVICT);
				act(AT_ACTION, "You slap $N.", victim, NULL, ch,
				    TO_CHAR);
