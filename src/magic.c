...
bool saves_spell_staff(int level, CHAR_DATA * victim)
{
	...
	if (IS_NPC(victim) && level > 10)
		level -= 5;
	save =
	    50 + (victim->top_level - level - victim->saving_spell_staff) * 2;
	save = URANGE(5, save, 95);
	return chance(victim, save);
}
	...
	while (comp[0] != '\0') {
		...
		if (obj) {
			if (val >= 0 && val < 6) {
				...
				obj->count = 1;
				act(AT_MAGIC, "$p glows briefly.", ch, obj,
				    NULL, TO_CHAR);
				obj->count = count;
			}
		}
...
void do_cast(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		add_timer(ch, TIMER_DO_FUN, UMIN(skill->beats / 10, 3), do_cast,
			  1);
		act(AT_MAGIC,
	...
	case 1:
		...
		if (skill->participants > 1) {
			...
			for (tmp = ch->in_room->first_person; tmp;
				...
				if (tmp != ch
				    && (t =
					get_timerptr(tmp, TIMER_DO_FUN)) != NULL
	       ...
	       OBJ_DATA * obj)
{
	...
	if ((skill->target == TAR_CHAR_OFFENSIVE || number_bits(7) == 1)	/* 1/128 chance if non-offensive */
		...
		case 0:
			...
			break;
		case 1:
			act(AT_MAGIC, "The $t backfires!", ch, skill->name,
			    victim, TO_CHAR);
			if (victim)
				act(AT_MAGIC, "$n's $t backfires!", ch,
				    skill->name, victim, TO_VICT);
			act(AT_MAGIC, "$n's $t backfires!", ch, skill->name,
			    victim, TO_NOTVICT);
...
ch_ret spell_charm_person(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (IS_AFFECTED(victim, AFF_CHARM)
	    || chance == 1000 || IS_AFFECTED(ch, AFF_CHARM)
...
ch_ret spell_dispel_evil(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (IS_GOOD(victim)) {
		act(AT_MAGIC, "The light side protects $N.", ch, NULL, victim,
		    TO_ROOM);
		return rSPELL_FAILED;
	}
...
ch_ret spell_enchant_weapon(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (obj->item_type != ITEM_WEAPON
	    || IS_OBJ_STAT(obj, ITEM_MAGIC) || obj->first_affect)
		return rSPELL_FAILED;
...
ch_ret spell_faerie_fire(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	affect_to_char(victim, &af);
	act(AT_PINK, "You are surrounded by a pink outline.", victim, NULL,
	    NULL, TO_CHAR);
...
/* Modifications on 1/2/96 to work on player/object - Scryn */
	...
	if (victim) {
		...
		if (obj) {
			...
			act(AT_MAGIC, "$p fades out of existence.", ch, obj,
			    NULL, TO_CHAR);
			return rNONE;
		}
	}
...
ch_ret spell_sleep(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (!IS_NPC(victim)) {
		sprintf(log_buf, "%s has cast sleep on %s.", ch->name,
			victim->name);
		log_string_plus(log_buf, LOG_NORMAL, ch->top_level);
		to_channel(log_buf, CHANNEL_MONITOR, "Monitor",
			   UMAX(LEVEL_IMMORTAL, ch->top_level));
	}

	if (IS_AWAKE(victim)) {
		act(AT_MAGIC, "You feel very sleepy ..... zzzzzz.", victim,
		    NULL, NULL, TO_CHAR);
		...
		for (obj_lose = victim->first_carrying; obj_lose;
			...
			case ITEM_ARMOR:
				if (obj_lose->value[0] > 0) {
					...
					act(AT_DAMAGE,
					    "$p is pitted and etched!", victim,
					    obj_lose, NULL, TO_CHAR);
					if ((iWear =
					     obj_lose->wear_loc) != WEAR_NONE)
						victim->armor -=
						    apply_ac(obj_lose, iWear);
					obj_lose->value[0] -= 1;
					obj_lose->cost = 0;
					if (iWear != WEAR_NONE)
						victim->armor +=
...
ch_ret spell_recharge(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (obj->item_type == ITEM_STAFF || obj->item_type == ITEM_WAND) {
		...
		if (obj->value[2] == obj->value[1]
			...
			act(AT_FIRE, "$p bursts into flames, injuring you!", ch,
			    obj, NULL, TO_CHAR);
			act(AT_FIRE, "$p bursts into flames, charring $n!", ch,
			    obj, NULL, TO_ROOM);
			extract_obj(obj);
			if (damage(ch, ch, obj->level * 2, TYPE_UNDEFINED) ==
		...
		if (chance(ch, 2)) {
			...
			act(AT_WHITE, "$p disintegrates into a void.", ch, obj,
			    NULL, TO_CHAR);
			act(AT_WHITE,
			...
			act(AT_MAGIC, "$p feels warm to the touch.", ch, obj,
			    NULL, TO_CHAR);
			--obj->value[1];
			obj->value[2] = obj->value[1];
			return rNONE;
...
ch_ret spell_remove_invis(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (obj) {
		...
		act(AT_MAGIC, "$p becomes visible again.", ch, obj, NULL,
		    TO_CHAR);
		...
		if (victim) {
			...
			if (!IS_NPC(victim)) {
				...
				if (chance(ch, 50)
				    && ch->skill_level[FORCE_ABILITY] + 15 <
				    victim->top_level) {
					failed_casting(skill, ch, victim, NULL);
					return rSPELL_FAILED;
				}
...
ch_ret spell_animate_dead(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (!IS_NPC(ch)) {
		if (ch->mana - (pMobIndex->level * 4) < 0) {
			send_to_char
			    ("You do not have enough mana to reanimate this "
			     "corpse.\n\r", ch);
			return rSPELL_FAILED;
		} else
	...
	if (IS_IMMORTAL(ch)
	    || (chance(ch, 75) && pMobIndex->level - ch->top_level < 10)) {
		...
		if (!pMobIndex->hitnodice)
			mob->max_hit =
			    pMobIndex->level * 8 +
			    number_range(pMobIndex->level * pMobIndex->level /
					 4,
					 pMobIndex->level * pMobIndex->level);
		else
			mob->max_hit =
			    dice(pMobIndex->hitnodice, pMobIndex->hitsizedice)
			    + pMobIndex->hitplus;
		mob->max_hit = UMAX(URANGE(mob->max_hit / 4,
					   (mob->max_hit * corpse->value[3]) /
					   100, ch->top_level * dice(20, 10)),
				    1);
		...
		sprintf(buf, "The animated corpse of %s",
...
ch_ret spell_possess(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (IS_AFFECTED(victim, AFF_POSSESS)
	    || level < (victim->top_level + 30) || victim->desc
	    || !chance(ch, 25)) {
		failed_casting(skill, ch, victim, NULL);
		return rSPELL_FAILED;
	}
	...
	af.type = sn;
	af.duration =
	    20 + (ch->skill_level[FORCE_ABILITY] - victim->top_level) / 2;
	af.location = 0;
	af.modifier = 0;
	...
	for (saf = skill->affects; saf; saf = saf->next) {
		...
		if (af.duration == 0) {
			...
			case APPLY_MANA:
				...
				victim->mana =
				    URANGE(0, victim->mana + af.modifier,
					   victim->max_mana);
				update_pos(victim);
				break;
			case APPLY_MOVE:
				...
				victim->move =
				    URANGE(0, victim->move + af.modifier,
					   victim->max_move);
				update_pos(victim);
				break;
			default:
	...
	if (!groupsp && !areasp) {
		...
		if (skill->hit_char && skill->hit_char[0] != '\0') {
			if (strstr(skill->hit_char, "$N"))
				...
				act(AT_MAGIC, skill->hit_char, ch, NULL, NULL,
				    TO_CHAR);
		}
		if (skill->hit_room && skill->hit_room[0] != '\0') {
			if (strstr(skill->hit_room, "$N"))
				...
				act(AT_MAGIC, skill->hit_room, ch, NULL, NULL,
				    TO_ROOM);
		}
	...
	for (; victim; victim = victim->next_in_room) {
		if (groupsp || areasp) {
			...
			if (hitvict && ch != victim) {
				act(AT_MAGIC, skill->hit_vict, ch, NULL, victim,
				    TO_VICT);
				if (hitroom) {
					act(AT_MAGIC, skill->hit_room, ch, NULL,
					    victim, TO_NOTVICT);
					act(AT_MAGIC, skill->hit_room, ch, NULL,
					    victim, TO_CHAR);
				}
			...
			if (ch == victim) {
				if (hitvict)
					act(AT_MAGIC, skill->hit_vict, ch, NULL,
					    ch, TO_CHAR);
				else if (hitchar)
					act(AT_MAGIC, skill->hit_char, ch, NULL,
					    ch, TO_CHAR);
			} else if (hitchar)
				act(AT_MAGIC, skill->hit_char, ch, NULL, victim,
				    TO_CHAR);
		}
		retcode = spell_affectchar(sn, level, ch, victim);
		if (!groupsp && !areasp) {
...
ch_ret spell_obj_inv(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	case SA_CREATE:
		...
		case SP_NONE:
			if (obj->cost >
			    ch->skill_level[FORCE_ABILITY] * get_curr_int(ch) *
			    get_curr_wis(ch)) {
				failed_casting(skill, ch, NULL, obj);
				return rNONE;
			}
		...
		case SP_GREATER:
			if (ch->skill_level[FORCE_ABILITY] - obj->level < 5
			    || obj->cost >
			    ch->skill_level[FORCE_ABILITY] * 10 *
		...
		case SP_MAJOR:
			if (ch->skill_level[FORCE_ABILITY] - obj->level < 0
			    || obj->cost >
			    ch->skill_level[FORCE_ABILITY] * 50 *
			    get_curr_int(ch) * get_curr_wis(ch)) {
				failed_casting(skill, ch, NULL, obj);
				return rNONE;
			}
		...
		if (vo && SPELL_ACTION(skill) == SA_DESTROY) {
			...
			if (SPELL_DAMAGE(skill) == SD_POISON) {
				if (is_affected(victim, gsn_poison)) {
					affect_strip(victim, gsn_poison);
					victim->mental_state =
					    URANGE(-100, victim->mental_state,
						   -10);
...
ch_ret spell_spectral_furor(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (saves_spell_staff(level, victim))
		dam /= 2;
	act(AT_MAGIC, "The fabric of the cosmos strains in fury about $N!", ch,
...
ch_ret spell_mind_wrack(int sn, int level, CHAR_DATA * ch, void *vo)
{
	...
	if (saves_spell_staff(level, victim))
		dam /= 2;
	act(AT_MAGIC,
	    "$n stares intently at $N, causing $N to seem very lethargic.", ch,
	    NULL, victim, TO_NOTVICT);
	return damage(ch, victim, dam, sn);
}
