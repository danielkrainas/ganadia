...
int make_color_sequence_desc(const char *col, char *buf, DESCRIPTOR_DATA * d)
{
	...
	if (!*col)
		...
		bug("Make_color_sequence: command '%c' not '&' or '^'.",
		    *ctype);
		ln = -1;
	} else if (*col == *ctype) {
		buf[0] = *col;
		...
		switch (*ctype) {
		default:
			bug("Make_color_sequence: bad command char '%c'.",
			    *ctype);
			ln = -1;
			break;
		case '&':
...
void explode(OBJ_DATA * obj)
{
	if (obj->armed_by) {
		...
		for (xch = first_char; xch; xch = xch->next)
			if (!IS_NPC(xch)
			    && nifty_is_name(obj->armed_by, xch->name)) {
				if (obj->carried_by) {
					act(AT_WHITE,
				...
				if (room) {
					...
					if (!held && room->first_person)
						act(AT_WHITE, "$p EXPLODES!",
						    room->first_person, obj,
						    NULL, TO_ROOM);
...
void
room_explode_1(OBJ_DATA * obj, CHAR_DATA * xch, ROOM_INDEX_DATA * room,
	       int blast)
{
	CHAR_DATA *rch;
	CHAR_DATA *rnext;
	...
	for (robj = room->first_content; robj; robj = robj_next) {
		...
		if (robj != obj && robj->item_type != ITEM_SPACECRAFT
		...
		for (pexit = room->first_exit; pexit; pexit = pexit->next) {
			if (pexit->to_room && pexit->to_room != room) {
				if (blast > 0) {
					...
					roomblast = blast - 1;
					room_explode_1(obj, xch, pexit->to_room,
						       roomblast);
				} else
					echo_to_room(AT_WHITE, pexit->to_room,
	...
	case APPLY_FULL:
		if (!IS_NPC(ch))
			ch->pcdata->condition[COND_FULL] =
			    URANGE(0, ch->pcdata->condition[COND_FULL] + mod,
				   48);
		break;
	...
	case APPLY_THIRST:
		if (!IS_NPC(ch))
			ch->pcdata->condition[COND_THIRST] =
			    URANGE(0, ch->pcdata->condition[COND_THIRST] + mod,
				   48);
		break;
	...
	case APPLY_DRUNK:
		if (!IS_NPC(ch))
			ch->pcdata->condition[COND_DRUNK] =
			    URANGE(0, ch->pcdata->condition[COND_DRUNK] + mod,
				   48);
		break;
...
/* skill apply types	-Thoric */
	...
	if (!IS_NPC(ch)
		...
		if (depth == 0) {
			depth++;
			act(AT_ACTION,
			    "You are too weak to wield $p any longer.", ch,
			    wield, NULL, TO_CHAR);
			act(AT_ACTION, "$n stops wielding $p.", ch, wield, NULL,
			    TO_ROOM);
...
void affect_join(CHAR_DATA * ch, AFFECT_DATA * paf)
{
	...
	for (paf_old = ch->first_affect; paf_old; paf_old = paf_old->next)
		if (paf_old->type == paf->type) {
			paf->duration =
			    UMIN(1000000, paf->duration + paf_old->duration);
			if (paf->modifier)
				paf->modifier =
				    UMIN(5000,
...
void equip_char(CHAR_DATA * ch, OBJ_DATA * obj, int iWear)
{
	...
	if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch))
		 ...
		 */
		if (loading_char != ch) {
			act(AT_MAGIC, "You are zapped by $p and drop it.", ch,
			    obj, NULL, TO_CHAR);
			act(AT_MAGIC, "$n is zapped by $p and drops it.", ch,
			    obj, NULL, TO_ROOM);
		}
		if (obj->carried_by)
			obj_from_char(obj);
...
void extract_obj(OBJ_DATA * obj)
{
	...
	if (obj_extracted(obj)) {
		bug("extract_obj: obj %d already extracted!",
		    obj->pIndexData->vnum);
		return;
	}
...
void extract_char(CHAR_DATA * ch, bool fPull, bool capture)
{
	...
	if (!ch->in_room) {
		bug("Extract_char: %s in NULL room.",
		    ch->name ? ch->name : "???");
		return;
	}
...
//          location = get_room_index( ch->pcdata->clan->recall );
		...
		if (!IS_NPC(ch) && bounty && bounty->amount > 0 && capture) {
			location = get_room_index(ROOM_PRISON);
			ch->pcdata->bountyrelease +=
			    (bounty->amount / 625000) + 2;
			...
			ch->pcdata->bountyrelease +=
			    (ch->pcdata->bountyrelease * 2);
			if (ch->pcdata->bountyrelease > 80)
				ch->pcdata->bountyrelease = 80;
		...
		if ((wch = get_char_room(ch, "healer")) != NULL) {
			...
			act(AT_MAGIC,
			    "$n appears from some strange swirling mists!", ch,
			    NULL, NULL, TO_ROOM);
			sprintf(buf,
			...
			act(AT_MAGIC,
			    "$n is rushed into the emergency room and bandaged up.",
...
OBJ_DATA *find_obj(CHAR_DATA * ch, char *argument, bool carryonly)
{
	...
	if (arg2[0] == '\0') {
		if (carryonly && (obj = get_obj_carry(ch, arg1)) == NULL) {
			...
			act(AT_PLAIN, "I see no $T here.", ch, NULL, arg1,
			    TO_CHAR);
			return NULL;
		}
		return obj;
		...
		if (!obj)
			act(AT_PLAIN, IS_OBJ_STAT(container, ITEM_COVERING) ?
			    ...
			    "I see nothing like that in $p.", ch, container,
			    NULL, TO_CHAR);
		return obj;
	}
	return NULL;
...
bool room_is_private(CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
	...
	if (ch->plr_home != pRoomIndex) {	/* Check for War or Bounty - Funf */
		if (ch->pcdata && ch->pcdata->clan
		    && IS_SET(pRoomIndex->room_flags, ROOM_PLR_HOME)) {
			/*bug( "Bounty/War check entered\r\n" ); */
			for (rch = pRoomIndex->first_person; rch;
				...
				if (rch->pcdata && rch->pcdata->clan) {
					...
					if (!strcmp
					    (ch->pcdata->clan->name, "RBH")
					    || !strcmp(ch->pcdata->clan->name,
						       "ISSP")) {
						if (is_disintigration(rch)) {	/* Check for bounty */
	...
	if (IS_SET(pRoomIndex->room_flags, ROOM_PLR_HOME)
	    ...
	    && IS_SET(pRoomIndex->room_flags2, ROOM_VISITORS)) {
		for (rch = pRoomIndex->first_person; rch;
		     rch = rch->next_in_room) {
		...
		return TRUE;
	} else if (IS_SET(pRoomIndex->room_flags, ROOM_PLR_HOME)
		   && ch->plr_home != pRoomIndex
		   && !IS_SET(pRoomIndex->room_flags2, ROOM_VISITORS))
		return TRUE;
...
char *affect_bit_name(int vector)
{
	...
	if (vector & AFF_SCRYING)
		strcat(buf, " scrying");
	if (vector & AFF_FRENZY)
		strcat(buf, " frenzy");
	if (vector & AFF_SHOCKSHIELD)
		strcat(buf, " shockshield");
	if (vector & AFF_BLOODYEYE)
		strcat(buf, " bloody_eye");
	if (vector & AFF_POSSESS)
		strcat(buf, " possess");
...
ch_ret spring_trap(CHAR_DATA * ch, OBJ_DATA * obj)
{
	...
	case TRAP_TYPE_SLEEPING_GAS:
		retcode =
		    obj_cast_spell(skill_lookup("sleep"), lev, ch, ch, NULL);
		break;
	case TRAP_TYPE_ACID_SPRAY:
		retcode =
		    obj_cast_spell(skill_lookup("acid blast"), lev, ch, ch,
				   NULL);
		break;
	case TRAP_TYPE_SEX_CHANGE:
		retcode =
		    obj_cast_spell(skill_lookup("change sex"), lev, ch, ch,
				   NULL);
		break;
	case TRAP_TYPE_FLAME:
	case TRAP_TYPE_EXPLOSION:
...
ch_ret check_room_for_traps(CHAR_DATA * ch, int flag)
{
	...
	for (check = ch->in_room->first_content; check;
	     check = check->next_content) {
		if (check->item_type == ITEM_LANDMINE
		    && flag == TRAP_ENTER_ROOM) {
			explode(check);
			return rNONE;
 ...
 */
void
add_timer(CHAR_DATA * ch, sh_int type, sh_int count, DO_FUN * fun, int value)
{
	TIMER *timer;
...
bool in_soft_range(CHAR_DATA * ch, AREA_DATA * tarea)
{
	if (IS_IMMORTAL(ch))
		...
		return TRUE;
	else if (ch->top_level >= tarea->low_soft_range
		 || ch->top_level <= tarea->hi_soft_range)
		return TRUE;
	else
		return FALSE;
}
...
bool in_hard_range(CHAR_DATA * ch, AREA_DATA * tarea)
{
	if (IS_IMMORTAL(ch))
		...
		return TRUE;
	else if (ch->top_level >= tarea->low_hard_range
		 && ch->top_level <= tarea->hi_hard_range)
		return TRUE;
	else
		return FALSE;
}
...
*/
	    ...
	    && obj1->timer == obj2->timer && obj1->value[0] == obj2->value[0]
...
void split_obj(OBJ_DATA * obj, int num)
{
	...
	if (obj->carried_by) {
		LINK(rest, obj->carried_by->first_carrying,
		     obj->carried_by->last_carrying, next_content,
		     prev_content);
		rest->carried_by = obj->carried_by;
		rest->in_room = NULL;
		rest->in_obj = NULL;
	} else if (obj->in_room) {
		LINK(rest, obj->in_room->first_content,
		     obj->in_room->last_content, next_content, prev_content);
		rest->carried_by = NULL;
		rest->in_room = obj->in_room;
		rest->in_obj = NULL;
	} else if (obj->in_obj) {
		LINK(rest, obj->in_obj->first_content,
		     obj->in_obj->last_content, next_content, prev_content);
