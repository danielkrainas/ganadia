	...
	if ((fp = fopen(filename, "r")) != NULL) {
		...
		for (;;) {
			...
			if (!str_cmp(word, "RESTORE")) {
				...
				restorecount++;
				LINK(restore, first_restore, last_restore, next,
				     prev);
				continue;
	...
	if (!str_cmp(who, "all")) {
		...
		for (vch = first_char; vch; vch = vch_next) {
			...
			if (!IS_NPC(vch) && !IS_IMMORTAL(vch)) {
				if (type[0] == '\0') {
					act(AT_IMMORT,
					    "You feel a surge or energy course through you as the Gods Restore you.",
					    ch, NULL, vch, TO_VICT);
					vch->hit = vch->max_hit;
					vch->mana = vch->max_mana;
					vch->move = vch->max_move;
					vch->pcdata->condition[COND_BLEEDING] =
					    0;
					update_pos(vch);
					found = TRUE;
				} else {
		...
		if (type[0] == '\0') {
			act(AT_IMMORT, "You restore $N!", ch, NULL, victim,
			    TO_CHAR);
			act(AT_IMMORT,
			    "You feel a surge of energy course through you as the Gods Restore you.",
