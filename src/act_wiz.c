...
void do_setlevels(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_IMMORTAL(ch)) {
		sprintf(buf, "WARNING: MORTAL %s ATTEMPTING TO SET LEVELS.",
			ch->name);
		log_string(buf);
		send_to_char
		    ("YOU CAN'T DO THAT!! The immortals have been notified.\n\r",
		     ch);
		return;
	}
...
/*    		ch_printf( ch, "%s is at level %d.\n\r", cmd->name, cmd->level );*/
		...
		ch_printf(ch, "You restrict %s to level %d\n\r", cmd->name,
			  level);
		sprintf(buf, "%s restricting %s to level %d", ch->name,
			cmd->name, level);
		log_string(buf);
	} else
...
void do_rpauthorize(CHAR_DATA * ch, char *argument)
{
	...
	if (arg2[0] == '\0' || !str_cmp(arg2, "yes")) {
		if (!IS_SET(victim->pcdata->flags, PCFLAG_RPAUTH)) {
			...
			ch_printf(ch, "You removed %s from being able to RP!",
				  victim->name);
			ch_printf(victim,
...
void do_mstat(CHAR_DATA * ch, char *argument)
{
	...
	if (get_trust(ch) >= LEVEL_GOD && !IS_NPC(victim) && victim->desc)
		ch_printf(ch,
			  "User: %s@%s   Descriptor: %d   Trust: %d   AuthedBy: %s\n\r",
	...
	ch_printf(ch,
		  ...
		  IS_NPC(victim) ? victim->pIndexData->killed : victim->pcdata->
		  mdeaths + victim->pcdata->pdeaths);
	...
	ch_printf(ch, "Hps: %d/%d  Force: %d/%d   Move: %d/%d   PC Age: %d\n\r",
		  ...
		  victim->move, victim->max_move, victim->pcage);
	if (!IS_NPC(victim)) {
		int ability;
	...
	if (IS_NPC(victim)) {
		...
		ch_printf(ch,
			  "MentalState: %d   EmotionalState: %d    Jail Time: %d\n\r",
			  victim->mental_state, victim->emotional_state,
			  victim->pcdata->bountyrelease);
	}
	...
	ch_printf(ch,
		  ...
		  victim->numattacks);
	ch_printf(ch,
		  "Hours: %d   Seconds Played: %d   Timer: %d   Act: %d\n\r",
	...
	if (IS_NPC(victim)) {
		...
		ch_printf(ch, "Wanted flags: %s\n\r",
			  flag_string(victim->pcdata->wanted_flags,
				      planet_flags));
	}
...
void do_qpset(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "Ok, %s now has %d quest points.", victim->name,
		victim->pcdata->quest_curr);
	send_to_char(buf, ch);
	do_save(victim, "");
	if (arg3 > 0) {
		sprintf(buf, "&w&WThe Immortals have blessed you with %d QP!",
			arg3);
		send_to_char(buf, victim);
		act(AT_MAGIC,
		    "&w&W$n flashes in a bright light because of the Immortals Blessing!",
...
void do_purge(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		act(AT_IMMORT, "You have purged the room!", ch, NULL, NULL,
		    TO_CHAR);
		return;
	}
	victim = NULL;
...
/* Single object purge in room for high level purge - Scryn 8/12*/
	if (obj) {
		...
		act(AT_IMMORT, "You make $p disappear in a puff of smoke!", ch,
		    obj, NULL, TO_CHAR);
		extract_obj(obj);
		return;
	}
...
*/
	...
	if (!remove(buf))
		...
		ch_printf(ch,
			  "Unknown error #%d - %s (immortal data).  Report to Jet or Spike\n\r",
	...
	for (pArea = first_build; pArea; pArea = pArea->next)
		if (!strcmp(pArea->filename, buf2)) {
			...
			if (!rename(buf, buf2))
				...
				ch_printf(ch,
					  "Unknown error #%d - %s (area data).  Report to Jet or Spike.\n\r",
					  errno, strerror(errno));
				sprintf(buf2, "%s destroying %s", ch->name,
					buf);
				perror(buf2);
			}
		}
...
void do_new_silence(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->pcdata->unsilence_date != 0) {
		ch_printf(ch,
			  "They are already in silence until %24.24s, by %s.\n\r",
	...
	if (time <= 0) {
		send_to_char
		    ("You cannot silence for zero or negative time.\n\r", ch);
		return;
	}
	argument = one_argument(argument, arg);
	if (!*arg || !str_prefix(arg, "hours"))
		...
		send_to_char
		    ("You may not silence a person for more than 30 days at a time.\n\r",
		     ch);
		return;
	}
	...
	ch_printf(ch, "%s will be released from silence at %24.24s.\n\r",
		  victim->name, ctime(&victim->pcdata->unsilence_date));
	act(AT_MAGIC, "$n disappears in a cloud of silenceish light.", victim,
	    NULL, ch, TO_NOTVICT);
	SET_BIT(victim->act, PLR_SILENCE);
	act(AT_MAGIC, "$n appears in a could of silenceish light.", victim,
	    NULL, ch, TO_NOTVICT);
	ch_printf(victim,
...
void do_new_unsilence(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char
	    ("The gods have smiled on you and released you from silence early!\n\r",
	     victim);
...
void do_ban(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		send_to_pager
		    ("---- ---- ------------------------ ---------------\n\r",
		     ch);
		for (pban = first_ban, bnum = 1; pban;
		     pban = pban->next, bnum++)
	...
	if (!str_cmp(arg, "help")) {
		...
		send_to_char
		    ("Syntax: ban <ban number> <level <lev>|newban|mortal|"
		     "total>\n\r", ch);
		return;
	}
...
void close_area(AREA_DATA * pArea)
{
	...
	for (icnt = 0; icnt < MAX_KEY_HASH; icnt++) {
		...
		for (mid = mob_index_hash[icnt]; mid; mid = mid_next) {
			...
			if (mid->pShop) {
				UNLINK(mid->pShop, first_shop, last_shop, next,
				       prev);
				DISPOSE(mid->pShop);
			}
			if (mid->rShop) {
...
void do_destroy(CHAR_DATA * ch, char *argument)
{
	...
	if (!victim) {
		...
		if (victim->plr_home != NULL) {
			...
			sprintf(buf3, "%s%c/%s.home", PLAYER_DIR,
				tolower(victim->name[0]),
...
void do_unhell(CHAR_DATA * ch, char *argument)
{
	...
	if (!location)
		location = ch->in_room;
	MOBtrigger = FALSE;
	act(AT_MAGIC, "$n disappears in a cloud of godly light.", victim, NULL,
	    ch, TO_NOTVICT);
	char_from_room(victim);
	char_to_room(victim, location);
	send_to_char
...
void do_ipcompare(CHAR_DATA * ch, char *argument)
{
	...
	if (arg[0] == '\0') {
		...
		IPCOMPARE_DATA *first_ip = NULL, *last_ip =
		    NULL, *hmm, *hmm_next;
	...
	if (arg1[0] != '\0') {
		...
		if (arg2[0] != '\0') {
			if (is_number(arg2))
				...
				send_to_char
				    ("Please see help ipcompare for more info.\n\r",
				     ch);
				return;
			}
		}
...
void do_mortality(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_IMMORTAL(ch)) {
		...
		act(AT_PLAIN, "$n coalesces into a mortal avatar.", ch, NULL,
		    NULL, TO_ROOM);
		act(AT_PLAIN, "You coalesce into a mortal avatar.", ch, NULL,
		    NULL, TO_CHAR);

		if (!str_cmp(argument, "broadcast")
		    || !str_cmp(arg, "broadcast")) {
			sprintf(buf,
				"%s has taken on a Mortal avatar within the realms.",
				ch->name);
			echo_to_all(AT_IMMORT, buf, ECHOTAR_ALL);
		}
		return;
	...
	if (!IS_IMMORTAL(ch) && ch->tmplevel > 100) {
		...
		ch->tmptrust = 0;
		save_char_obj(ch);
		act(AT_PLAIN, "$n returns to a immortal state.", ch, NULL, NULL,
		    TO_ROOM);
		act(AT_PLAIN, "You return to your immortal state.", ch, NULL,
		    NULL, TO_CHAR);
		if (!str_cmp(argument, "broadcast")
		    || !str_cmp(arg, "broadcast")) {
			sprintf(buf, "%s ascends into the heavens once more.",
				ch->name);
			echo_to_all(AT_IMMORT, buf, ECHOTAR_ALL);
		}
		return;
...
void do_invade(CHAR_DATA * ch, char *argument)
{
	...
	for (created = 0; created < count; created++) {
		...
		victim = create_mobile(pMobIndex);
		char_to_room(victim, location);
		act(AT_IMMORT, "$N appears as part of an invasion force!", ch,
		    NULL, victim, TO_ROOM);
	}
...
void do_oinvade(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch,
		  "&YAt your bidding %d level %d %s appear. (&W#%d &Y- &W%s&Y)\n\r",
		  created, level, pObjIndex->short_descr, pObjIndex->vnum,
		  pObjIndex->name);
...
void do_newpassword(CHAR_DATA * ch, char *argument)
{
	...
	if (victim == '\0') {
		ch_printf(ch,
			  "%s isn't here, they have to be here to reset passwords.\n\r",
			  arg1);
		return;
	}
	...
	for (p = pwdnew; *p != '\0'; p++) {
		if (*p == '~') {
			send_to_char
			    ("New password not acceptable, (can't use ~) try again.\n\r",
			     ch);
			return;
		}
	}
...
void do_pcrename(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->pcdata->area) {
		...
		sprintf(newfilename, "%s%s.are.bak", BUILD_DIR,
			capitalize(arg2));
		rename(filename, newfilename);
	}
	...
	remove(backname);
	if (remove(oldname)) {
		sprintf(buf, "Error: Couldn't delete file %s in do_rename.",
			oldname);
