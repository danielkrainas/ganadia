		 ...
		 OBJ_DATA * obj, void *vo, CHAR_DATA * rndm)
{
	...
	*p++ = '\0';
	...
	for (;;) {
		...
		default:
			progbug
			    ("Ifcheck Syntax warning (missing right bracket)",
			     mob);
			goto doneargs;
			break;
		}
	...
	*p = '\0';
	...
	if (cvar[0] == '$') {
		...
		case 'p':
			...
			break;
		default:
			sprintf(rval, "Bad argument '%c' to '%s'", cvar[0],
				chck);
			progbug(rval, mob);
			return BERR;
		}
	...
	if (!str_cmp(chck, "economy")) {
		...
		return
		    mprog_veval(((room->area->high_economy >
				  0) ? 1000000000 : 0)
	...
	if (chkchar) {
		...
		if (!str_cmp(chck, "numfighting")) {
			return mprog_veval(chkchar->num_fighting - 1, opr,
					   atoi(rval), mob);
		}
		if (!str_cmp(chck, "hitprcnt")) {
			return mprog_veval(chkchar->hit / chkchar->max_hit, opr,
					   atoi(rval), mob);
		}
		...
		if (!str_cmp(chck, "norecall")) {
			return IS_SET(chkchar->in_room->room_flags,
		...
		if (!str_cmp(chck, "doingquest")) {
			return IS_NPC(chkchar) ? FALSE
			    : mprog_veval(chkchar->pcdata->quest_number, opr,
					  atoi(rval), mob);
		}
		if (!str_cmp(chck, "ishelled")) {
			return IS_NPC(chkchar) ? FALSE
			    : mprog_veval(chkchar->pcdata->release_date, opr,
					  atoi(rval), mob);
		}

		if (!str_cmp(chck, "level")) {
			return mprog_veval(get_trust(chkchar), opr, atoi(rval),
					   mob);
		}
		...
		if (!str_cmp(chck, "weight")) {
			return mprog_veval(chkchar->carry_weight, opr,
					   atoi(rval), mob);
		}
		...
		if (!str_cmp(chck, "clan")) {
			if (IS_NPC(chkchar) || !chkchar->pcdata->clan)
				return FALSE;
			return mprog_seval(chkchar->pcdata->clan->name, opr,
					   rval, mob);
		}
		...
		if (!str_cmp(chck, "wearing")) {
			...
			for (obj = chkchar->first_carrying; obj;
...
*/
		...
		if (!str_cmp(chck, "bounty")) {
			bounty = get_disintigration(chkchar->name);
			if (!bounty)
				return mprog_veval(0, opr, atoi(rval), mob);
			else
				return mprog_veval(bounty->amount, opr,
						   atoi(rval), mob);
		}
		if (!str_cmp(chck, "hps")) {
	...
	if (!str_cmp(chck, "number")) {
		if (chkchar) {
			...
			lhsvl =
			    (chkchar ==
			     mob) ? chkchar->gold : chkchar->pIndexData->vnum;
	...
	if (!str_cmp(chck, "rank")) {	/* Shaddai */
		if (chkchar && !IS_NPC(chkchar))
			return mprog_seval(chkchar->pcdata->rank, opr, rval,
					   mob);
		return FALSE;
	}
		...
		if (!str_prefix("mpsleep", cmnd)) {
			...
			if (mpsleep->mob->pIndexData->vnum == 3) {
				if (!str_prefix
				    ...
				    if (!str_prefix
					("Object", mpsleep->mob->description))
					mpsleep->type = MP_OBJ;
			} else
				mpsleep->type = MP_MOB;
	...
	while (mpsleep) {	/* Find progs to continue */
		if (--mpsleep->timer <= 0) {
			...
			mprog_driver(mpsleep->com_list, mpsleep->mob,
				     mpsleep->actor, mpsleep->obj, mpsleep->vo,
				     mpsleep->single_step);
 ...
 */
void
mprog_act_trigger(char *buf, CHAR_DATA * mob, CHAR_DATA * ch,
		  ...
		  OBJ_DATA * targ, void *vo)
{
	...
	if (obj->pIndexData->progtypes & USE_PROG) {
		...
		if (obj->item_type == ITEM_STAFF) {
			if (vict)
				executed =
				    oprog_percent_check(supermob, ch, obj, vict,
							USE_PROG);
			else
				executed =
				    oprog_percent_check(supermob, ch, obj, targ,
							USE_PROG);
		} else {
			executed =
...
void
oprog_act_trigger(char *buf, OBJ_DATA * mobj, CHAR_DATA * ch,
		  OBJ_DATA * obj, void *vo)
{
...
void
oprog_wordlist_check(char *arg, CHAR_DATA * mob, CHAR_DATA * actor,
		     OBJ_DATA * obj, void *vo, int type, OBJ_DATA * iobj)
{
...
void
rprog_act_trigger(char *buf, ROOM_INDEX_DATA * room, CHAR_DATA * ch,
		  OBJ_DATA * obj, void *vo)
{
...
void
rprog_wordlist_check(char *arg, CHAR_DATA * mob, CHAR_DATA * actor,
