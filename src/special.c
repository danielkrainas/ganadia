					    ...
					    UMIN(obj->cost * 10,
						 (exp_level
						  ...
						  exp_level(victim->
							    ...
							    [DIPLOMACY_ABILITY])));
					ch_printf(victim,
						  "You lose %ld diplomacy experience.\n\r ",
						  ch_exp);
			...
			if (IS_SET(ch->vip_flags, 1 << vip)
				...
				if (ch->top_level >= victim->top_level)
					...
					case 1:
						jail =
						    get_room_index
						    (ROOM_JAIL_MON_CALAMARI_1);
						break;
					case 2:
						jail =
						    get_room_index
						    (ROOM_JAIL_MON_CALAMARI_2);
						break;
					case 3:
						jail =
						    get_room_index
				...
				if (jail) {
					REMOVE_BIT(victim->pcdata->wanted_flags,
						   1 << vip);
					act(AT_ACTION,
					    "$n ushers $N off to jail.", ch,
					    NULL, victim, TO_NOTVICT);
					act(AT_ACTION,
					    "$n escorts you to jail.", ch, NULL,
					    victim, TO_VICT);
					char_from_room(victim);
...
bool spec_thief(CHAR_DATA * ch)
{
	...
	for (victim = ch->in_room->first_person; victim; victim = v_next) {
		...
		if (IS_AWAKE(victim) && number_range(0, ch->top_level) == 0) {
			act(AT_ACTION,
			    "You discover $n's hands in your wallet!", ch, NULL,
			    victim, TO_VICT);
