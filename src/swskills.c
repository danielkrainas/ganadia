	...
	if(found)
	{
	    if (!bootup)
	      ...
	      return;
        	...
        	if ( number_percent() - get_curr_int(ch) > ch->pcdata->learned[gsn_hackwho] || number_range(0,5) > 4 )
    		{
        	        send_to_char( "Your attempt to hack is discovered!\n\r", ch );
...
*/
   ...
   ch_printf( victim2 , "&R1#22@3 &Ctransfers &Y%ld &Cwulongs to your account.&W\n\r" , amount);
...
void do_hackbounty(CHAR_DATA *ch, char *argument )
{
   ...
   if ( IS_SET( ch->in_room->room_flags, ROOM_SAFE ) ||
    ...
    {
	send_to_char("&RHackbounty 'add/remove' 'name' # (Number for Add only)\n\r", ch);
	return;
    }
   ...
   if ( !str_prefix( arg1 , "add" ) )
   {
	if( amount < 5000)
	{
		send_to_char("&RYou need to put atleast 5,000 wulongs on their head!\n\r", ch);
		return;
	}
	bounty = get_disintigration( victim->name );
	if(bounty && (bounty->amount + amount > 500000000))
	{
		send_to_char("&ROUT OF BOUNDS ERROR: Line 2231, contact ISSP to correct error.\r\n", ch);
		return;
	}
	if(bounty && bounty->amount < 100000000)
	{
		send_to_char("&RThis would look a bit obvious, wouldn't it?\n\r", ch);
		return;
	}
...
!checkmotherboard )  || ( !checkharddrive ) || (!checkcase))
    {
       ...
       send_to_char("&RA foul smell and rich black smoke come from the worthless machine.\n\r", ch);
       learn_from_failure( ch, gsn_makelaptop );
       return;
    }
...
*/
   ...
   {
	victim->pcdata->bank -= amount;
	ch_printf( ch, "You were able to hack %ld wulongs from %s's acount!\n\r", amount, victim->name);
        gain_exp(ch, (ch->top_level/2+60), FORCE_ABILITY);
        ch_printf( ch , "You gain %d hacking experience.\n\r", (ch->top_level/2+60) );
	learn_from_success( ch, gsn_hackbank);
	return;
...
void do_group_heal(CHAR_DATA *ch, char *argument)
{
        ...
        switch( ch->substate )
        {
    			...
    			{
            			if ( is_same_group( gch, ch ) && gch != ch && gch->perm_frc <= 0 && gch->hit < 999)
            			{
					group++;
				}
			}
...
void do_appeal(CHAR_DATA *ch, char *argument)
{
	...
	ch_printf(ch, "%s has donated %ld to %s!", victim->name, amount, clan->name);
	clan->funds += amount;
	victim->gold -= amount;
