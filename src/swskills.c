...
void do_makeknife(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (is_profane(arg)) {
			send_to_char
			    ("Your name has been tested and found to be profain in some way!\r\n",
			     ch);
			send_to_char
			    ("If this was not profain, and the filter made a mistake send a personal gnote to imms!\r\n",
			     ch);
			return;
		}
		checktool = FALSE;
		...
		if (strlen_color(arg) > 75) {
			send_to_char
			    ("&RYou can't make items with names longer than 75 characters!\r\n",
			     ch);
			return;
		}
		...
		if (!checktool) {
			send_to_char
			    ("&RYou need a toolkit to make a knife.\n\r", ch);
			return;
		}
		...
		if (!checkbatt) {
			send_to_char
			    ("&RYou need a battery to power the oven.\n\r", ch);
			return;
		}
	...
	if (number_percent() > chance * 2 || (!checktool) || (!checkdura)
	    || (!checkplas) || (!checkoven)) {
		send_to_char
		    ("&RYou finish putting together your newly created knife.\n\r",
		     ch);
		send_to_char
		    ("&RIt looks okay for a moment but then breaks apart into a dozen pieces.\n\r",
		     ch);
		learn_from_failure(ch, gsn_makeknife);
		return;
	}
	...
	obj->value[1] = (int)(level / 50);	/* min dmg  */
	obj->value[2] = (int)(level / 10 + 2);	/* max dmg */
...
void do_makegun(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (!checkdura) {
			send_to_char
			    ("&RYou need some steel to make it out of.\n\r",
			     ch);
			return;
		}
		...
		if (!checkoven) {
			send_to_char
			    ("&RYou need a small furnace to heat the materials.\n\r",
			     ch);
			return;
		}
		...
		if (!checkmetal) {
			send_to_char
			    ("&RYou need a rare metal for those fine components.\n\r",
			     ch);
			return;
		}
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of making a gun.\n\r",
			     ch);
			act(AT_PLAIN,
			    "$n takes $s tools and a small oven and begins to work on something.",
			    ch, NULL, argument, TO_ROOM);
	...
	if (number_percent() > chance * 2 || (!checktool) || (!checkdura)
	    || (!checkbatt) || (!checkoven) || (!checkplas) || (!checkmetal)) {
		send_to_char
		    ("&RYou hold up your new gun and aim at a leftover piece of plastic.\n\r",
		     ch);
		send_to_char
		    ("&RYou slowly squeeze the trigger hoping for the best...\n\r",
		     ch);
		send_to_char
		    ("&RYour gun backfires destroying your weapon and burning your hand.\n\r",
		     ch);
		learn_from_failure(ch, gsn_makegun);
		return;
	}
	...
	obj->value[1] = (int)(level / 50 + 4);	/* min dmg  */
	obj->value[2] = (int)(level / 10 + 8);	/* max dmg  */
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created gun.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new gun.", ch, NULL, argument,
	    TO_ROOM);
...
void do_makesword(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (!checkoven) {
			send_to_char
			    ("&RYou need a small oven to heat and shape the components.\n\r",
			     ch);
			return;
		}
		...
		if (!checkplas) {
			send_to_char
			    ("&RYou need some plastic for the hilt.\n\r", ch);
			return;
		}
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of crafting a sword.\n\r",
			     ch);
			act(AT_PLAIN,
			    "$n takes $s tools and a small oven and begins to work on something.",
			    ch, NULL, argument, TO_ROOM);
	...
	if (number_percent() > chance * 2 || (!checktool) || (!checkdura)
	    || (!checkbatt) || (!checkoven) || (!checkplas))
	{
		send_to_char
		    ("&RYou finish putting together your newly created sword.\n\r",
		     ch);
		send_to_char
		    ("&RIt looks okay for a moment but then breaks apart into a dozen pieces.\n\r",
		     ch);
	...
	obj->description = STRALLOC(buf);
	obj->value[0] = INIT_WEAPON_CONDITION;	/* condition  */
	obj->value[1] = (int)(level / 25);	/* min dmg  */
	obj->value[2] = (int)(level / 25 + 10);	/* max dmg */
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created sword.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new sword.", ch, NULL, argument,
	    TO_ROOM);
...
void do_makebloodyeye(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (!IS_SET(ch->in_room->room_flags, ROOM_REFINERY)) {
			send_to_char
			    ("&RYou need to be in a refinery to create drugs.\n\r",
			     ch);
			return;
		}
		if (strlen_color(arg) > 75) {
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of creating the drug Bloody Eye.\n\r",
			     ch);
			act(AT_PLAIN, "$n begins working on something.", ch,
			    NULL, argument, TO_ROOM);
	...
	obj->cost += obj->value[1] * 100000;
	{
		...
		xpgain =
		    UMIN(obj->cost * 50,
			 (exp_level(ch->skill_level[CHEMIST_ABILITY] + 1) -
...
void do_makeyelloweye(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of creating the drug Yellow Eye.\n\r",
			     ch);
			act(AT_PLAIN, "$n begins working on something.", ch,
			    NULL, argument, TO_ROOM);
...
void do_makepurpleeye(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of creating the drug Purple Eye.\n\r",
			     ch);
			act(AT_PLAIN, "$n begins working on something.", ch,
			    NULL, argument, TO_ROOM);
...
void do_makeblueeye(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of creating the drug Blue Eye.\n\r",
			     ch);
			act(AT_PLAIN, "$n begins working on something.", ch,
			    NULL, argument, TO_ROOM);
...
void do_makegreeneye(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of creating the drug Green Eye.\n\r",
			     ch);
			act(AT_PLAIN, "$n begins working on something.", ch,
			    NULL, argument, TO_ROOM);
...
void do_makeapplicator(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (!checktool) {
			send_to_char
			    ("&RYou need toolkit to make a applicator.\n\r",
			     ch);
			return;
		}
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of making an applicator.\n\r",
			     ch);
			act(AT_PLAIN,
			    "$n takes $s tools and a small oven and begins to work on something.",
			    ch, NULL, argument, TO_ROOM);
			add_timer(ch, TIMER_DO_FUN, timer, do_makeapplicator,
				  1);
			ch->dest_buf = str_dup(arg);
			return;
		}
	...
	level =
	    IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->
					       learned[gsn_makeapplicator]);
	vnum = 4391;

	if ((pObjIndex = get_obj_index(vnum)) == NULL) {
		send_to_char
		    ("&RThe item you are trying to create is missing from the database. Please inform the administratiors.\n\r",
		     ch);
		return;
	}
	...
	if (number_percent() > chance * 2 || (!checktool) || (!checkdura)
	    || (!checkbatt) || (!checkoven) || (!checkplas) || (!checkmetal)) {
		send_to_char
		    ("&RYou hold up your new applicator and put it to your eye.\n\r",
		     ch);
		send_to_char
		    ("&RYou slowly squeeze the button and hope it works...\n\r",
		     ch);
		send_to_char
		    ("&RYour applicator explodes and pieces of metal get in your eyes.\n\r",
		     ch);
		learn_from_failure(ch, gsn_makeapplicator);
		return;
	}
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created applicator.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new applicator.", ch, NULL,
	    argument, TO_ROOM);

	{
		if (ch->skill_level[CHEMIST_ABILITY] < 50) {
			...
			xpgain =
			    UMIN(obj->cost * 50,
				 (exp_level
				  (ch->skill_level[CHEMIST_ABILITY] + 1) -
				  exp_level(ch->skill_level[CHEMIST_ABILITY])));
			xpgain = xpgain / 3;
			gain_exp(ch, xpgain, CHEMIST_ABILITY);
			ch_printf(ch, "You gain %d chemist experience.",
				  xpgain);
		} else
			send_to_char
			    ("Your level is too high to gain experience from this skill.\n\r",
			     ch);
	}
...
void do_makerawdrug(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (!checktool) {
			send_to_char
			    ("&RYou need toolkit to make a raw drug.\n\r", ch);
			return;
		}
		...
		if (!checkoven) {
			send_to_char
			    ("&RYou need a oven to heat the materials.\n\r",
			     ch);
			return;
		}
		...
		if (!checkchem) {
			send_to_char
			    ("&RYou need chemicals to make raw drugs!\n\r", ch);
			return;
		}
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of making a raw drug.\n\r",
			     ch);
			act(AT_PLAIN,
		...
		send_to_char
		    ("&RYou can't figure out how to mix the drug correctly.\n\r",
		     ch);
	...
	send_to_char
	    ("&GYou finish your work and gaze at your new raw drug.&w\n\r", ch);
	act(AT_PLAIN, "$n finishes making $s new raw drug.", ch, NULL, argument,
	    TO_ROOM);

	{
		...
		if (ch->skill_level[CHEMIST_ABILITY] < 50) {
			...
			send_to_char
			    ("Your level is too high to gain experience for this skill.\n\r",
			     ch);
	}
...
void do_makegrenade(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created grenade.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new grenade.", ch, NULL, argument,
	    TO_ROOM);
...
void do_makelandmine(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created landmine.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new landmine.", ch, NULL, argument,
	    TO_ROOM);
...
void do_makejewelry(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "body")
	    ...
	    || !str_cmp(arg, "hold")
	    || !str_cmp(arg, "feet")
	    || !str_cmp(arg, "hands") || !str_cmp(arg, "back")) {
		send_to_char
...
*/
	...
	default:
		...
		if (number_percent() < chance) {
			...
			act(AT_PLAIN,
			    ...
			    ch, NULL, argument, TO_ROOM);
			add_timer(ch, TIMER_DO_FUN, chance == 100 ? 8 : 15,
				  do_makejewelry, 1);
			ch->dest_buf = str_dup(arg);
	...
	obj->description = STRALLOC(buf);
	ac = ((int)
	      (obj->value[1] * (ch->pcdata->learned[gsn_makearmor]) / 100));
	ac -= 3;
	if (ch->pcdata && ch->pcdata->clan
	    && (!str_cmp(ch->pcdata->clan->name, "GLM")))
		ac += 2;
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created jewelry.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making  some new jewelry.", ch, NULL,
	    argument, TO_ROOM);
...
void do_makearmor(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "ears")
	    || !str_cmp(arg, "finger")
	    || !str_cmp(arg, "neck")
	    || !str_cmp(arg, "wrist") || !str_cmp(arg, "hip")) {
		send_to_char
	...
	if (!str_cmp(arg, "eyes")) {
		send_to_char
		    ("&RYou cannot make clothing worn on the eyes.\n\r&w", ch);
		return;
	}
	if (!str_cmp(arg, "wield")) {
		send_to_char
...
void do_makecomlink(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "finger")
	    ...
	    || !str_cmp(arg, "feet")
	    || !str_cmp(arg, "eyes")
	    || !str_cmp(arg, "about") || !str_cmp(arg, "arms")) {
		send_to_char
	...
	if (!str_cmp(arg, "shield")) {
		send_to_char
		    ("&RYou cannot make a comlink worn as a shield.\n\r&w", ch);
		send_to_char("&RTry MAKESHIELD.\n\r&w", ch);
		return;
	}
	if (!str_cmp(arg, "eyes")) {
		send_to_char
		    ("&RYou cannot make a comlink worn on the eyes.\n\r&w", ch);
		return;
	}
	if (!str_cmp(arg, "wield")) {
		send_to_char
		    ("&RAre you going to fight with your comlink?\n\r&w", ch);
		send_to_char("&RTry MAKEBLADE...\n\r&w", ch);
		return;
	}
	//strcpy( arg , argument );
	...
	default:
		...
		if (arg[0] == '\0' || arg2[0] == '\0') {
			send_to_char
			    ("&RUsage: Makecomlink <wearlocation> <name>\n\r&w",
			     ch);
			return;
		}
		if (strlen_color(arg) > 75) {
...
*/
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created comlink.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes crafting a comlink.", ch, NULL, argument,
	    TO_ROOM);
...
void do_makeshield(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (!checktool) {
			send_to_char
			    ("&RYou need toolkit to make an shield.\n\r", ch);
			return;
		}

		if (!checkbatt) {
			send_to_char
			    ("&RYou need a power source for the oven.\n\r", ch);
			return;
		}

		if (!checkoven) {
			send_to_char
			    ("&RYou need an oven to heat the components.\n\r",
			     ch);
			return;
		}
		...
		if (!checkdura) {
			send_to_char
			    ("&RYou still need some plastic for your shield.\n\r",
			     ch);
			return;
		}
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of crafting a shield.\n\r",
			     ch);
			act(AT_PLAIN,
			    "$n takes $s tools and begins to work on something.",
			    ch, NULL, argument, TO_ROOM);
	...
	{
		send_to_char
		    ("&RYou hold up your new shield and flip it over hoping for the best.\n\r",
		     ch);
		send_to_char
		    ("&RThe shield falls apart and shatters into many pieces.\n\r",
		     ch);
		learn_from_failure(ch, gsn_makeshield);
		return;
	}
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created shield.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new shield.", ch, NULL, argument,
	    TO_ROOM);
...
void do_makecontainer(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "feet")
	    || !str_cmp(arg, "hands") || !str_cmp(arg, "face")) {
		send_to_char
		    ("&RYou cannot make a container for that body part.\n\r&w",
		     ch);
		send_to_char("&RTry MAKEARMOR.\n\r&w", ch);
...
void do_reinforcements(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (ch->gold < ch->skill_level[LEADERSHIP_ABILITY] * 50) {
			ch_printf(ch,
				  "&RYou dont have enough wulongs to send for reinforcements.\n\r");
			return;
		}
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin making the call for reinforcements.\n\r",
			     ch);
			act(AT_PLAIN,
			    "$n begins issuing orders int $s comlink.", ch,
			    NULL, argument, TO_ROOM);
	...
	ch_printf(ch, "It cost you %d wulongs.\n\r", credits);
...
void add_reinforcements(CHAR_DATA * ch)
{
	...
	if (ch->backup_mob == MOB_VNUM_STORMTROOPER ||
		...
		if (ch->pcdata && ch->pcdata->clan) {
			...
			sprintf(tmpbuf, "(%s) %s", ch->pcdata->clan->name,
				mob->long_descr);
...
void do_torture(CHAR_DATA * ch, char *argument)
{
	...
	fail = FALSE;
	chance =
	    ris_save(victim, ch->skill_level[HUNTING_ABILITY], RIS_PARALYSIS);
	if (chance == 1000)
		fail = TRUE;
	else
	...
	if (!fail && (IS_NPC(ch)
		...
		act(AT_SKILL,
		    "$N slowly tortures you. The pain is excruciating.", victim,
		    NULL, ch, TO_CHAR);
		...
		act(AT_SKILL, "$N tries to cut off your finger!", victim, NULL,
		    ch, TO_CHAR);
...
void do_snipe(CHAR_DATA * ch, char *argument)
{
	...
	WAIT_STATE(ch, 20);
	if (number_percent() < chance) {
		sprintf(buf, "A rifle shot fires at you from the %s.",
			dir_name[dir]);
		act(AT_ACTION, buf, victim, NULL, ch, TO_CHAR);
		act(AT_ACTION, "You fire at $N.", ch, NULL, victim, TO_CHAR);
		sprintf(buf, "A rifle shot fires at $N from the %s.",
			dir_name[dir]);
		act(AT_ACTION, buf, ch, NULL, victim, TO_NOTVICT);
		...
		act(AT_ACTION, "You fire at $N but don't even come close.", ch,
		    NULL, victim, TO_CHAR);
		sprintf(buf,
			"A rifle shot fired from the %s barely misses you.",
			dir_name[dir]);
		act(AT_ACTION, buf, ch, NULL, victim, TO_ROOM);
	...
	make_shell(ch);
	act(AT_YELLOW,
	    "You hear the sound of your empty rifle shell hit the ground.", ch,
	    NULL, NULL, TO_CHAR);
	act(AT_YELLOW, "$n's rifle shell hits the ground with a loud ping.", ch,
	    NULL, NULL, TO_ROOM);
	act(AT_PLAIN, "You lower your rifle and prepare for another shot.", ch,
	    NULL, NULL, TO_CHAR);
	act(AT_PLAIN, "$n lowers $s rifle and prepares for another shot.", ch,
	    ch, NULL, TO_ROOM);
...
void do_throw(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->position == POS_FIGHTING) {
		...
		if (victim) {
			...
			if (IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim) {
				act(AT_PLAIN, "$N is your beloved master.", ch,
				    NULL, victim, TO_CHAR);
				return;
			}
	...
	if (!victim || char_died(victim))
		...
		if (IS_NPC(ch)
			...
			if ((obj->item_type == ITEM_WEAPON)
			    && (obj->value[3] == 2 || obj->value[3] == 3))
				global_retcode =
				    damage(ch, victim,
					   (number_range
					    (obj->value[1], obj->value[2]) * 5),
					   TYPE_HIT);
			else
				global_retcode =
				    damage(ch, victim,
...
void do_beg(CHAR_DATA * ch, char *argument)
{
	...
	if (percent > ch->pcdata->learned[gsn_beg]) {
		...
		if (victim->alignment < 0
			...
			sprintf(buf,
				"%s is an annoying beggar and needs to be taught a lesson!",
				ch->name);
			do_yell(victim, buf);
			global_retcode = multi_hit(victim, ch, TYPE_UNDEFINED);
		}
	...
	ch->gold += amount;
	victim->gold -= amount;
	ch_printf(ch, "%s gives you %d wulongs.\n\r", victim->short_descr,
		  amount);
...
void do_hijack(CHAR_DATA * ch, char *argument)
{
	...
	if (number_percent() < chance) {
		...
		act(AT_PLAIN,
		    "$n starts up the ship and begins the launch sequence.", ch,
		    NULL, argument, TO_ROOM);
...
void do_special_forces(CHAR_DATA * ch, char *argument)
{
	...
	default:
		if (ch->backup_wait) {
			send_to_char
			    ("&RYour forces are already on the way.\n\r", ch);
			return;
		}

		if (!ch->pcdata->clan) {
			send_to_char
			    ("&RYou need to be a member of an organization before you can call for special forces.\n\r",
			     ch);
			return;
		}
		...
		if (ch->gold < ch->skill_level[LEADERSHIP_ABILITY] * 60) {
			ch_printf(ch,
				  "&RYou dont have enough wulongs to send for elite forces.\n\r");
			return;
		}
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin making the call for special forces.\n\r",
			     ch);
			act(AT_PLAIN,
	...
	case SUB_TIMER_DO_ABORT:
		...
		ch->substate = SUB_NONE;
		send_to_char
		    ("&RYou are interrupted before you can finish your call.\n\r",
		     ch);
		return;
	}
...
void do_jail(CHAR_DATA * ch, char *argument)
{
	...
	if (jail->area && ch->in_room->area
	    ...
	    (!jail->area->planet
	     || jail->area->planet != ch->in_room->area->planet)) {
...
void do_propaganda(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->frustrated_by && victim->frustrated_by == ch
	    && victim->frustration > 3) {
		send_to_char
		    ("You try to tell them about your organization.\r\n", ch);
		sprintf(buf, "No thanks, %s, I'm tired of all your crap.",
			ch->name);
		do_say(victim, buf);
		return;
	}
	...
	if (percent - get_curr_cha(ch) + victim->top_level >
	    ch->pcdata->learned[gsn_propaganda] && !IS_IMMORTAL(ch)) {
...
void do_bribe(CHAR_DATA * ch, char *argument)
{
	...
	if (clan == planet->governed_by) {
		...
		amount =
		    UMIN(amount,
			 (exp_level(ch->skill_level[DIPLOMACY_ABILITY] + 1) -
			  exp_level(ch->skill_level[DIPLOMACY_ABILITY])));
...
void do_mass_propaganda(CHAR_DATA * ch, char *argument)
{
	...
	if (number_percent() - get_curr_cha(ch) + victim->top_level >
	    ch->pcdata->learned[gsn_masspropaganda]) {
		if (planet->governed_by != clan) {
			sprintf(buf, "%s is a traitor!", ch->name);
			do_yell(victim, buf);
			global_retcode = multi_hit(victim, ch, TYPE_UNDEFINED);
...
void do_rumors(CHAR_DATA * ch, char *argument)
{
	...
	if ((victim = get_char_world(ch, arg2)) == NULL) {
		send_to_char
		    ("You can't spread rumors about that person currently.\n\r",
		     ch);
		return;
	}
	...
	if ((number_range(10, 120) - get_curr_cha(ch) / 3 -
	     ch->pcdata->learned[gsn_rumors]) > 0) {
//      send_to_char("You fail at spreading rumors!\r\n", ch);
		sprintf(buf, "You spread rumors about %s to %s.\n\r",
			victim->name, hater->name);
		send_to_char(buf, ch);
		act(AT_ACTION, "$n spreads rumors to $N.", ch, NULL, hater,
		    TO_ROOM);
...
void do_hack_systems(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch) && !IS_SET(ch->pcdata->flags, PCFLAG_HACKWARN)) {
		send_to_char
		    ("&RWARNING! Hacking is illegal in the galaxy of Cowboy Bebop: Space Cowboy!\r\n",
		     ch);
		send_to_char
		    ("&YIf you hack and fail, you will get a bounty which could lead to attacks by ISSP or RBH!\r\n",
		     ch);
		send_to_char
		    ("&RThis is your first and only warning, next time you hack you wont see this.\r\n",
		     ch);
	...
	if (!power) {
		send_to_char
		    ("&RMaybe you should 'bootup' your computer first.\n\r",
		     ch);
		return;
	}
/*
...
*/
	...
	if (found) {
		...
		act(AT_ACTION,
		    "$n begins to tamper with a comlink and computer.\n\r", ch,
		    NULL, NULL, TO_ROOM);

		if (number_percent() - get_curr_int(ch) / 5 >
		    ch->pcdata->learned[gsn_hack_systems] - 10) {
			send_to_char("Your attempt to hack is discovered!\n\r",
				     ch);
			disintigration(ch, ch, 3000);
			...
			for (obj = ch->last_carrying; obj;
				...
				if (obj->item_type == ITEM_COMPUTER
					...
					if ((obj->value[0] - 1) > 0) {
						obj->value[0]--;
						send_to_char
						    ("&RYour computer was damaged!\n\r",
						     ch);
					} else {
						obj->value[0]--;
						send_to_char
						    ("Your computer was destroyed!\n\r",
						     ch);
					}
					break;
				}
...
*/
	...
	if (found) {
		if (!bootup) {
			act(AT_RED,
			    "Maybe you should 'bootup' your computer first?",
			    ch, NULL, NULL, TO_CHAR);
			return;
		}
		if (number_percent() - get_curr_int(ch) >
		    ch->pcdata->learned[gsn_hackwho] || number_range(0, 5) > 4)
		{
			send_to_char("Your attempt to hack is discovered!\n\r",
				     ch);
			disintigration(ch, ch, 10000);
			if (obj->value[0] - 1 > 0) {
				obj->value[0]--;
...
void do_hackbaydoor(CHAR_DATA * ch, char *argument)
{
	...
	if (abs(eShip->vx - ship->vx) >
	    100 * ((ship->comm) + (eShip->comm) + 20)
	    || abs(eShip->vy - ship->vy) >
	    100 * ((ship->comm) + (eShip->comm) + 20)
	    || abs(eShip->vz - ship->vz) >
	    100 * ((ship->comm) + (eShip->comm) + 20)) {
		send_to_char
	...
	if (number_percent() - get_curr_int(ch) >
	    ch->pcdata->learned[gsn_hackbaydoor]) {
		send_to_char("Your attempt to hack is discovered!\n\r", ch);
		disintigration(ch, ch, 10000);
		for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
			if (obj->item_type == ITEM_COMPUTER
...
void do_hackbanktransfer(CHAR_DATA * ch, char *argument)
{
	...
	for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
		if (obj->pIndexData->item_type == ITEM_COMLINK)
			comlink = TRUE;
		if (obj->pIndexData->item_type == ITEM_COMPUTER
		    && obj->value[0] > 0)
			computer = TRUE;
	...
	if (!power) {
		act(AT_RED, "Maybe you should 'bootup' your computer first?",
		    ch, NULL, NULL, TO_CHAR);
		return;
	}
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0') {
		send_to_char
		    ("&CHacktransfer: Bank Account 1/Bank Account 2/Amount\n\r",
		     ch);
		return;
	}
	...
	if (amount < 5) {
		send_to_char
		    ("&RYou need to pick an amount higher than that!\r\n", ch);
		return;
	}
	...
	if (ch == victim2) {
		send_to_char
		    ("&RIsn't that a bit obvious for the ISSP to spot?\n\r",
		     ch);
		return;
	}
	if (IS_NPC(victim1) || IS_NPC(victim2) || NOT_AUTHED(victim1)
	    || NOT_AUTHED(victim2)) {
		send_to_char
		    ("&RTarget characters need to be players and authorized first!\n\r",
		     ch);
		return;
	}
	...
	if (get_age(victim1) < 6 || get_age(victim2) < 6) {
		send_to_char
		    ("&RTarget characters need to have played atleast 6 hours first!\r\n",
		     ch);
		return;
	}
	...
	if (victim1->pcdata->bank < amount) {
		send_to_char
		    ("&RThe selected sender's account doesn't have that much!\n\r",
		     ch);
		return;
	}
	if ((victim2->pcdata->bank + amount) > MAX_BANK) {	/* replaced hard coded value - Funf */
		send_to_char
		    ("&RThat would put the reciever's account over the limit!\n\r",
		     ch);
		return;
	}
	if (amount == 0) {
	...
	if (number_percent() - get_curr_int(ch) >
	    ch->pcdata->learned[gsn_hack_systems] || number_range(0, 5) > 4) {
		send_to_char("Your attempt to hack is discovered!\n\r", ch);
		disintigration(ch, ch, 10000);
		//     send_to_char( "Your computer was destroyed!\n\r", ch);
		for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
	...
	ch_printf(ch, "You were able to hack %ld wulongs from %s to %s!\n\r",
		  amount, victim1->name, victim2->name);
	ch_printf(victim2,
		  "&R1#22@3 &Ctransfers &Y%ld &Cwulongs to your account.&W\n\r",
		  amount);
...
void do_hackshipdoor(CHAR_DATA * ch, char *argument)
{
	...
	if ((ship = ship_in_room(ch->in_room, arg)) != NULL) {
		...
		if (ship->shipstate != SHIP_DOCKED
			...
			send_to_char
			    ("&RThat ship has already started to launch", ch);
			return;
		}
		if (ship->hatchopen) {
		...
		if (number_percent() - get_curr_int(ch) >
		    ch->pcdata->learned[gsn_hackshipdoor]
		    || number_range(1, 5) > 4) {
			send_to_char("You were detected!\n\r", ch);
			disintigration(ch, ch, 5000);
			/*
			   send_to_char( "Your computer was destroyed!\n\r", ch);
			   separate_obj( comp );
			   obj_from_char( comp );
		...
		if (!ship->hatchopen) {
			ship->hatchopen = TRUE;
			act(AT_PLAIN,
			    "You hack the lock and hatch opens on $T!", ch,
			    NULL, ship->name, TO_CHAR);
			act(AT_PLAIN, "$n hacked open the hatch on $T!", ch,
...
void do_hacklaunch(CHAR_DATA * ch, char *argument)
{
	...
	if (number_percent() - get_curr_int(ch) >
	    ch->pcdata->learned[gsn_hacklaunch] || number_range(1, 5) > 3) {
		send_to_char("Your attempt to hack is discovered!\n\r", ch);
		disintigration(ch, ch, 10000);
		//  send_to_char( "Your computer was destroyed!\n\r", ch);
		for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
...
void do_hackbroadcast(CHAR_DATA * ch, char *argument)
{
	...
	if (number_percent() - (get_curr_int(ch) * 2) >
	    ch->pcdata->learned[gsn_hackbroadcast]) {
		send_to_char("Your attempt to hack is discovered!\n\r", ch);
		disintigration(ch, ch, 5000);
		//  send_to_char( "Your computer was destroyed!\n\r", ch);
		for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
...
void do_hackbounty(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		send_to_char
		    ("&RHackbounty 'add/remove' 'name' # (Number for Add only)\n\r",
		     ch);
		return;
	}
	...
	if (!str_cmp(arg1, "add") && amount > 500000000) {
	...
	if (number_percent() - get_curr_int(ch) >
	    ch->pcdata->learned[gsn_hackbounty]) {
		send_to_char("Your attempt to hack is discovered!\n\r", ch);
		disintigration(ch, ch, 10000);
		//   send_to_char( "Your computer was destroyed!\n\r", ch);
		for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
	...
	if (!str_prefix(arg1, "add")) {
		if (amount < 5000) {
			send_to_char
			    ("&RYou need to put atleast 5,000 wulongs on their head!\n\r",
			     ch);
			return;
		}
		bounty = get_disintigration(victim->name);
		if (bounty && (bounty->amount + amount > 500000000)) {
			send_to_char
			    ("&ROUT OF BOUNDS ERROR: Line 2231, contact ISSP to correct error.\r\n",
			     ch);
			return;
		}
		if (bounty && bounty->amount < 100000000) {
			send_to_char
			    ("&RThis would look a bit obvious, wouldn't it?\n\r",
			     ch);
			return;
		}
		sprintf(buf,
			"[ERRORTHREAD1A#] has added %s wulongs to the bounty on %s.",
...
void do_makerifle(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of making a rifle.\n\r",
			     ch);
			act(AT_PLAIN,
			    "$n takes $s tools and a small oven and begins to work on something.",
			    ch, NULL, argument, TO_ROOM);
	...
	if (number_percent() > chance * 2 || (!checktool) || (!checkdura)
	    || (!checkbatt) || (!checkoven) || (!checkplas) || (!checkmetal)) {
		send_to_char
		    ("&RYou hold up your new rifle and aim at a leftover piece of plastic.\n\r",
		     ch);
		send_to_char
		    ("&RYou slowly squeeze the trigger hoping for the best...\n\r",
		     ch);
		send_to_char
		    ("&RYour rifle backfires destroying your weapon and burning your hand.\n\r",
		     ch);
		learn_from_failure(ch, gsn_makerifle);
		return;
	}
	...
	obj->description = STRALLOC(buf);
	obj->value[0] = INIT_WEAPON_CONDITION;	/* condition  */
	obj->value[1] = (int)(level / 50 + 6);	/* min dmg  */
	obj->value[2] = (int)(level / 10 + 15);	/* max dmg  */
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created rifle.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new rifle.", ch, NULL, argument,
	    TO_ROOM);
...
void do_webwho(CHAR_DATA * ch, char *argument)
{
	...
	if (!computer && !IS_SET(ch->in_room->room_flags2, ROOM_COMPUTER)) {
		send_to_char
		    ("&RYou need a computer or a computer terminal to check who is currently in the world!\n\r",
		     ch);
		return;
	}
...
void do_makelaptop(CHAR_DATA * ch, char *argument)
{
	...
	char buf[MAX_STRING_LENGTH];
	int level, chance;
	bool checktool, checkbattery, checkmonitor, checkmotherboard,
	    checkharddrive, checkcase;
	OBJ_DATA *obj;
	OBJ_INDEX_DATA *pObjIndex;
	int vnum, power, scope;
	AFFECT_DATA *paf;
	...
	default:
		...
		if (!checkmonitor) {
			send_to_char
			    ("&RYou need a monitor to make a laptop.\n\r", ch);
			return;
		}

		if (!checkbattery) {
			send_to_char
			    ("&RYou need a battery to power the laptop.\n\r",
			     ch);
			return;
		}
		...
		if (!checkmotherboard) {
			send_to_char
			    ("&RYou need a motherboard for the laptop.\n\r",
			     ch);
			return;
		}
		...
		if (!checkharddrive) {
			send_to_char
			    ("&RYou need a hard drive for the computer.\n\r",
			     ch);
			return;
		}
		if (!checkcase) {
		...
		chance =
		    IS_NPC(ch) ? ch->top_level : (int)(ch->pcdata->
						       learned[gsn_makelaptop]);
		if (number_percent() < chance) {
			send_to_char
			    ("&GYou begin the long process of making a laptop.\n\r",
			     ch);
			act(AT_PLAIN,
			    "$n takes $s tools and a small oven and begins to work on something.",
			    ch, NULL, argument, TO_ROOM);
	...
	if (number_percent() > chance * 2 || (!checktool) || (!checkbattery)
	    || (!checkmonitor) || (!checkmotherboard) || (!checkharddrive)
	    || (!checkcase)) {
		send_to_char
		    ("&RYou try to login to your new lapto and it fizzles out.\n\r",
		     ch);
		send_to_char
		    ("&RA foul smell and rich black smoke come from the worthless machine.\n\r",
		     ch);
		learn_from_failure(ch, gsn_makelaptop);
		return;
	}
	...
	send_to_char
	    ("&GYou finish your work and hold up your newly created laptop.&w\n\r",
	     ch);
	act(AT_PLAIN, "$n finishes making $s new laptop.", ch, NULL, argument,
	    TO_ROOM);
...
void do_hackbank(CHAR_DATA * ch, char *argument)
{
	...
	if (!comp) {
		send_to_char
		    ("&RYou need a computer which is turned on to hack!\r\n",
		     ch);
		return;
	}
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		send_to_char
		    ("&Chackbank: 'Account Person's Name' 'Add/Remove' 'Amount'\n\r",
		     ch);
		return;
	}
	...
	if (get_age(victim) < 6) {
		send_to_char
		    ("&RThe victim must have played atleast 6 hours!\r\n", ch);
		return;
	}
	if (amount > level) {
		ch_printf(ch, "&RYou can only transfer up to %ld!", level);
		return;
	}
	if (amount < 0) {
		send_to_char
		    ("&RYou can't have a negitive amount of wulongs!\n\r", ch);
		return;
	}
	...
	if ((victim->pcdata->bank - amount) < 0 && !str_prefix(arg2, "remove")) {
		send_to_char
		    ("&RThat would put the reciever's account under the limit!\n\r",
		     ch);
		return;
	}
	if (amount == 0) {
...
*/
	...
	if (number_percent() - get_curr_int(ch) >
	    ch->pcdata->learned[gsn_hackbounty] || number_range(0, 5) > 4) {
		send_to_char("Your attempt to hack is discovered!\n\r", ch);
	...
	if (!str_prefix(arg2, "add")) {
		...
		ch_printf(ch,
			  "You were able to hack %ld wulongs to %s's account!\n\r",
			  amount, victim->name);
	...
	if (!str_prefix(arg2, "remove")) {
		victim->pcdata->bank -= amount;
		ch_printf(ch,
			  "You were able to hack %ld wulongs from %s's acount!\n\r",
			  amount, victim->name);
		gain_exp(ch, (ch->top_level / 2 + 60), FORCE_ABILITY);
		ch_printf(ch, "You gain %d hacking experience.\n\r",
			  (ch->top_level / 2 + 60));
	...
	send_to_char
	    ("&RYou can only add or remove from a person's bank account!\n\r",
	     ch);
	return;
}
...
void do_hackclanaccount(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0') {
		send_to_char
		    ("&RHackclanaccount 'add/remove' 'name' # (Number for Add only)\n\r",
		     ch);
		return;
	}
	clan = get_clan(arg2);
	...
	if (number_percent() - get_curr_int(ch) >
	    ch->pcdata->learned[gsn_hackclanaccount] || number_range(0, 5) > 4)
	{
		send_to_char("Your attempt to hack is discovered!\n\r", ch);
		disintigration(ch, ch, 10000);
		for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
	...
	if (!str_prefix(arg1, "add")) {
		if (amount < 5000) {
			send_to_char
			    ("&RYou need to put atleast 5,000 wulongs in their account\n\r",
			     ch);
			return;
		}
		if (clan->funds > MAX_BANK - MAX_BANK / 4) {	/* tweaked check and removed hard coded value - Funf */
			send_to_char
			    ("&RYou can't add anymore to this account!\n\r",
			     ch);
			return;
		}
		clan->funds += amount;
...
void do_newwhois(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch, "&C----------=<(&GConnecting...&C)>=---------&W&W\n\r");
	ch_printf(ch,
	...
	if (victim->pcdata->clan) {
		if (victim->pcdata->clan->clan_type == CLAN_GUILD) {
			ch_printf(ch,
				  "&C|&RClan        &R[&WRBH            &R]\n\r");
			ch_printf(ch,
				  "&C|&RPosition    &R[&W&cMember     &W&R]\n\r");
		} else {
			ch_printf(ch, "&C|&RClan        &R[&W%-15s&R]\n\r",
				  victim->pcdata->clan->name);
			ch_printf(ch, "&C|&RPosition    &R[&W%-15s&R]\n\r",
				  victim->pcdata->clan->leader ==
				  victim->name ? "&CL&ceader&W" : victim->
				  pcdata->clan->number1 ==
				  victim->name ? "&CF&cirst&W" : victim->
				  pcdata->clan->number2 ==
				  victim->name ? "&CS&cecond&W" : "&cMember&W");
		}
	} else
		ch_printf(ch, "&C|&RClan        &R[&W%-15s&R]\n\r",
			  "No clan available!");
	...
	ch_printf(ch,
		  "&C-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=&W\n\r");
	ch_printf(ch, "&C----------=<(&GDisconnected.&C)>=----------&W\n\r");
	...
	if (IS_IMMORTAL(ch)) {
		...
		if (victim->pcdata->unsilence_date != 0)
			ch_printf(ch,
				  "%s was silenced by %s, and will be released on %24.24s.\n\r",
				  victim->sex ==
				  SEX_MALE ? "He" : victim->sex ==
				  SEX_FEMALE ? "She" : "It",
...
void do_background(CHAR_DATA * ch, char *argument)
{
	...
	if (ch->pcdata && ch->pcdata->clan
		...
		send_to_char
		    ("Only ISSP can check up on backgrounds legally!\n\r", ch);
		return;
	}
	...
	if (((victim = get_char_world(ch, buf)) == NULL)) {
		send_to_char
		    ("&C----------=<(&GConnecting...&C)>=----------&W\n\r", ch);
		send_to_char
		    ("&C----------&RAccessed Background Check &C----------&W\n\r",
		     ch);
		send_to_char("Whois file: &RDENIED&w\n\r", ch);
		send_to_char("&GPossible cause is player is not on-line!&W\n\r",
			     ch);
...
void do_commandeer(CHAR_DATA * ch, char *argument)
{
	...
	if (!comlink) {
		send_to_char
		    ("&RYou need a communication device to commandeer ships!\n\r",
		     ch);
		return;
	}
	if (ship->class == FIGHTER_SHIP)
	...
	sprintf(buf1,
		"&w%s says into is communication device, '&CISSP Officer %s requesting codes to commandeer vessel %s.&w'",
		ch->name, ch->name, ship->name);
	act(AT_PLAIN, buf1, ch, NULL, NULL, TO_ROOM);
	sprintf(buf1,
		"&wYou say into your communication device, '&CISSP Officer %s requesting codes to commandeer vessel %s.&w'",
		ch->name, ship->name);
	act(AT_PLAIN, buf1, ch, NULL, NULL, TO_CHAR);
	act(AT_PLAIN,
	    "&wA voice on $n's comlink says, '&COkay Officer, permission granted.&w'",
	    ch, NULL, NULL, TO_ROOM);
	act(AT_PLAIN,
	    "&wA voice on your comlink says, '&COkay Officer, permission granted.&w'",
	    ch, NULL, NULL, TO_CHAR);
...
void do_bankfreeze(CHAR_DATA * ch, char *argument)
{
	...
	ch_printf(ch, "%s now has a freeze %s %s bank account.\r\n",
		  victim->name,
		  (IS_SET(victim->pcdata->flags, PCFLAG_FREEZE)) ?
		  "removed from" : "added to",
		  (victim->sex == 1) ? "his" : "her");
	ch_printf(victim, "Your bank account has been %s.\r\n",
		  (IS_SET(victim->pcdata->flags, PCFLAG_FREEZE)) ? "unfrozen" :
		  "frozen");
...
void do_attend(CHAR_DATA * ch, char *argument)
{
	...
	medpac->value[0]--;
	if ((number_range(1, 100) - get_curr_int(victim) -
	     (ch->pcdata->learned[gsn_attend] / 2)) > 80) {
		act(AT_DGREEN,
		    "You try to attend $N but make too many mistakes.", ch,
		    NULL, victim, TO_CHAR);
		act(AT_DGREEN,
		    "$n tries to attend to your wounds but makes too many mistakes.",
		    ch, NULL, victim, TO_VICT);
		act(AT_DGREEN,
		    "$n tries to attend to $N but seems to struggle and make mistakes.",
		    ch, NULL, victim, TO_NOTVICT);
		return;
	}
	lag = 20;
	...
	act(AT_DGREEN, "$n attends to $N's wounds.", ch, NULL, victim,
	    TO_NOTVICT);
	learn_from_success(ch, gsn_attend);
	heal = number_range(10, 30);
	victim->hit += heal;
	if (victim->hit > victim->max_hit)
...
void do_bandage(CHAR_DATA * ch, char *argument)
{
	...
	medpac->value[0]--;
	if ((number_range(20, 150) - get_curr_int(victim) -
	     (ch->pcdata->learned[gsn_bandage] / 2)) > 80) {
		act(AT_DGREEN,
		    "You try to bandage $N but make too many mistakes.", ch,
		    NULL, victim, TO_CHAR);
		act(AT_DGREEN,
		    "$n tries to bandage to your wounds but makes too many mistakes.",
		    ch, NULL, victim, TO_VICT);
		act(AT_DGREEN,
		    "$n tries to bandage to $N but seems to struggle and make mistakes.",
		    ch, NULL, victim, TO_NOTVICT);
		return;
	}
	lag = 20;
	...
	act(AT_DGREEN, "$n bandages $N's wounds.", ch, NULL, victim,
	    TO_NOTVICT);
	learn_from_success(ch, gsn_bandage);
	heal = number_range(35, 50);
	victim->hit += heal;
	if (victim->hit > victim->max_hit)
		victim->hit = victim->max_hit;
	long xpgain;
...
void do_heal(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (victim->hit < 500) {
			send_to_char
			    ("They are far too injured to try to use heal on them!\r\n",
			     ch);
			return;
		}
		if (victim->hit > 999) {
		...
		if (!medpac || medpac->item_type != ITEM_MEDPAC) {
			send_to_char
			    ("You need to be holding a Medical Kit.\n\r", ch);
			return;
		}
		...
		act(AT_DGREEN, "$n begins to try and heal $N.", ch, NULL,
		    victim, TO_NOTVICT);
		act(AT_DGREEN, "You begin to try and heal $N.", ch, NULL,
		    victim, TO_CHAR);
		act(AT_DGREEN, "$n begins to try and heal you.", ch, NULL,
		    victim, TO_VICT);
	...
	case SUB_TIMER_DO_ABORT:
		DISPOSE(ch->dest_buf);
		ch->substate = SUB_NONE;
		act(AT_DGREEN, "$n gets distracted and stops trying to heal.",
		    ch, NULL, NULL, TO_ROOM);
		act(AT_DGREEN, "You get distracted and stop trying to heal.",
		    ch, NULL, NULL, TO_CHAR);
		return;
	}
	ch->substate = SUB_NONE;
	...
	medpac->value[0]--;
	if ((number_range(20, 120) - get_curr_int(victim) -
	     (ch->pcdata->learned[gsn_heal] / 2)) > 80) {
		act(AT_DGREEN, "You try to heal $N but make too many mistakes.",
		    ch, NULL, victim, TO_CHAR);
		act(AT_DGREEN,
		    "$n tries to heal your wounds but makes too many mistakes.",
		    ch, NULL, victim, TO_VICT);
		act(AT_DGREEN,
		    "$n tries to heal $N but seems to struggle and make miostakes.",
		    ch, NULL, victim, TO_NOTVICT);
		return;
	}

	act(AT_DGREEN, "You heal $N's wounds.", ch, NULL, victim, TO_CHAR);
	act(AT_DGREEN, "$n heals some of your wounds.", ch, NULL, victim,
	    TO_VICT);
	act(AT_DGREEN, "$n heals $N's wounds.", ch, NULL, victim, TO_NOTVICT);
	learn_from_success(ch, gsn_heal);
	heal = number_range(50, 75);
	victim->hit += heal;
	if (victim->hit > victim->max_hit)
		victim->hit = victim->max_hit;
	long xpgain;
...
void do_afkmessage(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0') {
		send_to_char
		    ("Syntax: AFKMessage (Message) or AFKMessage 'Default' to clear.\r\n",
		     ch);
		return;
	}
...
void do_self_medicate(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (ch->perm_frc > 0) {
			send_to_char
			    ("You heart is racing to fast for this!\r\n", ch);
			return;
		}
		if (medpac->value[0] <= 0) {
			send_to_char("Your Medical Kit seems to be empty.\n\r",
		...
		if (ch->hit > 970) {
			send_to_char
			    ("You are far to healthy to gain anything from this!\r\n",
			     ch);
			return;
		}
		timer = 6;
		...
		act(AT_GREEN,
		    "You start to dole out some medication for yourself", ch,
		    NULL, NULL, TO_CHAR);
		act(AT_GREEN, "$n startes to dole out some medication.", ch,
		    NULL, NULL, TO_ROOM);
		return;
	case 1:
		act(AT_GREEN, "You down the medication.", ch, NULL, NULL,
		    TO_CHAR);
		act(AT_GREEN, "$n downs the medication.", ch, NULL, NULL,
		    TO_ROOM);
		medpac = get_eq_char(ch, WEAR_HOLD);
		if (!medpac || medpac->item_type != ITEM_MEDPAC) {
			send_to_char
		...
		medpac->value[0]--;
		if ((number_range(20, 110) - get_curr_int(ch) -
		     (ch->pcdata->learned[gsn_self_medicate] / 2)) > 80) {
			act(AT_DGREEN,
			    "You gasp as you take a bad dosage of medicine", ch,
			    NULL, NULL, TO_CHAR);
			act(AT_DGREEN,
			    "$n gasps as $e took a bad dosage of medicine", ch,
			    NULL, NULL, TO_ROOM);
			return;
		}
		...
		if (xpgain > 0) {
			ch_printf(ch,
				  "You gain %ld experience from your success!",
				  xpgain);
			gain_exp(ch, xpgain, MEDIC_ABILITY);
		}
		return;
	case SUB_TIMER_DO_ABORT:
		...
		ch->substate = SUB_NONE;
		act(AT_DGREEN, "$n gets distracted and puts the medicine back.",
		    ch, NULL, NULL, TO_ROOM);
		act(AT_DGREEN, "You get distracted and put the medicine back.",
		    ch, NULL, NULL, TO_CHAR);
...
void do_surgery(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (victim->hit < 300) {
			send_to_char
			    ("They are far too injured to try to use surgery on them!\r\n",
			     ch);
			return;
		}
		if (victim->hit > 999) {
		...
		act(AT_DGREEN, "$n begins to try surgery on $N.", ch, NULL,
		    victim, TO_NOTVICT);
		act(AT_DGREEN, "You begin to try surgery on $N.", ch, NULL,
		    victim, TO_CHAR);
		act(AT_DGREEN, "$n begins to try surgery on you.", ch, NULL,
		    victim, TO_VICT);
	...
	case SUB_TIMER_DO_ABORT:
		...
		act(AT_DGREEN,
		    "You get distracted and stop trying to perform surgery.",
		    ch, NULL, NULL, TO_CHAR);
		act(AT_DGREEN,
		    "$n gets distracted and stops trying to perform surgery.",
		    ch, NULL, NULL, TO_ROOM);
		return;
	}
	ch->substate = SUB_NONE;
	...
	medpac->value[0]--;
	if ((number_range(20, 140) - get_curr_int(victim) -
	     (ch->pcdata->learned[gsn_surgery] / 2)) > 80) {
		act(AT_DGREEN,
		    "You try to surgery $N but make too many mistakes.", ch,
		    NULL, victim, TO_CHAR);
		act(AT_DGREEN,
		    "$n tries to surgery your wounds but makes too many mistakes.",
		    ch, NULL, victim, TO_VICT);
		act(AT_DGREEN,
		    "$n tries to surgery $N but seems to struggle and make mistakes.",
		    ch, NULL, victim, TO_NOTVICT);
		victim->hit -= 50;
		update_pos(victim);
		return;
	...
	act(AT_DGREEN, "Your surgery on $N's wounds was a success!", ch, NULL,
	    victim, TO_CHAR);
	act(AT_DGREEN, "$n's surgery on you was a success!", ch, NULL, victim,
	    TO_VICT);
	act(AT_DGREEN, "$n finishes surgery on $N's wounds.", ch, NULL, victim,
	    TO_NOTVICT);
	learn_from_success(ch, gsn_surgery);
	surgery = number_range(100, 125);
...
void do_treat(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (victim->hit < 50) {
			send_to_char
			    ("They are far too injured to be treated!\r\n", ch);
			return;
		}
		if (victim->hit > 999) {
		...
		act(AT_DGREEN, "$n begins to treat $N.", ch, NULL, victim,
		    TO_NOTVICT);
		act(AT_DGREEN, "You begin to treat $N.", ch, NULL, victim,
		    TO_CHAR);
		act(AT_DGREEN, "$n begins to treat you.", ch, NULL, victim,
		    TO_VICT);
	...
	case SUB_TIMER_DO_ABORT:
		...
		act(AT_DGREEN, "You get distracted and stop treating someone.",
		    ch, NULL, NULL, TO_CHAR);
		act(AT_DGREEN, "$n gets distracted and stops treating someone.",
		    ch, NULL, NULL, TO_ROOM);
		return;
	}
	ch->substate = SUB_NONE;
	...
	medpac->value[0]--;
	if ((number_range(20, 130) - get_curr_int(victim) -
	     (ch->pcdata->learned[gsn_treat] / 2)) > 80) {
		act(AT_DGREEN,
		    "You try to treat $N but make too many mistakes.", ch, NULL,
		    victim, TO_CHAR);
		act(AT_DGREEN,
		    "$n tries to treat your wounds but makes too many mistakes.",
		    ch, NULL, victim, TO_VICT);
		act(AT_DGREEN,
		    "$n tries to treat $N but seems to struggle and make mistakes.",
		    ch, NULL, victim, TO_NOTVICT);
		return;
	}
	...
	act(AT_DGREEN, "$n finishes treating $N's wounds.", ch, NULL, victim,
	    TO_NOTVICT);
...
void do_resuscitate(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->hit <= -400) {
		act(AT_PLAIN,
		    "$N's condition is beyond your resuscitating ability.", ch,
		    NULL, victim, TO_CHAR);
		return;
	}
...
void do_group_heal(CHAR_DATA * ch, char *argument)
{
	...
	default:
		...
		if (group < 1) {
			send_to_char
			    ("You need to be in a group full of injured people first!\r\n",
			     ch);
			return;
		}
		if (medpac->value[0] < group) {
			send_to_char
			    ("You Medical Kit doesn't have enough uses for your group!\r\n",
			     ch);
			return;
		}
		time = group * 4;
		...
		act(AT_DGREEN, "$n begins to run around healing $s group.", ch,
		    NULL, NULL, TO_ROOM);
		act(AT_DGREEN, "You begin to run around healing your group", ch,
		    NULL, NULL, TO_CHAR);
	...
	case SUB_TIMER_DO_ABORT:
		...
		ch->substate = SUB_NONE;
		act(AT_DGREEN,
		    "$n gets distracted and stops healing his group.", ch, NULL,
		    NULL, TO_ROOM);
		act(AT_DGREEN,
		    "You get distracted and stop healing your group.", ch, NULL,
		    NULL, TO_CHAR);
	...
	if ((number_range(20, 120) - get_curr_int(ch) -
	     (ch->pcdata->learned[gsn_group_heal] / 2)) > 80) {
		medpac->value[0]--;
		act(AT_DGREEN,
		    "You try to heal your group but are too overwhelmed.", ch,
		    NULL, NULL, TO_CHAR);
		act(AT_DGREEN,
		    "$n tries to heal $s group, but is too overwhelmed.", ch,
		    NULL, NULL, TO_ROOM);
		return;
	}
	for (gch = ch->in_room->first_person; gch; gch = gch->next_in_room) {
		if (is_same_group(gch, ch) && gch != ch && gch->hit < 999) {
			if (gch->hit < 50) {
				act(AT_DGREEN,
				    "$N's wounds are too much to heal.", ch,
				    NULL, gch, TO_CHAR);
				continue;
			}
			if (gch->perm_frc > 0) {
				act(AT_DGREEN,
				    "$N has too much adrenaline to heal.", ch,
				    NULL, gch, TO_CHAR);
				continue;
			}
			if (medpac->value[0] <= 0) {
				send_to_char
				    ("You ran out of Medical Supplies in your Kit!\r\n",
				     ch);
				break;
			}
			medpac->value[0]--;
			group++;
			act(AT_DGREEN, "$n heals some of your wounds!", ch,
			    NULL, gch, TO_VICT);
			act(AT_DGREEN, "You heal some of $N's wounds!", ch,
			    NULL, gch, TO_CHAR);
	...
	act(AT_DGREEN, "You finish healing your group!", ch, NULL, NULL,
	    TO_CHAR);
	act(AT_DGREEN, "$n finishes healing $s group!", ch, NULL, NULL,
	    TO_ROOM);
...
void do_bootup(CHAR_DATA * ch, char *argument)
{
	...
	for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
		if (obj->item_type == ITEM_COMPUTER) {
			...
			if (obj->value[2] == 0 || obj->value[2] > 1) {
				act(AT_RED,
				    "$n flips the switch on their computer, and the screen lights up.",
				    ch, NULL, NULL, TO_ROOM);
				act(AT_RED,
				    "You flip the on switch on your computer.",
				    ch, NULL, NULL, TO_CHAR);
				act(AT_RED,
				    "The bios detects the hardware and the unix environment starts.",
				    ch, NULL, NULL, TO_CHAR);
				act(AT_RED,
				    "The desktop GUI initializes and loads, the computer is ready.",
				    ch, NULL, NULL, TO_CHAR);
				obj->value[2] = 1;
				found = TRUE;
				break;
	...
	if (found != TRUE) {
		if (count == 0) {
			act(AT_RED,
			    "You don't have any computers to turn on...", ch,
			    NULL, NULL, TO_CHAR);
			return;
		}
		if (count > 0) {
			act(AT_RED, "Your computer is already on!", ch, NULL,
			    NULL, TO_CHAR);
			return;
		}
...
void do_shutoff(CHAR_DATA * ch, char *argument)
{
	...
	for (obj = ch->last_carrying; obj; obj = obj->prev_content) {
		if (obj->item_type == ITEM_COMPUTER) {
			...
			if (obj->value[2] > 0) {
				act(AT_RED,
				    "$n flips the switch on their computer, and the screen slowly fickers off and dies.",
				    ch, NULL, NULL, TO_ROOM);
				act(AT_RED,
				    "You flip the off switch on your computer.",
				    ch, NULL, NULL, TO_CHAR);
				act(AT_RED,
				    "The GUI fades away into a text shutdown script.",
				    ch, NULL, NULL, TO_CHAR);
				act(AT_RED,
				    "The script fades, and the screen flickers off.",
				    ch, NULL, NULL, TO_CHAR);
				obj->value[2] = 0;
				found = TRUE;
				break;
	...
	if (found != TRUE) {
		...
		if (count == 0) {
			act(AT_RED,
			    "You don't have any computers to turn off...", ch,
			    NULL, NULL, TO_CHAR);
			return;
		}
		if (count > 0) {
			act(AT_RED, "Your computer is already off!", ch, NULL,
			    NULL, TO_CHAR);
			return;
		}
	}
...
void do_deceive(CHAR_DATA * ch, char *argument)
{
	...
	if (target->frustrated_by) {
		ch_printf(ch,
			  "%s is already frustrated by someone and needs to calm down first.\r\n",
			  target->name);
		return;
	}
	...
	case 3:
		act(AT_RED, "You grin evilly as you try to deceive $N", ch,
		    NULL, target, TO_CHAR);
		act(AT_RED, "$n grins evilly as $e tries to deceive $N", ch,
		    NULL, target, TO_ROOM);
		break;
	case 2:
		act(AT_RED,
		    "You calmly lean in to whisper things to deceive $N.", ch,
		    NULL, target, TO_CHAR);
		act(AT_RED,
		    "$n looks calm as $e leans over and whispers something to $N.",
		    ch, NULL, target, TO_ROOM);
		break;
	case 1:
		act(AT_RED, "You smile caringly as you try to deceive $N.", ch,
		    NULL, target, TO_CHAR);
		act(AT_RED, "$n smiles caringly as $e tries to deceive $N.", ch,
		    NULL, target, TO_ROOM);
		break;
	default:
		act(AT_RED, "You try to deceive $N.", ch, NULL, target,
		    TO_CHAR);
		act(AT_RED, "$n try to deceive $N.", ch, NULL, target, TO_ROOM);
		break;
	}

	if ((number_range(20, 120) - get_curr_cha(ch) / 5 -
	     ch->pcdata->learned[gsn_deceive]) < 1) {
...
*/
	...
	act(AT_PLAIN, "$n tries to calm down $N.", ch, NULL, victim,
	    TO_NOTVICT);
	act(AT_PLAIN, "$n tries to calm down you.", ch, NULL, victim, TO_VICT);

	if ((number_range(20, 120) - get_curr_cha(ch) / 5 -
	     ch->pcdata->learned[gsn_deceive]) < 1) {
		act(AT_PLAIN, "You fail at calming $N!", ch, NULL, victim,
		    TO_CHAR);
		act(AT_PLAIN, "$n fails at calming $N!", ch, NULL, victim,
		    TO_NOTVICT);
		act(AT_PLAIN, "$n fails at calming you.", ch, NULL, victim,
		    TO_VICT);
...
void do_appeal(CHAR_DATA * ch, char *argument)
{
	...
	if (clan->clan_type == 1) {
		send_to_char
		    ("You can not donate money to autonomous groups!\r\n", ch);
		return;
	}
	...
	if ((number_range(20, 120) - get_curr_cha(ch) / 5 -
	     ch->pcdata->learned[gsn_deceive]) < 1) {
		do_say(victim,
		       "Ugh, I hate it when people ask me to donate money to a clan!");
		if (number_range(0, 3) == 3)
			victim->frustrated_by = ch;
		victim->frustration += number_range(0, 6);
		return;
	...
	ch_printf(ch, "%s has donated %ld to %s!", victim->name, amount,
		  clan->name);
	clan->funds += amount;
	victim->gold -= amount;
...
void do_support(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->frustrated_by == ch) {
		...
		act(AT_PLAIN, "$N ignores $n's asking for help!", ch, NULL,
		    victim, TO_NOTVICT);
		return;
	}
	...
	if (number_range(0, 140) - get_curr_cha(ch) / 3 -
	    ch->pcdata->learned[gsn_support] > 0) {
...
void do_plead(CHAR_DATA * ch, char *argument)
{
	...
	if ((number_range(0, 5) + get_curr_cha(ch) / 3 -
	     victim->frustration / 2) < 5) {
...
void do_lobby(CHAR_DATA * ch, char *argument)
{
	...
	if (victim->frustration > 0) {
		do_say(victim,
		       "I am far too frustrated to talk about politics!");
		return;
	}
	...
	if (planet->governed_by && planet->governed_by == clan) {
		send_to_char
		    ("That clan can not be used! They own the planet!\r\n", ch);
		return;
	}
	...
	if (percent - get_curr_cha(ch) + victim->top_level >
	    ch->pcdata->learned[gsn_lobby] || number_range(0, 10) > 8) {
	...
	if (planet->frustrated_by == clan) {
		ch_printf(ch,
			  "The people of %s grow more frustrated towards %s.\r\n",
		...
		ch_printf(ch,
			  "The people of %s grow less frustrated at %s and more at %s.",
			  planet->name, planet->frustrated_by->name,
			  clan->name);
	} else {
		planet->frustrated_by = clan;
		planet->frust_level += .1 + ch->top_level / 50;
		ch_printf(ch, "The people of %s grow frustrated at %s!",
			  planet->name, clan->name);
	}
