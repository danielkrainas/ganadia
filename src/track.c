...
void found_prey(CHAR_DATA * ch, CHAR_DATA * victim)
{
	...
	if (!can_see(ch, victim)) {
		...
		case 0:
			...
			break;
		case 1:
			act(AT_ACTION, "$n sniffs around the room for $N.", ch,
			    NULL, victim, TO_NOTVICT);
			act(AT_ACTION, "You sniff around the room for $N.", ch,
			    NULL, victim, TO_CHAR);
			act(AT_ACTION, "$n sniffs around the room for you.", ch,
			    NULL, victim, TO_VICT);
	...
	if (IS_SET(ch->in_room->room_flags, ROOM_SAFE)) {
		...
		case 2:
			...
			break;
		case 3:
			act(AT_ACTION, "$n takes a few swipes at $N.", ch, NULL,
			    victim, TO_NOTVICT);
