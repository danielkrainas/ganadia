...
char *const crystal_table[8] = {
	"non-adegan", "kathracite", "relacite", "danite", "mephite", "ponite",
	"illum", "corusca"
};
...
char *const r_flags[] = {
	...
	"donation", "nodropall", "silence", "logspeech", "nodrop",
	    "clanstoreroom",
	"plr_home", "empty_home", "teleport", "hotel", "nofloor", "refinery",
	"factory",
	"republic_recruit", "empire_recruit", "spacecraft", "prototype",
	    "auction"
};
...
char *const o_flags[] = {
	...
	"antisoldier", "antithief", "antihunter", "antijedi", "small_size",
	"large_size",
...
char *const item_w_flags[] = {
	"take", "finger", "neck", "neck", "neck", "body", "head", "legs",
	    "feet",
	"hands", "arms", "shield", "about", "waist", "wrist", "wrist", "wield",
	"hold", "dual", "ears", "eyes", "missile", "back", "face", "ankle",
	    "ankle",
...
char *const o_types[] = {
	...
	"missile",
	"treasure", "armor", "potion", "_worn", "furniture", "trash",
	    "_oldtrap",
	"container", "_note", "drinkcon", "key", "food", "money", "pen",
	    "_boat",
	"corpse", "corpse_pc", "fountain", "pill", "blood", "_bloodstain",
	"scraps", "_pipe", "_herbcon", "_herb", "_incense", "fire", "book",
	"switch",
	"lever", "_pullchain", "button", "dial", "_rune", "_runepouch",
	    "_match",
	"trap",
	"map", "_portal", "paper", "_tinder", "lockpick", "_spike", "_disease",
	"_oil",
	"fuel", "_shortbow", "_longbow", "_crossbow", "ammo", "_quiver",
	    "shovel",
	"salve", "rawspice", "lens", "crystal", "duraplast", "battery",
	"toolkit", "durasteel", "oven", "mirror", "circuit", "superconductor",
	"comlink", "medpac",
	"fabric", "rare_metal", "magnet", "thread", "spice", "smut", "device",
	"spacecraft",
...
char *const a_types[] = {
	"none", "strength", "dexterity", "intelligence", "wisdom",
	    "constitution",
	"sex", "null", "level", "age", "height", "weight", "force", "hit",
	    "move",
...
char *const a_flags[] = {
	...
	"detect_hidden", "weaken", "sanctuary", "faerie_fire", "infrared",
	    "curse",
...
char *const act_flags[] = {
	"npc", "sentinel", "scavenger", "day", "night", "aggressive",
	    "stayarea",
...
char *const pc_flags[] = {
	...
	"guest",
	"nosummon", "pageron", "notitled", "room", "rpauth", "outlaw",
	    "exercise",
	"toggle",
	"autofocus",
	"w", "bankfreeze", "r13",
...
char *const plr_flags[] = {
	...
	"wizinvis", "roomvnum", "silence", "noemote", "attacker", "notell",
	    "log",
...
char *const trap_flags[] = {
	...
	"unlock", "north", "south", "east", "r1", "west", "up", "down",
	    "examine",
...
char *const ris_flags[] = {
	"fire", "cold", "electricity", "energy", "blunt", "pierce", "slash",
	    "acid",
	"poison", "drain", "sleep", "charm", "hold", "nonmagic", "plus1",
	    "plus2",
...
char *const trig_flags[] = {
	"up", "unlock", "lock", "d_north", "d_south", "d_east", "d_west",
	    "d_up",
...
char *const part_flags[] = {
	...
	"sharpscales", "beak", "haunches", "hooves", "paws", "forelegs",
	    "feathers",
	"r1", "r2"
};
...
void do_goto(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_SET(ch->act, PLR_WIZINVIS))
		if (ch->pcdata && ch->pcdata->bamfin[0] != '\0')
			...
			act(AT_IMMORT, "$n $T", ch, NULL,
			    "enters in a swirl of the Force.", TO_ROOM);
...
*/
	if (arg1[0] == '\0'
	    || (arg2[0] == '\0' && ch->substate != SUB_REPEATCMD)
	    || !str_cmp(arg1, "?")) {
		if (ch->substate == SUB_REPEATCMD) {
			if (victim)
 ...
 */
	...
	if (!str_cmp(arg2, "sav1")) {
		...
		if (value < -30 || value > 30) {
			send_to_char
			    ("Saving throw range vs poison is -30 to 30.\n\r",
			     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "sav2")) {
		...
		if (value < -30 || value > 30) {
			send_to_char
			    ("Saving throw range vs wands is -30 to 30.\n\r",
			     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "sav3")) {
		...
		if (value < -30 || value > 30) {
			send_to_char
			    ("Saving throw range vs para is -30 to 30.\n\r",
			     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "sav4")) {
		...
		if (value < -30 || value > 30) {
			send_to_char
			    ("Saving throw range vs bad breath is -30 to 30.\n\r",
			     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "qp")) {
		...
		if (value < 0 || value > 5000) {
			send_to_char
			    ("The current quest point range is 0 to 5000.\n\r",
			     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "flags")) {
		...
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Usage: mset <victim> flags <flag> [flag]...\n\r",
			     ch);
			send_to_char
			    ("sentinel, scavenger, aggressive, stayarea, wimpy, practice, immortal,\n\r",
			     ch);
			send_to_char
		...
		while (argument[0] != '\0') {
			...
			value =
			    IS_NPC(victim) ? get_actflag(arg3) :
			    get_plrflag(arg3);
	...
	if (!str_cmp(arg2, "wanted")) {
		...
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Usage: mset <victim> wanted <planet> [planet]...\n\r",
			     ch);
			return;
		}
		...
		while (argument[0] != '\0') {
			...
			if (value < 0 || value > 31)
				...
				TOGGLE_BIT(victim->pcdata->wanted_flags,
					   1 << value);
		}
		return;
	}
	...
	if (!str_cmp(arg2, "resistant")) {
		...
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Usage: mset <victim> resistant <flag> [flag]...\n\r",
			     ch);
			return;
		}
		while (argument[0] != '\0') {
	...
	if (!str_cmp(arg2, "part")) {
		if (!IS_NPC(victim) && get_trust(ch) < LEVEL_LESSER) {
			send_to_char
			    ("You can only modify a mobile's parts.\n\r", ch);
			return;
		}
		if (!can_mmodify(ch, victim))
			return;
	...
	if (!str_cmp(arg2, "defense")) {
		...
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Usage: mset <victim> defense <flag> [flag]...\n\r",
			     ch);
	...
	if (!str_cmp(arg2, "hitnumdie")) {
		...
		if (value < 0 || value > 32767) {
			send_to_char
			    ("Number of hitpoint dice range is 0 to 30000.\n\r",
			     ch);
			return;
		}
		if (IS_NPC(victim) && IS_SET(victim->act, ACT_PROTOTYPE))
	...
	if (!str_cmp(arg2, "hitsizedie")) {
		...
		if (value < 0 || value > 32767) {
			send_to_char
			    ("Hitpoint dice size range is 0 to 30000.\n\r", ch);
			return;
		}
		if (IS_NPC(victim) && IS_SET(victim->act, ACT_PROTOTYPE))
	...
	if (!str_cmp(arg2, "damnumdie")) {
		...
		if (value < 0 || value > 100) {
			send_to_char
			    ("Number of damage dice range is 0 to 100.\n\r",
			     ch);
			return;
		}
		if (IS_NPC(victim) && IS_SET(victim->act, ACT_PROTOTYPE))
	...
	if (!str_cmp(arg2, "speaks")) {
		...
		while (argument[0] != '\0') {
			...
			if (value == LANG_UNKNOWN)
				...
				int valid_langs =
				    LANG_COMMON | LANG_WOOKIEE | LANG_TWI_LEK |
				    LANG_RODIAN | LANG_HUTT | LANG_MON_CALAMARI
				    | LANG_NOGHRI | LANG_GAMORREAN | LANG_JAWA |
				    LANG_ADARIAN | LANG_EWOK | LANG_VERPINE |
				    LANG_DEFEL | LANG_TRANDOSHAN |
				    LANG_CHADRA_FAN | LANG_QUARREN |
				    LANG_DUINUOGWUIN;

				if (!(value &= valid_langs)) {
					ch_printf(ch,
						  "Players may not know %s.\n\r",
						  arg3);
					continue;
				}
			}
		...
		if (!IS_NPC(victim)) {
			REMOVE_BIT(victim->speaks,
				   race_table[victim->race].language);
	...
	if (ch->substate == SUB_REPEATCMD) {
		...
		if (argument[0] == '\0' || !str_cmp(argument, " ")
			...
			if (obj)
				...
				send_to_char
				    ("No object selected.  Type '?' for help.\n\r",
				     ch);
			return;
		}
		if (!str_cmp(argument, "done") || !str_cmp(argument, "off")) {
...
*/
	...
	if (!obj && get_trust(ch) <= LEVEL_IMMORTAL) {
		...
		if ((obj = get_obj_world(ch, arg1)) == NULL) {
			send_to_char
			    ("There is nothing like that in all the realms.\n\r",
			     ch);
			return;
		}
	}
...
*/
	...
	if (!str_cmp(arg2, "type")) {
		...
		if (!argument || argument[0] == '\0') {
			...
			send_to_char
			    ("Treasure    Armor      Comlink    Fabric      Grenade\n\r",
			     ch);
			send_to_char
			    ("Furniture   Trash      Container  Drink_con   Landmine\n\r",
			     ch);
			send_to_char
			    ("Key         Food       Money      Pen         Fuel\n\r",
			     ch);
			send_to_char
			    ("Fountain    Pill       Weapon     Medpac      Missile\n\r",
			     ch);
			send_to_char
			    ("Fire        Book       Superconductor         Rare_metal\n\r",
			     ch);
			send_to_char
			    ("Switch      Lever      Button     Dial        Government\n\r",
			     ch);
			send_to_char
			    ("Trap        Map        Portal     Paper       Magnet\n\r",
			     ch);
			send_to_char
			    ("Lockpick    Shovel     Thread     Smut        Ammo\n\r",
			     ch);
			send_to_char
			    ("Rawspice    Lens       Crystal    Duraplast   Battery\n\r",
			     ch);
			send_to_char
			    ("Toolkit     Durasteel  Oven       Mirror      Circuit\n\r",
			     ch);
			send_to_char
			    ("Potion      Salve      Pill       Device      Spacecraft\n\r",
			     ch);
			send_to_char
			    ("Bolt        Chemical   Applicator Computer\n\r",
			     ch);
			return;
		}
	...
	if (!str_cmp(arg2, "wear")) {
		...
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Usage: oset <object> wear <flag> [flag]...\n\r",
			     ch);
			...
			send_to_char
			    ("feet   hands    arms    shield  about  waist\n\r",
			     ch);
			send_to_char
			    ("wrist  wield    hold    ears    eyes   ankle\n\r",
			     ch);
	...
	if (!str_cmp(arg2, "affect")) {
		...
		if (!arg2 || arg2[0] == '\0' || !argument || argument[0] == 0) {
			send_to_char
			    ("Usage: oset <object> affect <field> <value>\n\r",
			     ch);
	...
	if (!str_cmp(arg2, "rmaffect")) {
		...
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Usage: oset <object> rmaffect <affect#>\n\r", ch);
			return;
		}
		loc = atoi(argument);
		if (loc < 1) {
	...
	if (!str_cmp(arg2, "rmed")) {
		if (!arg3 || arg3[0] == '\0') {
			send_to_char
			    ("Syntax: oset <object> rmed <keywords>\n\r", ch);
			return;
		}
		if (IS_OBJ_STAT(obj, ITEM_PROTOTYPE)) {
	...
	case ITEM_WEAPON:
		if (!str_cmp(arg2, "weapontype")) {
			...
			value = -1;
			for (x = 0;
			     x < sizeof(weapon_table) / sizeof(weapon_table[0]);
			     x++)
			...
			if (value < 0) {
				...
				send_to_char("\n\rChoices:\n\r", ch);
				send_to_char
				    ("   none, sword, knife, rifle, heavy weapon, pistol, bludgeon\n\r",
				     ch);
				return;
			}
			tmp = 3;
	...
	case ITEM_SPICE:
		...
		if (!str_cmp(arg2, "spicetype")) {
			...
			value = -1;
			for (x = 0;
			     x < sizeof(spice_table) / sizeof(spice_table[0]);
			     x++)
			...
			if (value < 0) {
				...
				send_to_char("\n\rChoices:\n\r", ch);
				send_to_char
				    ("   bloodyeye, carsanum, ryll, andris\n\r",
				     ch);
				return;
			}
			tmp = 0;
...
char *sprint_reset(CHAR_DATA * ch, RESET_DATA * pReset, sh_int num, bool rlist)
{
	...
	case 'M':
		...
		sprintf(buf, "%2d) %s (%d) -> %s (%d) [%d]\n\r",
			num,
			mobname, pReset->arg1, roomname, pReset->arg3,
			pReset->arg2);
		break;
	case 'E':
		if (!mob)
		...
		sprintf(buf, "%2d) %s (%d) -> %s (%s) [%d]\n\r",
			num,
			objname,
			pReset->arg1, mobname, wear_locs[pReset->arg3],
			pReset->arg2);
		break;
	case 'H':
		if (pReset->arg1 > 0
	...
	case 'P':
		...
		sprintf(buf, "%2d) (Put) %s (%d) -> %s (%d) [%d]\n\r",
			...
			objname,
			pReset->arg1,
			roomname, obj ? obj->vnum : pReset->arg3, pReset->arg2);
		break;
	case 'D':
		...
		sprintf(buf, "%2d) %s [%d] the %s [%d] door %s (%d)\n\r",
			num,
			mobname,
			pReset->arg3, objname, pReset->arg2, roomname,
			pReset->arg1);
		break;
	case 'R':
		if ((room = get_room_index(pReset->arg1)) == NULL)
	...
	case 'T':
		sprintf(buf, "%2d) TRAP: %d %d %d %d (%s)\n\r",
			...
			pReset->arg1,
			pReset->arg2,
			pReset->arg3, flag_string(pReset->extra, trap_flags));
		break;
	}
...
*/
	...
	if (!str_cmp(arg, "rlist")) {
		...
		for (pReset = tarea->first_reset; pReset; pReset = pReset->next) {
			num++;
			if ((bptr =
			     sprint_reset(ch, pReset, num, TRUE)) == NULL)
				continue;
		...
		while (argument[0] != '\0') {
			...
			if (value < 0 || value > 31)
				ch_printf(ch, "Unknown flag: %s\n\r", arg2);
				 ...
				 && get_trust(ch) < LEVEL_IMPLEMENTOR)
				send_to_char
	...
	if (!str_cmp(arg, "flags2")) {
		...
		while (argument[0] != '\0') {
			...
			if (value < 0 || value > 31)
				...
				send_to_char
				    ("There must be a plr_home flag to set the visitors flag",
				     ch);
			else
	...
	if (!str_cmp(arg, "teledelay")) {
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Set the delay of the teleport. (0 = off).\n\r",
			     ch);
	...
	if (!str_cmp(arg, "televnum")) {
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Set the vnum of the room to teleport to.\n\r",
			     ch);
	...
	if (!str_cmp(arg, "exflags")) {
		if (!argument || argument[0] == '\0') {
			...
			send_to_char
			    ("Usage: redit exflags <dir> <flag> [flag]...\n\r",
			     ch);
			send_to_char("\n\rExit flags:\n\r", ch);
			send_to_char
	...
	if (!str_cmp(arg, "exit")) {
		...
		if (addexit || !xit) {
			if (numnotdir) {
				send_to_char
				    ("Cannot add an exit by number, sorry.\n\r",
				     ch);
				return;
			}
			if (addexit && xit
			    && get_exit_to(location, edir, tmp->vnum)) {
				send_to_char
			...
			xit->key = -1;
			xit->exit_info = 0;
			act(AT_IMMORT, "$n reveals a hidden passage!", ch, NULL,
			    NULL, TO_ROOM);
		} else
		...
		if (xit->to_room != tmp) {
			...
			xit->vnum = evnum;
			texit =
			    get_exit_to(xit->to_room, rev_dir[edir],
					location->vnum);
			if (texit) {
				texit->rexit = xit;
				xit->rexit = texit;
	...
	if (!str_cmp(arg, "exdesc")) {
		...
		if (!arg2 || arg2[0] == '\0') {
			send_to_char
			    ("Create or clear a description for an exit.\n\r",
			     ch);
			send_to_char
			    ("Usage: redit exdesc <dir> [description]\n\r", ch);
			return;
		}
		if (arg2[0] == '#') {
			edir = atoi(arg2 + 1);
...
void edit_buffer(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '/' || argument[0] == '\\') {
		...
		if (!str_cmp(cmd + 1, "?")) {
			...
			send_to_char
			    ("/f <format>     format text in buffer\n\r", ch);
...
void temp_fold_area(AREA_DATA * area)
{
	char buf[MAX_STRING_LENGTH];
	EXTRA_DESCR_DATA * ed;
	EXIT_DATA *xit;
	...
	DATA_CONN * dc;
	DATA_ENTRY * entry;
	DATA_ENTRY * groupedEntry;
	DATA_ENTRY * subEntry;
	DATA_ENTRY areaEntry[1];
	if((dc = dopen()) == NULL) {
		bug("temp_fold_area: could not connect to database", 0);
		return;
	}

	dentry_init(areaEntry);
	if(area->id) {
		dwrite_id(DKEY_ID, area->id, areaEntry);
	}

	dwrite_string("name", area->name, areaEntry);
	dwrite_string("author", area->author, areaEntry);
	dwrite_number("flags", area->flags, areaEntry);
	dwrite_number("highEconomy", area->high_economy, areaEntry);
	dwrite_number("lowEconomy", area->low_economy, areaEntry);
	if(area->resetmsg)
		dwrite_string("resetMsg", area->resetmsg, areaEntry);

	if(area->reset_frequency)
		dwrite_number("resetFrequency", area->reset_frequency, areaEntry);

	dwrite_push_array("rooms", areaEntry);
	for (vnum = area->low_r_vnum; vnum <= area->hi_r_vnum; vnum++) {
		...
		dentry_init(entry);
		if(room->id) {
			dwrite_array_string(roomCounter++, room->id, areaEntry);
			dwrite_id(DKEY_ID, room->id, entry);
		}

		dwrite_number("vnum", vnum, entry);
		dwrite_string("name", room->name, entry);
		dwrite_string("description", strip_cr(room->description), entry);
		dwrite_number("flags", room->room_flags, entry);
		dwrite_number("flags2", room->room_flags2, entry);
		dwrite_number("sectorType", room->sector_type, entry);
		if ((room->tele_delay > 0 && room->tele_vnum > 0)
		    || room->tunnel > 0) 
		{
			dwrite_number("teleVnum", room->tele_vnum, entry);
			dwrite_number("teleDelay", room->tele_delay, entry);
			dwrite_number("tunnel", room->tunnel, entry);
		}

		counter = -1;
		dwrite_push_array("exits", entry);
		for (xit = room->first_exit; xit; xit = xit->next) 
		{
			if (IS_SET(xit->exit_info, EX_PORTAL))	/* don't fold portals */
				continue;

			dwrite_array_push_object(++counter, entry);
			dwrite_number("door", xit->vdir, entry);
			dwrite_string("description", strip_cr(xit->description), entry);
			dwrite_string("keyword", strip_cr(xit->keyword), entry);
			dwrite_number("info", xit->exit_info & ~EX_BASHED, entry);
			dwrite_number("key", xit->key, entry);
			dwrite_number("vnum", xit->vnum, entry);
			if(xit->distance > 1)
				dwrite_number("distance", xit->distance, entry);
			
			dwrite_pop_object(entry);
		}

		dwrite_pop_array(entry);
		counter = -1;
		dwrite_push_array("extraDescriptions", entry);
		for (ed = room->first_extradesc; ed; ed = ed->next)
		{
			dwrite_array_push_object(++counter, entry);
			dwrite_string("keyword", ed->keyword, entry);
			dwrite_string("description", strip_cr(ed->description), entry);
			dwrite_pop_object(entry);
		}
...
//		if (room->map) {	/* maps */
#ifdef OLDMAPS
//			dwrite_string("map", strip_cr(room->map), subEntry);
#endif
/*			dwrite_number("map", 
			fprintf(fpout, "M %d %d %d %c\n", room->map->vnum,
				room->map->x, room->map->y, room->map->entry);
		}*/
/*		if (room->mudprogs) {
...
//		fprintf(fpout, "S\n");
		...
		dentry_save(entry, DSET_ROOMS, dc);
		dentry_destroy(entry);
		DISPOSE(entry);
	}

	dwrite_pop_array(areaEntry);	
	dentry_finalize(areaEntry);
	dentry_save(areaEntry, DSET_AREAS, dc);
	dentry_destroy(areaEntry);	
	dclose(dc);
}
...
void fold_area(AREA_DATA * tarea, char *filename, bool install)
{
	...
	temp_fold_area(tarea);// temp save area
	...
	fprintf(fpout, "%d %d %d %d %d %d %d %d\n", tarea->low_soft_range,
		...
		tarea->hi_hard_range,
		tarea->generatecit, tarea->rareeq, tarea->loweq, tarea->higheq);
	fprintf(fpout, "$\n\n");
	...
	for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++) {
		...
		if (complexmob) {
			...
			fprintf(fpout, "%d 0 %d %d %d %d %d\n", pMobIndex->race,
				pMobIndex->height, pMobIndex->weight,
				pMobIndex->speaks, pMobIndex->speaking,
				pMobIndex->numattacks);
			fprintf(fpout, "%d %d %d %d %d %d %d %d\n",
				pMobIndex->hitroll, pMobIndex->damroll,
				pMobIndex->xflags, pMobIndex->resistant,
				pMobIndex->immune, pMobIndex->susceptible,
				pMobIndex->attacks, pMobIndex->defenses);
			fprintf(fpout, "%d 0 0 0 0 0 0 0\n",
				pMobIndex->vip_flags);
		}
		if (pMobIndex->mudprogs) {
			for (mprog = pMobIndex->mudprogs; mprog;
			     mprog = mprog->next)
				fprintf(fpout, "> %s %s~\n%s~\n",
					mprog_type_to_name(mprog->type),
					mprog->arglist,
	...
	for (vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++) {
		...
		if (pObjIndex->mudprogs) {
			for (mprog = pObjIndex->mudprogs; mprog;
			     mprog = mprog->next)
				fprintf(fpout, "> %s %s~\n%s~\n",
					mprog_type_to_name(mprog->type),
					mprog->arglist,
	...
	for (vnum = tarea->low_r_vnum; vnum <= tarea->hi_r_vnum; vnum++) {
		...
		if (install) {
			...
			for (victim = room->first_person; victim;
			     victim = vnext) {
		...
		if ((room->tele_delay > 0 && room->tele_vnum > 0)
			...
			fprintf(fpout, "0 %d %d %d %d %d %d\n",
				...
				room->room_flags2, room->tele_delay,
				room->tele_vnum, room->tunnel);
		else
	...
	for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++) {
		...
		fprintf(fpout, " %d   %2d %2d %2d %2d %2d   %3d %3d",
			...
			pShop->buy_type[2],
			pShop->buy_type[3],
			pShop->buy_type[4], pShop->profit_buy,
			pShop->profit_sell);
	...
	for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++) {
		...
		fprintf(fpout, " %d   %2d %2d %2d         %3d %3d",
			...
			pRepair->fix_type[0],
			pRepair->fix_type[1],
			pRepair->fix_type[2], pRepair->profit_fix,
			pRepair->shop_type);
		fprintf(fpout, "        %2d %2d    ; %s\n", pRepair->open_hour,
...
void do_savearea(CHAR_DATA * ch, char *argument)
{
	...
	if (argument[0] == '\0')
		...
		for (found = FALSE, tarea = first_build; tarea;
		     tarea = tarea->next)
	...
	for (tarea = first_build; tarea; tarea = tarea->next) {
		if (!str_cmp(tarea->filename, arg)) {
			...
			send_to_char("Renaming author's building file.\n\r",
				     ch);
			sprintf(buf, "%s%s.installed", BUILD_DIR,
				tarea->filename);
...
RESET_DATA *parse_reset(AREA_DATA * tarea, char *argument, CHAR_DATA * ch)
{
	...
	if (arg1[0] == '\0') {
		send_to_char
		    ("Reset commands: mob obj give equip door rand trap hide.\n\r",
		     ch);
		return NULL;
	}
	...
	if (!str_cmp(arg1, "hide")) {
		...
		if (val3 < 0 || val3 > 2) {
			send_to_char
			    ("Reset: DOOR: invalid door state (0 = open, 1 = close, 2 = lock)\n\r",
			     ch);
			return NULL;
		}
		letter = 'D';
...
void do_astat(CHAR_DATA * ch, char *argument)
{
	...
	if (!proto) {
		ch_printf(ch,
			  "Max players: %d  IllegalPks: %d  Credits Looted: %d\n\r",
			  tarea->max_players, tarea->illegal_pk,
			  tarea->gold_looted);
		if (tarea->high_economy)
			ch_printf(ch,
				  "Area economy: %d billion and %d credits.\n\r",
		...
		if (tarea->planet) {
			...
			if (tarea->generatecit == 1) {
				ch_printf(ch, "Vnums: \r\n");
				ch_printf(ch,
					  "Rare EQ: %d    Low EQ Range: %d   High EQ Range: %d\r\n",
					  tarea->rareeq, tarea->loweq,
					  tarea->higheq);
			}
		}
		ch_printf(ch,
			  "Mdeaths: %d  Mkills: %d  Pdeaths: %d  Pkills: %d\n\r",
...
void do_aset(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0' || arg2[0] == '\0') {
		...
		send_to_char("\n\rField being one of:\n\r", ch);
		send_to_char
		    ("  low_room hi_room low_obj hi_obj low_mob hi_mob\n\r",
		     ch);
		send_to_char
		    ("  name filename low_soft hi_soft low_hard hi_hard\n\r",
		     ch);
		send_to_char
		    ("  author resetmsg resetfreq flags planet generatecit\n\r",
		     ch);
		...
		if (planet) {
			if (tarea->planet) {
				...
				UNLINK(tarea, old_planet->first_area,
				       old_planet->last_area, next_on_planet,
				       prev_on_planet);
			}
			tarea->planet = planet;
			LINK(tarea, planet->first_area, planet->last_area,
	...
	if (!str_cmp(arg2, "flags")) {
		if (!argument || argument[0] == '\0') {
			send_to_char
			    ("Usage: aset <filename> flags <flag> [flag]...\n\r",
			     ch);
			return;
		}
		while (argument[0] != '\0') {
...
void do_rpedit(CHAR_DATA * ch, char *argument)
{
	...
	if (arg1[0] == '\0') {
		...
		send_to_char("\n\r", ch);
		send_to_char
		    ("You should be standing in room you wish to edit.\n\r",
		     ch);
		return;
	}
