   ...
   for (i=1; i <= 200; i++)
    {
        ...
        if(count)
        {
                pager_printf( ch, "Your are almost able to learn skill%s: ",
                                (number > 1)? "s" : "");
                send_to_char( "\n\r", ch);
        }
   for (i=1; i <= 200; i++)
   ...
   for (i=1; i <= 200; i++)
    {
        ...
        if(count)
        {
                pager_printf( ch, "Your are getting near able to learn skill%s: ",
                                (number > 1)? "s" : "");
                send_to_char( "\n\r", ch);
        }
   for (i=1; i <= 200; i++)
...
void char_update( void )
{
	...
	if(!IS_NPC(ch) && bounty && ch->pcdata && ch->pcdata->clan
	...
	if( !IS_NPC(ch) && ch->pcdata->bountyrelease > 0 && ch->desc && ch->pcdata->release_date <= 0)
	{
