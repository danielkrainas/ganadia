...
/*    int level = 0;
    ...
    if ( ability == COMBAT_ABILITY )
    {
       ...
       if ( ch->race == RACE_WOOKIEE ) level += 30;
       ...
       if ( ch->race == RACE_DUINUOGWUIN ) level += 20;
    ...
    if ( ability == PILOTING_ABILITY )
    {
       ...
       if ( ch->race == RACE_DUINUOGWUIN ) level -= 30;
       level += ch->perm_dex*2;
    }
    ...
    if ( ability == SMUGGLING_ABILITY )
    {
       ...
       if ( ch->race == RACE_DUINUOGWUIN ) level -= 10;
      level += ch->perm_lck*2;
    }
...
void bank_update()
{
	...
	for (ch = last_char; ch; ch = gch_prev) {
		...
		if (!IS_NPC(ch) && ch->pcdata
		    && IS_SET(ch->pcdata->flags, PCFLAG_FREEZE)) {
			ch_printf(ch,
				  "&g[&zMonthly Bank Report&g] &WBank Freeze.\r\n");
		}
		...
		if (!IS_NPC(ch) && !IS_SET(ch->pcdata->flags, PCFLAG_FREEZE)) {
			...
			sprintf(buf,
				"&g[&zMonthly Bank Report&g] &W%s &ZYou made&g: &W%d dollars this month.\n\r",
				ch->name, value2);
			send_to_char(buf, ch);
			...
			if (ch->pcdata->clan) {
				if (!str_cmp(ch->pcdata->clan->name, "ISSP")) {
					...
					sprintf(buf,
						"&b[&zMonthly Salary Report&b] &W%s &ZYou made&b: &W%d dollars this month.\n\r",
						ch->name, value2);
					send_to_char(buf, ch);
				}
				if (!str_cmp(ch->pcdata->clan->name, "WTS")
					...
					ch->pcdata->bank += value2;
					sprintf(buf,
						"&r[&zMonthly Earnings Report&r] &W%s &ZYou made&r: &W%d dollars this month.\n\r",
						ch->name, value2);
					send_to_char(buf, ch);
				}
			}
...
void gain_exp(CHAR_DATA * ch, int gain, int ability)
{
	...
	while (ch->experience[ability] >=
		...
		case 6:
			ch_printf(ch,
				  "You feel more confident in your ability in %s.\r\n",
				  supercapitalize(ability_name[ability]));
			break;
		case 5:
			ch_printf(ch,
				  "%s seems slightly easier for you now.\r\n",
				  supercapitalize(ability_name[ability]));
			break;
		case 4:
			ch_printf(ch,
				  "You understand %s slightly better now.\r\n",
				  supercapitalize(ability_name[ability]));
			break;
		case 3:
			ch_printf(ch,
				  "You feel slightly more knowledgable in %s.\r\n",
				  supercapitalize(ability_name[ability]));
			break;
		case 2:
			ch_printf(ch,
				  "You gain more experience in the field of %s.\r\n",
				  supercapitalize(ability_name[ability]));
			break;
		case 1:
			ch_printf(ch, "You get more skilled in %s.\r\n",
				  supercapitalize(ability_name[ability]));
			break;
		default:
			ch_printf(ch,
				  "You feel your body and mind get a little wiser in %s.\r\n",
				  supercapitalize(ability_name[ability]));
			break;
		}
		...
		if (count) {
			pager_printf(ch,
				     "You can now learn the following skill%s: ",
				     (number > 1) ? "s" : "");
			send_to_char("\n\r", ch);
		}
		for (i = 1; i <= 200; i++) {
			for (sn = 0; sn < top_sn; sn++) {
				...
				if (i == skill_table[sn]->min_level
				    && ch->skill_level[ability] ==
				    skill_table[sn]->min_level && count) {
					pager_printf(ch, "&W&R%s&W ",
						     supercapitalize(skill_table
								     [sn]->
								     name));
				}
			}
		...
		if (count) {
			pager_printf(ch,
				     "Your are almost able to learn skill%s: ",
				     (number > 1) ? "s" : "");
			send_to_char("\n\r", ch);
		}
		for (i = 1; i <= 200; i++) {
		...
		if (count) {
			pager_printf(ch,
				     "Your are getting near able to learn skill%s: ",
				     (number > 1) ? "s" : "");
			send_to_char("\n\r", ch);
		}
		for (i = 1; i <= 200; i++) {
		...
		if (count) {
			pager_printf(ch,
				     "You have a little ways to go to be able to learn skill%s: ",
				     (number > 1) ? "s" : "");
			send_to_char("\n\r", ch);
		}
		for (i = 1; i <= 200; i++) {
		...
		if (ch->skill_level[ability] == 200)
			send_to_char
			    ("&WYou have now maxed out your level in this field!\n\r",
			     ch);
		if (ch->skill_level[ability] == 150
		    && ability == DIPLOMACY_ABILITY)
			send_to_char
			    ("&RYou can now use Broadcast! (help broadcast)\r\n",
			     ch);
	...
	for (drug = 0; drug <= 9; drug++) {
		...
		if (ch->pcdata->addiction[drug] >
			...
			if (ch->hit < 0 && ch->position != POS_STUNNED) {
				act(AT_ACTION,
				    "$n keels over from a drug overdose.", ch,
				    NULL, NULL, TO_ROOM);
				act(AT_ACTION,
				    "You keel over from a drug overdose.", ch,
				    NULL, NULL, TO_CHAR);
				update_pos(ch);
			}
		}
...
/** Extract Day and Night mobs ** Rythmic **/
		...
		if (IS_SET(ch->act, ACT_DAY) &&
			...
			act(AT_ACTION, "$n notices the sun setting and leaves.",
		...
		if (IS_SET(ch->act, ACT_NIGHT) &&
			...
			act(AT_ACTION, "$n notices the sun rising and leaves.",
			    ch, NULL, NULL, TO_ROOM);
			extract_char(ch, TRUE, FALSE);
			continue;
		}
		...
		if (IS_NPC(ch) && ch->frustration > 10
		    && number_range(0, 30) > 29)
			ch->frustration--;
		if (IS_NPC(ch) && ch->frustration > 0
		    && number_range(0, 70) > 65)
			ch->frustration--;
...
/* Clean up 'animated corpses' that are not charmed' - Scryn */
		...
		if (!IS_SET(ch->act, ACT_RUNNING)
		    && !IS_SET(ch->act, ACT_SENTINEL) && !ch->fighting
		    && ch->hunting) {
			if (ch->top_level < 20)
	...
	for (ch = last_char; ch; ch = gch_prev) {
		...
		if (!IS_NPC(ch) && ch->top_level < LEVEL_IMMORTAL) {
			...
			if ((obj = get_eq_char(ch, WEAR_LIGHT)) != NULL
				...
				if (--obj->value[2] == 0 && ch->in_room) {
					...
					act(AT_ACTION, "$p goes out.", ch, obj,
					    NULL, TO_ROOM);
					act(AT_ACTION, "$p goes out.", ch, obj,
					    NULL, TO_CHAR);
		...
		if (!char_died(ch)) {
			...
			if (!IS_NPC(ch)) {
				...
				if (ch->pcdata->condition[COND_BLEEDING] > 0) {
					...
					case 1:
						{
							act(AT_BLOOD,
							    "Blood drips slowly from your wounds...",
							    ch, NULL, NULL,
							    TO_CHAR);
							act(AT_BLOOD,
							    "Blood drips slowly from $n's wounds.",
							    ch, NULL, NULL,
							    TO_ROOM);
						}
						break;
					...
					case 3:
						{
							act(AT_BLOOD,
							    "Blood is dripping quickly from your wounds...",
							    ch, NULL, NULL,
							    TO_CHAR);
							act(AT_BLOOD,
							    "Blood is dripping quickly from $n's wounds.",
							    ch, NULL, NULL,
							    TO_ROOM);
						}
						break;
					...
					case 6:
						{
							act(AT_BLOOD,
							    "Blood runs freely from your wounds...",
							    ch, NULL, NULL,
							    TO_CHAR);
							act(AT_BLOOD,
							    "Blood runs freely from $n's wounds.",
							    ch, NULL, NULL,
							    TO_ROOM);
						}
						break;
			...
			if (!IS_NPC(ch) && ch->pcdata->unsilence_date == 0 &&
				...
				send_to_char
				    ("&RYour silence sentence is now over.\n\r",
				     ch);
			...
			if (!IS_NPC(ch) && !IS_IMMORTAL(ch)
				...
				send_to_char
				    ("You have been released from your hell sentance.\n\r",
				     ch);
				do_look(ch, "auto");
			...
			if (!IS_NPC(ch) && ch->pcdata->hackcount > 0
			    && number_range(0, 4) > 2) {
				ch->pcdata->hackcount--;
			}
			BOUNTY_DATA *bounty;
			...
			if (!IS_NPC(ch) && bounty && ch->pcdata->weaponl == 0) {
				ch_printf(ch,
					  "&BISSP tells you 'Hey! Weapon Licenses aren't for criminals!");
				ch->pcdata->weaponl = -1;
			}
			if (!IS_NPC(ch) && !bounty && ch->pcdata->weaponl > 1) {
				...
				if (ch->pcdata->weaponl == 1) {
					ch_printf(ch,
						  "&BISSP tells you 'You may now re-apply for a Weapon License.");
				}
			}
			if (!IS_NPC(ch) && !bounty && ch->pcdata->weaponl == -1) {
				ch_printf(ch,
					  "&BISSP tells you 'After reviewing our files, we will allow you get a new license after a mild time delay.");
				ch->pcdata->weaponl =
				    number_range(ch->top_level / 10,
						 ch->top_level / 2);
			}
			if (!IS_NPC(ch)) {
				if (bounty) {
					ch->pcdata->arresttime++;
					if (ch->pcdata->arresttime >
			...
			if (!IS_NPC(ch) && ch->pcdata->bountyrelease > 0
			    && ch->desc && ch->pcdata->release_date <= 0) {
				ch->pcdata->bountyrelease--;
				if (ch->pcdata->bountyrelease <= 0) {
					send_to_char
					    ("A large man comes in and tells you your sentence is up.\n\r",
					     ch);
					act(AT_PLAIN,
					    "A large man comes in and tells $n $s sentence is up",
					    ch, ch, ch, TO_ROOM);
					char_from_room(ch);
					char_to_room(ch, get_room_index(3543));
					do_look(ch, "");
					act(AT_PLAIN,
					    "$n gets thrown onto the street by a large officer.",
					    ch, ch, ch, TO_ROOM);
				}
			}
			// Fixes a slight bug
			...
			if (!IS_NPC(ch)
			    && (ch->pcdata->afkmessage == NULL
				|| strlen(ch->pcdata->afkmessage) < 5))
			...
			/*if(get_age(ch) > 6 && !IS_NPC(ch) && ch->desc)
			   ...
			   {
			   if(ch->inttrain < 10 && ch->inttrain > 5)
			   ...
			   {
			   if(ch->chatrain < 10 && ch->chatrain > 5)
			...
			sh_int statcount =
			    0, oldstr, olddex, oldint, oldcon, oldcha,
			    oldstatcount;
/*
	if(ch->perm_str > 9)
...
*/
			...
			if (statcount < 75) {
				if (ch->perm_str < 18) {
					if (ch->perm_str == 10) {
						if (ch->strtrain >= 200) {
							send_to_char
							    ("&RYou feel your body grow stronger.\n\r",
							     ch);
							ch->perm_str += 1;
						}
					}
			...
			if (ch->perm_wis < 18) {
				if (ch->perm_wis == 10) {
					if (ch->wistrain >= 100) {
						send_to_char
						    ("&RYou feel your wisdom grow slightly.\n\r",
						     ch);
						ch->perm_wis += 1;
					}
				}
			...
			if (statcount < 75) {
				if (ch->perm_int < 18) {
					if (ch->perm_int == 10) {
						if (ch->inttrain >= 170) {
							send_to_char
							    ("&RYour intelligence expands slightly.\n\r",
							     ch);
							ch->perm_int += 1;
						}
					}
			...
			if (statcount < 75) {
				if (ch->perm_con < 18) {
					if (ch->perm_con == 10) {
						if (ch->contrain >= 200) {
							send_to_char
							    ("&RYour condition increases slightly.\n\r",
							     ch);
							ch->perm_con += 1;
						}
					}
			...
			if (statcount < 75) {
				if (ch->perm_dex < 18) {
					if (ch->perm_dex == 10) {
						if (ch->dextrain >= 450) {
							send_to_char
							    ("&RYou feel your body get more flexible.\n\r",
							     ch);
							ch->perm_dex += 1;
						}
					}
			...
			if (oldstatcount < statcount && statcount == 75) {
				send_to_char
				    ("You feel your body shudder slightly as it can no longer take any more training.\r\n",
				     ch);
			}
			...
			if (ch->position == POS_STANDING && !IS_NPC(ch)) {
				...
				if (ch->hit < 300 && ch->hit > 201) {
					if (ch->alignment > 300) {
						act(AT_RED,
						    "$n grabs $s side and winces in pain!\n\r",
						    ch, NULL, NULL, TO_ROOM);
						act(AT_RED,
						    "You grab your side and wince in pain!\n\r",
						    ch, NULL, NULL, TO_CHAR);
					} else {
						act(AT_RED,
						    "$n snarls slightly as $e grabs his side in pain!\r\n",
						    ch, NULL, NULL, TO_ROOM);
						act(AT_RED,
						    "You snarl slightly as you grab your side in pain!\r\n",
						    ch, NULL, NULL, TO_CHAR);
					}
				}
				if (ch->hit < 200 && ch->hit > 101) {
					if (ch->alignment > 300) {
						act(AT_RED,
						    "$n braces $sself up, barely standing!\n\r",
						    ch, NULL, NULL, TO_ROOM);
						act(AT_RED,
						    "You almost fall, but brace yourself up!\n\r",
						    ch, NULL, NULL, TO_CHAR);
					} else {
						act(AT_RED,
						    "$n stumbles slightly, barely able to stand.\r\n",
						    ch, NULL, NULL, TO_ROOM);
						act(AT_RED,
						    "You stumble slightly, barely able to stand.\r\n",
						    ch, NULL, NULL, TO_CHAR);
					}
				}
				if (ch->hit < 100 && ch->hit > 0) {
					if (ch->alignment > 300) {
						act(AT_RED,
						    "$n violently coughs up some blood!\n\r",
						    ch, NULL, NULL, TO_ROOM);
						act(AT_RED,
						    "You violently cough up some blood!\n\r",
						    ch, NULL, NULL, TO_CHAR);
					} else {
						act(AT_RED,
						    "$n grabs $s chest and vomits up some blood!\r\n",
						    ch, NULL, NULL, TO_ROOM);
						act(AT_RED,
						    "You grab your chest and vomit up some blood!\r\n",
						    ch, NULL, NULL, TO_CHAR);
					}
				}
			}
			...
			if (get_age(ch) < 100 && !IS_IMMORTAL(ch)
			    && !SET_BIT(ch->deaf, CHANNEL_AVTALK))
				SET_BIT(ch->deaf, CHANNEL_AVTALK);

			// Gradually lose Adrenaline - Gatz
			if (!IS_NPC(ch) && ch->perm_frc > 0 && ch->desc) {
				recov = number_range(1, 2);
				if (ch->perm_frc < recov)
					ch->perm_frc = 0;
				else
					ch->perm_frc -= recov;
				if (ch->perm_frc == 0)
					send_to_char
					    ("&RYour adrenaline subsides.\n\r",
					     ch);
			}
...
void obj_update(void)
{
	...
	for (obj = last_object; obj; obj = gobj_prev) {
		...
		if (obj->item_type == ITEM_WEAPON && obj->carried_by &&
		    ...
		    && obj->value[3] != WEAPON_HEAVY_WEAPON) {
			obj->value[4]--;
		}
...
void aggr_update(void)
{
	...
	for (d = first_descriptor; d; d = dnext) {
		...
		for (ch = wch->in_room->first_person; ch; ch = ch_next) {
			...
			if (!IS_NPC(ch)
			    || ch->fighting || IS_AFFECTED(ch, AFF_CHARM)
			    || !IS_AWAKE(ch)
		...
		if (--tele->timer <= 0) {
			if (tele->room->first_person) {
				teleport(tele->room->first_person,
					 tele->room->tele_vnum, TELE_TRANSALL);
			}
...
void update_handler(void)
{
	...
	CHAR_DATA *victim = NULL, *last = NULL;
	DESCRIPTOR_DATA *d;
	bool found;
	BOUNTY_DATA *bounty;
	int count = 0;
		...
		if (count > 1) {
			for (d = first_descriptor; d; d = d->next)
				if ((d->connected == CON_PLAYING
				    ...
				    && !IS_SET(victim->act, PLR_WIZINVIS)
				    && (bounty =
					get_disintigration(victim->name)) > 0
				    && bounty) {
					victim = d->character;
...
//      citizen_generate( );
	...
	if (--pulse_point <= 0) {
		pulse_point =
		    number_range(PULSE_TICK * 0.75, PULSE_TICK * 1.25);
		auth_update();	/* Gorog */
		weather_update();
		char_update();
		obj_update();
...
/* Usage monitor */
	...
	if ((current_time % 1800) == 0) {
		sprintf(buf, "%s: %d players", ctime(&current_time),
			num_descriptors);
		append_to_file(USAGE_FILE, buf);
	}
...
/*  if ( timecheck == 15 )*/
	if (new_boot_time_t - current_time <= 900) {
		if (fifteen == FALSE) {
			sprintf(buf,
				"The aura of magic which once surrounded the realms seems slightly unstable.");
			echo_to_all(AT_YELLOW, buf, ECHOTAR_ALL);
			fifteen = TRUE;
		}
		return;
...
void reset_ship_weapons(void)
{
	...
	for (ship = first_ship; ship; ship = ship->next) {
		for (weapon = ship->first_weapon; weapon;
