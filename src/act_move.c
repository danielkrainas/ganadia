...
ch_ret move_char(CHAR_DATA * ch, EXIT_DATA * pexit, int fall)
{
	...
	if (!fall && IS_AFFECTED(ch, AFF_CHARM)
...
void do_sit(CHAR_DATA * ch, char *argument)
{
	...
	case POS_SLEEPING:
		...
		act(AT_ACTION, "$n wakes and sits up.", ch, NULL, NULL,
		    TO_ROOM);
...
void do_rest(CHAR_DATA * ch, char *argument)
{
	...
	case POS_SLEEPING:
		...
		act(AT_ACTION, "$n rouses from $s slumber.", ch, NULL, NULL,
		    TO_ROOM);
	...
	case POS_SITTING:
		...
		act(AT_ACTION, "$n lies back and sprawls out to rest.", ch,
		    NULL, NULL, TO_ROOM);
		ch->position = POS_RESTING;
		break;
...
void do_sleep(CHAR_DATA * ch, char *argument)
{
	...
	case POS_RESTING:
		...
		send_to_char("You close your eyes and drift into slumber.\n\r",
			     ch);
		act(AT_ACTION,
		    "$n closes $s eyes and drifts into a deep slumber.", ch,
		    NULL, NULL, TO_ROOM);
	...
	case POS_SITTING:
		...
		act(AT_ACTION,
		    "$n nods off and slowly slumps over, dead asleep.", ch,
		    NULL, NULL, TO_ROOM);
		ch->position = POS_SLEEPING;
		break;
	...
	case POS_STANDING:
		if (ch->mental_state > 30
		    && number_percent() < ch->mental_state) {
			send_to_char
			    ("You just can't seem to calm yourself down enough to sleep.\n\r",
			     ch);
			act(AT_ACTION,
		...
		act(AT_ACTION, "$n collapses into a deep sleep.", ch, NULL,
		    NULL, TO_ROOM);
		ch->position = POS_SLEEPING;
		break;
...
void do_enter(CHAR_DATA * ch, char *argument)
{
	...
	if (!IS_NPC(ch) && ch->perm_frc > 1) {
		chance = number_range(1, 2);
		if (chance == 1) {
			send_to_char
			    ("&RYour adrenaline rush makes you fumble around with the door!\n\r",
			     ch);
			return;
		}
	}
