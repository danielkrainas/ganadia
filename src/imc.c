...
#include "mud.h"
#endif
#if defined(IMCROM) || defined(IMCMERC)
#include "merc.h"
#ifdef IMCROM
#include "tables.h"
#endif
...
{
	if (!astr || astr[0] == '\0') {
		bug("imc_str_prefix: null astr.");
		return TRUE;
	}

	if (!bstr || bstr[0] == '\0') {
		bug("imc_str_prefix: null bstr.");
		return TRUE;
	}
...
imccolor_struct imccolor_table[] = {
	...
	,			// Purple/Magenta
	{"&p", "\e[0;0;35m", "~m"}
	,			// Duplicated to support duplicate IMC code for color
	{"&c", "\e[0;0;36m", "~c"}
	,			// Cyan
	{"&w", "\e[0;0;37m", "~w"}
	,			// Grey
	{"&w", "\e[0;0;37m", "~d"}
	,			// Duplicated to support triplicate IMC code for color
	{"&w", "\e[0;0;37m", "~!"}
	,			// Triplicated to support IMC reset code for color (defaults to grey)
	...
	/* Text Affects */
	{"&d", "\e[0m", ""}
	,			// Reset Text to client default color
	{"&D", "\e[0m", ""}
	,			// Reset Text to AFKMUD default color 
	{"&L", "\e[1m", ""}
	,			// Bolden Text(Brightens it)
	{"&u", "\e[4m", ""}
	,			// Underline Text
	{"&$", "\e[5m", ""}
	,			// Blink Text
	...
	for (count = 0; count < LSS; count++) {
		...
		if (!IS_SET(IMC_PFLAGS(ch), PSET_IMCPCOLOR)) {
			for (c = 0; imccolor_table[c].mud != NULL; c++) {
				if (imccolor_table[c].mud[0] == code[0]
				    && imccolor_table[c].mud[1] == code[1]) {
					for (x = 0;
					     imccolor_table[c].ansi[x] != '\0';
					     x++) {
						tbuf[count] =
						    imccolor_table[c].ansi[x];
						count++;
					}
					break;
				}
	...
	for (current = s, out = buf, count = 0;
		...
		if (c != 4) {
			for (i = 0; imccolor_table[i].mud != NULL; i++) {
				...
				if (l
				    && !strncmp(current, imccolor_table[i].imc,
						l))
					break;
			}
	...
	for (current = s, out = buf, count = 0;
		...
		if (c != 4) {
			for (i = 0; imccolor_table[i].mud != NULL; i++) {
				...
				if (l
				    && !strncmp(current, imccolor_table[i].mud,
						l))
					break;
			}
...
char *EmptyBlock(const char *argument, int leftsidespacing,
		 int rightsidespacing)
{
	static char buf[LSS];
	int length =
...
char *Alternating_Symbols(const char *argument, const char *symbol1,
			  const char *symbol2)
{
	static char buf[LSS];
	bool start = TRUE;
...
char *imc_makename(char *player, char *mud)
{
...
static struct {
	...
	{
	"IMC-Status", ALLOW_IMCSTATUS, DENY_IMCSTATUS, PSET_IMCSTATUS,
		    PERM_IMMORTAL}, {
	"IMC-Chan-FYI", ALLOW_IMCCHANFYI, DENY_IMCCHANFYI,
		    PSET_IMCCHANFYI, PERM_IMMORTAL}, {
	"imcptell", ALLOW_NOTSUPPORTED, DENY_IMCPTELL, PSET_IMCPTELL,
		    PERM_PLAYER}, {
	"imcpbeep", ALLOW_NOTSUPPORTED, DENY_IMCPBEEP, PSET_IMCPBEEP,
		    PERM_PLAYER}, {
	"imcpvisible", ALLOW_IMCPVISIBLE, DENY_IMCPVISIBLE,
		    PSET_IMCPVISIBLE, PERM_PLAYER}, {
	"imcpcolor", ALLOW_NOTSUPPORTED, DENY_NOTSUPPORTED,
		    PSET_IMCPCOLOR, PERM_PLAYER}
};
...
void
receive_oldchannel(imc_char_data * from, int number, char *argument, int emote)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
...
void imc_recv_chat(imc_char_data * from, int channel, char *argument)
{
	receive_oldchannel(from, channel, argument, 0);
}

void imc_recv_emote(imc_char_data * from, int channel, char *argument)
{
...
void
imc_recv_imcptell(imc_char_data * from, char *to, char *argument, int isreply)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
	IMC_BLACKLIST *entry;
	...
	snprintf(buf, IMC_DATA_LENGTH, tstring, from->name,
		 color_itom(argument));
...
bool imc_audible(IMC_CHANNEL * c, char *who)
{
 ...
 */
void process_imcplist(imc_char_data * from, char *argument)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *person;
	...
	case 1:		// The Winter Who Style by Kris
		...
		imc_whoreply_add("\n\r");
		snprintf(buf2, LSS, "%s~C|",
			 Alternating_Symbols(buf, "~W", "~C"));
		imc_whoreply_add("~C|");
	...
	default:		// IMC2 4.00 Continuum Default Who Style by Kris
		...
		imc_whoreply_add("\n\r");
		snprintf(buf2, LSS, "%s~G|",
			 Alternating_Symbols(buf, "~Y", "~C"));
		imc_whoreply_add("~G|");
	...
	if (i > 0) {
		...
		for (d = first_descriptor; d; d = d->next) {
			if (d->character && d->connected == CON_PLAYING) {
				...
				strlcpy(rank,
					SetFill(imcrankbuffer(person), 22, " "),
					LSS);
	...
	if (i > 0) {
		...
		case 1:	// The Winter Who Style by Kris
			imc_whoreply_add("\n\r\n\r");
			imc_whoreply_add(SetFill
					 ("  ~GMortal Players  ", 78, "~W*"));
			break;
		case 2:	// The Fiery Bowels of Hell Who Style by Kris
			imc_whoreply_add("\n\r\n\r");
			imc_whoreply_add(SetFill
					 ("  ~RMortal Players  ", 78, "~Y*"));
			break;
		case 3:	// IMC2 Classic Who Style - Slight altered from older versions of the IMC2 client
			imc_whoreply_add(SetFill
					 ("-=[ ~WPlayers ~B]=-", 78, "~B-"));
...
void process_imcpinfo(imc_char_data * from, char *to)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
	...
	for (i = 0; i < numfunctions; i++) {
		if (function_usable
		    (victim, imc_functions[i].deny_flag,
	...
	snprintf(buf, LSS, "~cPlayer Information\n\r"
		 ...
		 CH_IMCNAME(victim),
		 male_female[URANGE(0, CH_IMCSEX(victim), 2)], permissions,
		 (IMC_CSUBSCRIBED(victim)
...
char *imc_getstats(char *choice)
{
	...
	if (!choice || choice[0] == '\0' || !strcasecmp(choice, "network")) {
		...
		snprintf(buf, IMC_DATA_LENGTH,
			 ...
			 imc_stats.tx_bytes / secondspast, imc_stats.max_pkt,
			 evcount, imc_stats.sequence_drops,
	...
	if (!strcasecmp(choice, "general")) {
		snprintf(buf, IMC_DATA_LENGTH,
			 ...
			 imc_siteinfo.port, imc_siteinfo.www,
...
void imc_recv_imcpbeep(imc_char_data * from, char *to)
{
	DESCRIPTOR_DATA *d;
	CHAR_DATA *victim;
	IMC_BLACKLIST *entry;
...
void imc_recv_keepalive(char *from, char *version, char *networkname)
{
	REMOTEINFO *p;
...
void imc_send_traceroute(char *to, char *playername)
{
	PACKET out;
...
void imc_send_traceroutereply(char *to, char *path)
{
	PACKET out;
...
void imc_recv_traceroute(char *from, char *path)
{
...
void
imc_recv_traceroutereply(char *from, char *to, char *pathto, char *pathfrom)
{
	...
	imc_printf(ch, color_itom("~WTraceroute Information for %s\n\r"
				  "~cSend Path    ~W:   ~C%s\n\r"
				  "~cReturn Path  ~W:   ~C%s\n\r\n\r"),
...
#if !defined (IMCCIRCLE)
	IMCCREATE(ch->pcdata->imcchardata, IMC_CHARDATA, 1);
#endif
...
bool imc_loadchar(CHAR_DATA * ch, FILE * fp, char *word)
{
	if (IS_NPC(ch))
		return FALSE;
...
bool imc_read_config(void)
{
	...
	if ((fin = fopen(IMC_CONFIG_FILE, "r")) == NULL) {
...
void imc_readblacklist(void)
{
	...
	if (!(inf = fopen(IMC_BLACKLIST_FILE, "r"))) {
...
void imc_loadhistory(void)
{
	...
	for (tempchan = first_imc_channel; tempchan; tempchan = tempchan->next) {
		...
		snprintf(filename, SSS, "%s/%s.hist", "../imc",
			 tempchan->local_name);
...
void imc_startup(bool force)
{
	...
	if (!force) {
		imclog("%s",
		       "IMC2 data loaded. Autoconnect not set. IMC will need to be connected manually.");
		return;
	}
...
void imc_shutdown(bool reconnect)
{
	...
	if (reconnect) {
		...
		imclog("%s",
		       "Connection to hub was lost. Reconnecting in approximately 20 seconds.");
	}
...
void
imc_recv_msg_r(char *from, char *realfrom, char *chan, char *txt, int emote)
{
	IMC_CHANNEL *c;
	char *mud;
	...
	if (!c->local_name
	    || (c->policy != CHAN_PRIVATE && c->policy != CHAN_CPRIVATE))
		return;
...
void imc_recv_msg_b(char *from, char *chan, char *txt, int emote)
{
	IMC_CHANNEL *c;
...
void
imc_recv_update(char *from, char *chan, char *owner, char *operators,
		char *policy, char *invited, char *excluded)
{
	IMC_CHANNEL *c;
	char *mud;
...
int imc_recv_chan(PACKET * p, int bcast)
{
	...
	if (!strcasecmp(p->type, "ice-msg-r")) {
		imc_recv_msg_r(p->from, imc_getkey(p, "realfrom", ""),
			       imc_getkey(p, "channel", ""), imc_getkey(p,
									"text",
									""),
		...
		imc_recv_msg_b(p->from, imc_getkey(p, "channel", ""),
			       imc_getkey(p, "text", ""), imc_getkeyi(p,
								      "emote",
								      0));
		return 1;
	} else if (!strcasecmp(p->type, "ice-update")) {
		imc_recv_update(p->from, imc_getkey(p, "channel", ""),
				imc_getkey(p, "owner", ""), imc_getkey(p,
								       "operators",
								       ""),
				imc_getkey(p, "policy", ""), imc_getkey(p,
									"invited",
									""),
...
void run_imccadmin(CHAR_DATA * ch, char *argument)
{
...
void run_imccsettings(CHAR_DATA * ch, char *argument)
{
	...
	if (!strcasecmp(field, "show")) {
		...
		case CHAN_CLOSED:
			polly =
			    "~RClosed -- Sent to all muds but only invited people can talk or listen";
			break;
		case CHAN_PRIVATE:
			polly =
			    "~yPrivate -- Only invited people can talk or listen";
			break;
		case CHAN_CPRIVATE:
	...
	if (!strcasecmp(field, "rename")) {
		...
		imc_printf(ch,
			   "Channel %s has had its local name successfully changed to %s.\n\r",
			   channel->name, channel->local_name);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "permlevel")) {
		...
		imc_printf(ch,
			   "That channel has had its minimum permission level changed to %d.\n\r",
			   value);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "regformat")) {
		...
		imc_printf(ch,
			   "Channel %s has successfully had its regular format string changed.\n\r",
			   channel->name);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "emoteformat")) {
		...
		if (!verify_format(argument, 2)) {
			imc_to_char
			    ("The emote format must contain two %s's.\n\r", ch);
			return;
		}
		...
		imc_printf(ch,
			   "Channel %s has successfully had its emote format string changed.\n\r",
			   channel->name);
		imc_save_channels();
		return;
	}
	...
	if (!strcasecmp(field, "socformat")) {
		...
		if (!verify_format(argument, 1)) {
			imc_to_char
			    ("The social format must contain one %s.\n\r", ch);
			return;
		}
		...
		imc_printf(ch,
			   "Channel %s has successfully had its social format string changed.\n\r",
			   channel->name);
		imc_save_channels();
		return;
	}
...
void run_imcclist(CHAR_DATA * ch, char *argument)
{
	char buf[LSS];
	char *polly;
	IMC_CHANNEL *c;
	...
	for (c = first_imc_channel; c; c = c->next) {
		...
		snprintf(buf + strlen(buf), LSS,
			 ...
			 c->local_name ? 15 : 17,
			 c->local_name ? c->local_name : "~R(not local)  ",
			 c->owner, c->perm_level ? c->perm_level : 0, polly);
	}
...
void imc_saveblacklist(void)
{
	...
	if (!(out = fopen(IMC_BLACKLIST_FILE, "w"))) {
		imcbug("%s",
...
void imc_send_whois(imc_char_data * from, char *to)
{
	PACKET out;
...
void run_imcptell(CHAR_DATA * ch, char *argument)
{
...
void run_imcpreply(CHAR_DATA * ch, char *argument)
{
	char buf[LSS + 30];
...
void run_imcpbeep(CHAR_DATA * ch, char *argument)
{
	char buf[LSS];
	imc_char_data *chdata = imc_getdata(ch);
...
void run_imcpafk(CHAR_DATA * ch, char *argument)
{
	CHECKIMC(ch);
...
void run_imcplist(CHAR_DATA * ch, char *argument)
{
	imc_char_data *chdata = imc_getdata(ch);
...
void run_imcpfind(CHAR_DATA * ch, char *argument)
{
	imc_char_data *chdata = imc_getdata(ch);
...
void run_imcpinfo(CHAR_DATA * ch, char *argument)
{
	char name[LSS];
	imc_char_data *chdata = imc_getdata(ch);
...
void run_imcminfo(CHAR_DATA * ch, char *argument)
{
	char arg[LSS];
	imc_char_data *chdata = imc_getdata(ch);
	...
	CHECKMUD(ch, arg);
	imc_send_who(chdata, arg,
		     (argument && argument[0] != '\0' ? argument : "help"));
	IMCDISPOSE(chdata);
}

void run_imcmlist(CHAR_DATA * ch, char *argument)
{
	CHECKIMC(ch);
...
void run_imcmsettings(CHAR_DATA * ch, char *argument)
{
	char arg1[MSS];
	...
	if (!strcasecmp(arg1, "connected")) {
		...
		if (!strcasecmp(argument, "yes")) {
			if (this_imcmud->state > CONN_NONE) {
				imc_to_char
				    ("The IMC client is already connected to the hub.\n\r",
				     ch);
				return;
			}

			imc_to_char
			    ("The IMC client will now connect itself to the hub.\n\r",
			     ch);
			imcwait = 0;
			imc_startup(TRUE);
			return;
		...
		if (!strcasecmp(argument, "no")) {
			if (imcwait > 0) {
				imc_to_char
				    ("The IMC client will now stop its reconnect process to the hub.\n\r",
				     ch);
				imcwait = 0;
				return;
			}
			...
			if (this_imcmud->state < CONN_COMPLETE) {
				imc_to_char
				    ("The IMC client is already disconnected from the hub.\n\r",
				     ch);
				return;
			}

			imc_to_char
			    ("The IMC client will now disconnect itself from the hub.\n\r",
			     ch);
			imc_shutdown(FALSE);
			return;
		}
	...
	if (!strcasecmp(arg1, "netname")) {
		...
		imc_printf(ch,
			   "The IMC2 network that this mud is connected to will now be known as %s\n\r",
			   argument);
		imc_save_config();
		return;
	}
	...
	if (!strcasecmp(arg1, "whostyle")) {
		...
		if (!is_number(argument)) {
			for (value = 0; value < MAX_WHOSTYLE; value++)
				if (!imc_str_prefix
				    (whostylename_table[value], argument))
					break;
		} else
			value = atoi(argument);
		...
		imc_whostyle = value;
		imc_printf(ch,
			   "The color pattern style for your imcplist has been changed to %s.\n\r",
...
void run_imcmblacklist(CHAR_DATA * ch, char *argument)
{
	int count;
	IMC_BLACKLIST *entry;
	char arg[MSS];
...
void run_imcmstat(CHAR_DATA * ch, char *argument)
{
	CHECKIMC(ch);
...
void run_imcpsettings(CHAR_DATA * ch, char *argument)
{
	char arg[MSS];
	bool toggle;
	unsigned char mode;
	...
	if (!strcasecmp(arg, "show")) {
		...
		for (i = 0; i < numfunctions; i++) {
			if (!function_usable
			    ...
			    if (IS_SET
				(IMC_PFLAGS(ch), imc_functions[i].pset_flag)) {
	...
	while ((argument = imc_getarg(argument, arg, MSS)), arg[0]) {
		...
		} else if (mode == 2)	// Blacklist Entries
		{
			...
			if (entry) {
				if (toggle) {
					if ((strchr(arg + 1, '@')))
						imc_to_char
						    ("That person is already on your blacklist.\n\r",
						     ch);
					else
						imc_to_char
						    ("That mud is already on your blacklist.\n\r",
						     ch);
					return;
				}
				...
				if ((strchr(arg + 1, '@')))
					imc_to_char
					    ("That person has been removed from your blacklist.\n\r",
					     ch);
				else
					imc_to_char
					    ("That mud has been removed from your blacklist.\n\r",
					     ch);
			} else {
				if (!toggle) {
					if ((strchr(arg + 1, '@')))
						imc_to_char
						    ("That person is not on your blacklist.\n\r",
						     ch);
					else
						imc_to_char
						    ("That mud is not on your blacklist.\n\r",
						     ch);
					return;
				}
				...
				if ((strchr(arg + 1, '@')))
					imc_to_char
					    ("That person has been added to your blacklist.\n\r",
					     ch);
				else
					imc_to_char
					    ("That mud has been added to your blacklist.\n\r",
					     ch);
			}
		...
		} else		// Network Functions
		{
			if (!strcasecmp(arg + 1, "all")) {
				if (toggle) {
					imc_to_char
					    ("ALL available IMC network functions are now on.\n\r",
					     ch);
					...
					imc_to_char
					    ("ALL available IMC network functions are now off.\n\r",
					     ch);
				...
				if (i == numfunctions) {
					imc_printf(ch,
						   "You don't have access to an IMC network function called \"%s\".\n\r",
						   arg + 1);
					continue;
				}
			}
...
void run_imcpflags(CHAR_DATA * ch, char *argument)
{
	char arg[MSS];
	...
	case 0:		/* set deny flag */
		...
		if (get_permlevel(victim) < imc_functions[i].minlevel) {
			imc_to_char
			    ("That player is already below this network function's permission level.\n\r",
			     ch);
			return;
		}
	...
	case 1:		/* set allow flag */
		if (get_permlevel(victim) >= imc_functions[i].minlevel) {
			imc_to_char
			    ("That player is already at or above this network function's permission level.\n\r",
			     ch);
			return;
		}
		...
		if (imc_functions[i].allow_flag == ALLOW_NOTSUPPORTED) {
			imc_to_char
			    ("That network function can not be specially allowed.\n\r",
			     ch);
			return;
		}
 ...
 */
void run_imcmfind(CHAR_DATA * ch, char *argument)
{
	CHECKIMC(ch);
...
// Channel Directed Commands
	...
	{"imcclist", run_imcclist, PERM_PLAYER}
	,			// Continuum Version
	{"imcchanlist", run_imcclist, PERM_PLAYER}
	,			// Mud-Net Version
	{"ilist", run_imcclist, PERM_PLAYER}
	,			// Original IMC2 Version
...
// Player Directed Commands
	...
	,			// Mud-Net Version covering part of functionality
	{"imclisten", run_imcpsettings, PERM_PLAYER}
	,			// Mud-Net Version covering part of functionality
	{"rchannels", run_imcpsettings, PERM_PLAYER}
	,			// Original IMC2 Version covering part of functionality
...
char *return_imcsocial(char *socialname, unsigned char type)
{
#ifdef SOCIALSTRUCT
	SOCIAL_DATA *social;
#else
...
#ifdef SOCIALSTRUCT
	if ((social = find_social(socialname)) == NULL)
		return NULL;

	if (type == 1)
		return social->others_found;
	else if (type == 2)
		return social->others_auto;
	else
		return social->others_no_arg;
#else
...
char *imc_act_string(const char *format, CHAR_DATA * ch, char *vname)
{
...
bool imc_channel_handler(CHAR_DATA * ch, char *command, char *argument)
{
	IMC_CHANNEL *c;
	DESCRIPTOR_DATA *d;
	...
	/* Social code redone by Rogel, based off of work performed by Senir */
		...
		if (vname && vname[0] != '\0') {
			...
			if (strcasecmp(vname, "Self")
				...
				|| (separator
				    && strcasecmp(imc_mudof(vname),
						  imc_name)))) {
				if (!separator) {
					imc_to_char
					    ("The social target must be in the form of person@mud.\n\r",
					     ch);
					return TRUE;
				}
		...
		if (return_imcsocial(socialname, type) == NULL
			...
			imc_printf(ch,
				   "%s is either not a social or that part of it is blank.\n\r",
				   socialname);
			return TRUE;
		}

		strlcpy(newstring,
			imc_act_string(return_imcsocial(socialname, type), ch,
				       vname), LSS);
 ...
 * in the SSM, without this the entire usage by IMC2 is
 * reported as a big leak. We only deal with strings
 * as IMC2 v3m does NOT use the mud alloc() funcs,
...
{
	IMC_BLACKLIST *blacklistPtr = NULL;
	IMC_CHANNEL *ICEPtr = NULL;
	REMOTEINFO *remInfoPtr = NULL;
	CHAR_DATA *chPtr = NULL;
	int i = 0;

	markfn(wr_to);
	markfn(imc_versionid);

	markfn(this_imcmud->hubname);
	markfn(this_imcmud->host);
	markfn(this_imcmud->clientpw);
	markfn(this_imcmud->serverpw);
	...
	for (blacklistPtr = first_imc_mudblacklist; blacklistPtr;
	     blacklistPtr = blacklistPtr->next)
		markfn(blacklistPtr->name);

	for (ICEPtr = first_imc_channel; ICEPtr; ICEPtr = ICEPtr->next) {
		markfn(ICEPtr->name);
		markfn(ICEPtr->owner);
		markfn(ICEPtr->operators);
		markfn(ICEPtr->invited);
		markfn(ICEPtr->excluded);
		markfn(ICEPtr->local_name);
		markfn(ICEPtr->regformat);
		markfn(ICEPtr->emoteformat);
		markfn(ICEPtr->socformat);

		for (i = 0; i < 20; i++)
			markfn(ICEPtr->history[i]);
	}

	for (remInfoPtr = first_rinfo; remInfoPtr;
	     remInfoPtr = remInfoPtr->next) {
		markfn(remInfoPtr->name);
		markfn(remInfoPtr->path);
		markfn(remInfoPtr->version);
		markfn(remInfoPtr->netname);
	}
	...
	for (chPtr = first_char; chPtr; chPtr = chPtr->next) {
		if (!IS_NPC(chPtr)) {
			IMC_BLACKLIST *blacklistPtr;

			markfn(IMC_CSUBSCRIBED(chPtr));
			markfn(IMC_RREPLY(chPtr));
			for (blacklistPtr = FIRST_IMCBLACKLIST(chPtr);
			     blacklistPtr; blacklistPtr = blacklistPtr->next)
				markfn(blacklistPtr->name);
		}
	}
