...
/* And, of course, you will need to change the following lines for YOUR
   ...
   build up quest points :> Make the item worth their while. */
	...
	if (!strcmp(arg1, "list")) {
		act(AT_PLAIN, "$n asks $N for a list of quest items.", ch, NULL,
		    questman, TO_ROOM);
		act(AT_PLAIN, "You ask $N for a list of quest items.", ch, NULL,
		    questman, TO_CHAR);
...
[7] 500qp........30 Practices\n\r", QUEST_VALUE1, obj1->short_descr,
		...
		if (is_name(arg2, "1")) {
			...
			if (ch->pcdata->quest_curr >= 500) {
				...
				ch->gold += 10000000;
				act(AT_MAGIC,
				    "$N gives 10,000,000 wulongs to $n.", ch,
				    NULL, questman, TO_ROOM);
				act(AT_MAGIC, "$N hands you a cred stick.", ch,
		...
		if (ch->nextquest > 0) {
			sprintf(buf,
				...
				ch->name);
			do_say(questman, buf);
			sprintf(buf, "Please come back in about %d minutes.",
				ch->nextquest);
			do_say(questman, buf);
			return;
		}
		...
		if (IS_SET(ch->act, PLR_QUESTOR)) {
			if (ch->questmob == -1 && ch->countdown > 0) {
				...
				if (qchance(15)) {
					dipexpreward =
					    (ch->
					     ...
					     1000);
					sprintf(buf,
						"You gain %d diplomactic experience!\n\r",
						dipexpreward);
					send_to_char(buf, ch);
				...
				if (obj_found == TRUE) {
					...
					act(AT_PLAIN, "You hand $p to $N.", ch,
					    obj, questman, TO_CHAR);
					act(AT_PLAIN, "$n hands $p to $N.", ch,
		...
		act(AT_PLAIN,
		    "$n informs $N $e can no longer continue on $s quest.", ch,
		    NULL, questman, TO_ROOM);
		act(AT_PLAIN,
		    "You inform $N you can no longer continue on $s quest.", ch,
		    NULL, questman, TO_CHAR);
		if (ch->questgiver != questman) {
			sprintf(buf,
				"I never sent you on a quest! Perhaps you're thinking of someone else.");
		...
		if (IS_SET(ch->act, PLR_QUESTOR)) {
			if (ch->questmob != 0) {
				sprintf(buf,
					"I am very dissapointed that you could not bring the fugitive to justice.");
				do_say(questman, buf);
				REMOVE_BIT(ch->act, PLR_QUESTOR);
				ch->questgiver = NULL;
				ch->countdown = 0;
				...
				if (obj_found == FALSE) {
					sprintf(buf,
						"I am very dissapointed that our treasure could not be recoverd!");
					do_say(questman, buf);
					...
					sprintf(buf,
						"But you have recovered the object, why would you want to quit?!");
					do_say(questman, buf);
					return;
				}
			} else
				sprintf(buf,
					"I can't remember your quest, perhaps you should try another.");
			do_say(questman, buf);
			REMOVE_BIT(ch->act, PLR_QUESTOR);
			ch->questgiver = NULL;
			ch->countdown = 0;
		...
		sprintf(buf,
			"Perhaps you should obtain a quest before you try to fail it.");
		do_say(questman, buf);
		return;
	}
...
void generate_quest(CHAR_DATA * ch, CHAR_DATA * questman)
{
	...
	for (mcounter = 0; mcounter < 99999; mcounter++) {
		...
		if ((vsearch = get_mob_index(mob_vnum)) != NULL) {
			...
			if (((level_diff < 5 && level_diff > -5)
				 ...
				 && vsearch->level > 30 && vsearch->level < 50)
	...
	if (qchance(40)) {
		...
		sprintf(buf, "Vile pilferers have stolen %s from the treasury!",
	...
	/* Quest to kill a mob */
		...
		case 0:
			sprintf(buf,
				"An enemy of mine, %s, is making vile threats against the city.",
				victim->short_descr);
			do_say(questman, buf);
		...
		case 1:
			sprintf(buf,
				"Airu's most heinous criminal, %s, has escaped from the dungeon!",
				victim->short_descr);
			do_say(questman, buf);
			sprintf(buf,
